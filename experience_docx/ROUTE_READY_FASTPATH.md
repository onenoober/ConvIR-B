...
GitHub-main commit equal to `rules_commit`; it is not copied into the route
worktree. Finalize also writes one non-committed Git-private authoring receipt
...
files; route-ready consumes that receipt and rejects byte drift, while the MCP
...
rerunning the validator. A changed staged tree, main commit, operation set or
authoring receipt invalidates that cache.
