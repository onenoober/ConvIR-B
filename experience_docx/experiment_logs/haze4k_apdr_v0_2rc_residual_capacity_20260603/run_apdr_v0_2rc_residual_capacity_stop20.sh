...
LOG_DIR="$ROOT/experience_docx/experiment_logs/haze4k_apdr_v0_2rc_residual_capacity_20260603"
...
MODEL_NAME=${MODEL_NAME:-ConvIR-Haze4K-APDR-v0.2RC-residual-capacity-stop20-seed3407-20260603}
EVAL_TAG=${EVAL_TAG:-apdr_v0_2rc_residual_capacity_stop20_seed3407_vs_a0}
...
log_status "start_residual_capacity root=$ROOT selector=$SELECTOR model=$MODEL_NAME"
...
if [[ -f "$BEST" ]]; then
  ...
  "$PY" main.py \
    ...
    --apdr_residual_capacity shallow_mlp \
...
"$PY" "$ROOT/experience_docx/tools/eval_haze4k_checkpoint_compare.py" \
  ...
  --candidate_name apdr_v0_2rc_residual_capacity \
  ...
  --candidate_apdr_residual_capacity shallow_mlp \
  --output_dir "$LOG_DIR" \
  --tag "$EVAL_TAG"
...
if "$PY" "$ROOT/experience_docx/tools/gate_haze4k_apdr_stop20.py" \
  ...
  --stage "APDR-v0.2RC shallow residual-capacity stop20"; then
...
log_status "complete_residual_capacity $EVAL_TAG"
