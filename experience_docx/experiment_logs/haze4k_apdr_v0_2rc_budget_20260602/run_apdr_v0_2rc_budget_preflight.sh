...
ROOT=${ROOT:-/root/autodl-tmp/workspace/ConvIR-B-apdr-convir-v0-2rc-conservative-budget}
...
LOG_DIR="$ROOT/experience_docx/experiment_logs/haze4k_apdr_v0_2rc_budget_20260602"
STATUS="$LOG_DIR/status.txt"
TAG=${TAG:-apdr_v0_2rc_budget_seed3407}
...
"$PY" "$ROOT/experience_docx/tools/preflight_haze4k_apdr.py" \
  --data_dir "$DATA_DIR" \
  --checkpoint "$PRETRAIN" \
  --output "$LOG_DIR/preflight_apdr_v0_2rc_arch.json" \
  --stage apdr_v0_2rc_arch_preflight \
  --device cuda \
  --height 256 \
  --width 256 \
  --batch_size 1 \
...
log_status "budget_preflight_start global_epochs=$GLOBAL_EPOCHS spatial_epochs=$SPATIAL_EPOCHS"
"$PY" "$ROOT/experience_docx/tools/preflight_haze4k_apdr_v0_2rc_budget.py" \
  ...
  > "$LOG_DIR/budget_preflight_${TAG}.log" 2>&1
...
PY
then
  log_status "budget_gate_pass $TAG"
else
  log_status "budget_gate_fail_stop $TAG"
  exit 0
fi

log_status "complete_budget_preflight $TAG"
