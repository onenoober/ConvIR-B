...
TAG=${TAG:-apdr_v0_4_freeparam_low_sigma3_32_seed3407}
...
log_status "start_freeparam_low_sigma3 root=$ROOT selector=$SELECTOR tag=$TAG"
...
if "$PY" "$ROOT/experience_docx/tools/overfit_haze4k_apdr_v0_4_freeparam_lowcolor.py" \
  ...
  log_status "gate_pass_freeparam_low_sigma3 $TAG"
else
  log_status "gate_fail_freeparam_low_sigma3 $TAG"
fi

log_status "complete_freeparam_low_sigma3 $TAG"
