...
LOG=$EVID/v229_p2a_diagnostics.log
GPU=${GPU:-0}
...
{
  echo "v229_p2a_start $(date --iso-8601=seconds)"
...
CUDA_VISIBLE_DEVICES="$GPU" "$PY" \
  experience_docx/tools/nopost_v229_safe_oof_action_bank_calibration.py \
  --data-dir "$DATA" \
...
decision="UNKNOWN"
if [ -f "$EVID/v229_p2a_closeout.json" ]; then
  decision=$("$PY" - <<'PY'
import json
from pathlib import Path
p = Path("/sda/home/wangyuxin/ConvIR-B/repos/ConvIR-B-v2-29-nopost-ilfrb-acs-safe-oof-action-bank-calibration/experience_docx/experiment_logs/haze4k_v2_29_nopost_ilfrb_acs_safe_oof_action_bank_calibration_20260705/v229_p2a_closeout.json")
...
{
  echo "v229_p2a_done rc=$rc decision=$decision $(date --iso-8601=seconds)"
  if [ "$rc" -eq 0 ]; then
    echo "V229_P2A_COMMAND_OK"
  else
    echo "V229_P2A_COMMAND_FAILED"
  fi
} | tee -a "$STATUS"
