...
echo "v226_p3_script_start $(date --iso-8601=seconds) gpu=$GPU_ID" | tee -a "$STATUS"
set +e
"$PY" experience_docx/tools/nopost_lowband_v226_risk_signal_audit.py \
  ...
  --out-dir "$EVID" \
  --canary-sizes 32,64 \
  --canary-epochs 40 2>&1 | tee "$LOG"
rc=${PIPESTATUS[0]}
set -e
echo "v226_p3_script_done rc=$rc $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then echo V226_P3_SCRIPT_OK; else echo V226_P3_SCRIPT_FAILED; fi
exit "$rc"
