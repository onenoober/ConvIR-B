...
v2g supports the current bottleneck diagnosis: the old global LDHN target is over-broad and should not be used as a hard RARM-positive signal. The three-state actionable target is supported as a diagnostic contract, D7c beats density-only controls under that contract, and G4b selective probes did not safely improve over D7c. This does not authorize F5, v3, RARM, D2, or locked Haze4K test access.
