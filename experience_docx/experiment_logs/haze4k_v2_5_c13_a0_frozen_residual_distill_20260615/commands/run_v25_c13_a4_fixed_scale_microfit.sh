...
STATUS=$EVID/status_c13_a4_fixed_scale_microfit.txt

mkdir -p "$EVID/runtime_logs" "$EVID/commands" "$RUNROOT"
{
  echo "state=PREFLIGHT_RUNNING"
  echo "run_id=v25_c13_a4_fixed_scale_microfit"
  ...
  echo "design=direct residual, zero head, fixed residual_scale, clamped output"
...
{
  echo "state=C13_A4_MICROFIT_RUNNING"
  echo "microfit_start_time=$(date --iso-8601=seconds)"
} >> "$STATUS"

pids=()
specs=(
  c13a4_scale050:0.50:3:rgb_wavelet:32:3:10
  c13a4_scale055:0.55:4:rgb_wavelet:32:3:10
)
for spec in "${specs[@]}"; do
  IFS=: read -r variant residual_scale gpu feature width depth epochs <<< "$spec"
  out="$RUNROOT/$variant"
  if [ -e "$out" ]; then
  ...
  pids+=("$!")
  echo "launched_a4_microfit variant=$variant gpu=$gpu residual_scale=$residual_scale pid=${pids[-1]}" | tee -a "$STATUS"
done
...
if [ "$rc" -ne 0 ]; then
  echo "C13_A4_MICROFIT_FAILED" | tee -a "$STATUS"
...
for spec in "${specs[@]}"; do
  ...
  echo "eval_a4_quick_done variant=$variant" | tee -a "$STATUS"
done
...
variants = ["c13a4_scale050", "c13a4_scale055"]
rows = []
passes = []
...
leader = evid / "v25_c13_a4_fixed_scale_microfit_leaderboard.csv"
...
decision = {
    "route": "Haze4K v2.5 C13 A4 fixed-scale direct microfit",
    "locked_test_touched": False,
    "locked_per_image_read": False,
    "quick_gate_pass_variants": passes,
    "quick_gate_pass_count": len(passes),
    "decision": "C13_A4_QUICK_PASS_RUN_FULL_VAL" if passes else "C13_A4_QUICK_FAIL_STOP_OR_REDESIGN",
    "rows": rows,
}
(evid / "v25_c13_a4_fixed_scale_microfit_decision.json").write_text(
    json.dumps(decision, indent=2, sort_keys=True) + "\n",
    encoding="utf-8",
)
print("C13_A4_QUICK_DECISION_OK", json.dumps(decision, sort_keys=True))
PY

pass_variants=$("$PY" - "$EVID/v25_c13_a4_fixed_scale_microfit_decision.json" <<'PY'
import json, sys
...
if [ -n "$pass_variants" ]; then
  echo "a4_full_val_start variants=$pass_variants" | tee -a "$STATUS"
  for variant in $pass_variants; do
    residual_scale=0.50
    gpu=3
    if [ "$variant" = "c13a4_scale055" ]; then
      residual_scale=0.55
      gpu=4
    fi
    (
    ...
    echo "eval_a4_full_val_done variant=$variant" | tee -a "$STATUS"
  done
fi

{
  ...
  if [ -n "$pass_variants" ]; then
    echo "state=C13_A4_MICROFIT_DONE_FULL_VAL_REVIEW_BEFORE_B_SCREEN"
  else
    echo "state=C13_A4_MICROFIT_FAIL_STOP_OR_REDESIGN"
  fi
  echo "C13_A4_FIXED_SCALE_MICROFIT_OK"
} >> "$STATUS"
