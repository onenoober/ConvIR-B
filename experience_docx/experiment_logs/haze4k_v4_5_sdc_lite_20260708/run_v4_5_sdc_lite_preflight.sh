...
LOG=$EVID/v4_5_sdc_lite_preflight.log
JSON_OUT=$EVID/v4_5_sdc_lite_preflight.json
export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-2}
export TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1
mkdir -p "$EVID"
{
  echo "preflight_start v4_5_sdc_lite $(date --iso-8601=seconds)"
  echo "work=$WORK"
  echo "data=$DATA"
...
from models.SDCConvIR import build_sdc_lite_net, configure_sdc_train_scope, load_haze4k_partial
from data import train_dataloader

WORK = Path('/sda/home/wangyuxin/ConvIR-B/repos/ConvIR-B-haze4k-v4-5-sdc-lite')
...
TRAIN_SPLIT = WORK / 'docs/ai_text_packages/haze4k_v4_sfad/splits/haze4k_train_adapter_train.txt'
HOLDOUT_SPLIT = WORK / 'docs/ai_text_packages/haze4k_v4_sfad/splits/haze4k_train_internal_holdout256.txt'
JSON_OUT = WORK / 'experience_docx/experiment_logs/haze4k_v4_5_sdc_lite_20260708/v4_5_sdc_lite_preflight.json'
...
route = build_sdc_lite_net('base', 'Haze4K', 'original')
partial = load_haze4k_partial(route, str(A0), allowed_new_prefixes=('SFAD_SDC',))
route.to(device).eval()
scope = configure_sdc_train_scope(route, 'adapter_only')
route.eval()
...
result['sdc_stats'] = route.collect_modulation_stats(input_img)

missing_ok = all(k.startswith('SFAD_SDC') for k in partial['missing_new_modules'])
stats = result['sdc_stats']['SDC_1_2']
result['pass'] = bool(
    ...
    and scope['trainable_prefixes'] == ['SFAD_SDC']
...
rc=$?
set -e
echo "preflight_done rc=$rc v4_5_sdc_lite $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [[ "$rc" -eq 0 ]]; then
  echo "V4_5_SDC_LITE_PREFLIGHT_OK" | tee -a "$STATUS"
else
  echo "V4_5_SDC_LITE_PREFLIGHT_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
