...
EVID=$WORK/experience_docx/experiment_logs/haze4k_v4_6_dcfsb_bottleneck_20260708_adapter3
PY=$BASE/envs/convir-cu121/bin/python
DATA=$BASE/datasets/Haze4K/Haze4K
A0=$BASE/checkpoints/official/Haze4K/haze4k-base.pkl
CAND=$ITS/results/ConvIR-Haze4K-v4A6-DCFSB-Bottleneck-adapter3-notest-seed3407-20260708/Training-Results/Final.pkl
...
LOG=$EVID/audit_v4_6_dcfsb_adapter3_internal256.log
export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-2}
export TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1
mkdir -p "$EVID"
{
  echo "audit_start v4_6_dcfsb_adapter3_internal256 $(date --iso-8601=seconds)"
  echo "data=$DATA"
  echo "a0=$A0"
  echo "candidate=$CAND"
...
CAND = Path('/sda/home/wangyuxin/ConvIR-B/repos/ConvIR-B-haze4k-v4-6-dcfsb-bottleneck-independent/Dehazing/ITS/results/ConvIR-Haze4K-v4A6-DCFSB-Bottleneck-adapter3-notest-seed3407-20260708/Training-Results/Final.pkl')
EVID = Path('/sda/home/wangyuxin/ConvIR-B/repos/ConvIR-B-haze4k-v4-6-dcfsb-bottleneck-independent/experience_docx/experiment_logs/haze4k_v4_6_dcfsb_bottleneck_20260708_adapter3')
...
summary = {
    'route_id': 'haze4k_v4_6_dcfsb_bottleneck_20260708_adapter3',
...
(EVID / 'v46_dcfsb_adapter3_audit_summary.json').write_text(json.dumps(summary, indent=2, sort_keys=True) + '\n', encoding='utf-8')
(EVID / 'v46_dcfsb_adapter3_gate.json').write_text(json.dumps(gate, indent=2, sort_keys=True) + '\n', encoding='utf-8')
with (EVID / 'v46_dcfsb_adapter3_per_image.csv').open('w', newline='', encoding='utf-8') as f:
    writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
    writer.writeheader()
    writer.writerows(rows)
with (EVID / 'v46_dcfsb_adapter3_module_stats.jsonl').open('w', encoding='utf-8') as f:
...
rc=$?
set -e
echo "audit_done rc=$rc v4_6_dcfsb_adapter3_internal256 $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [[ "$rc" -eq 0 ]]; then
  echo "V4_6_DCFSB_ADAPTER3_INTERNAL256_AUDIT_OK" | tee -a "$STATUS"
else
  echo "V4_6_DCFSB_ADAPTER3_INTERNAL256_AUDIT_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
