...
SCRIPT=$EVID/run_v4_6_dcfsb_adapter5_lr5e5_notest.sh
...
SESSION=v46_dcfsb_train_adapter5_lr5e5
MODEL_NAME=ConvIR-Haze4K-v4A6-DCFSB-Bottleneck-adapter5-lr5e5-notest-seed3407-20260708
...
if [[ "${1:-}" == "--inside" ]]; then
  ...
  mkdir -p "$EVID"
  {
    echo "train_start v4_6_dcfsb_adapter5_lr5e5_notest $(date --iso-8601=seconds)"
    echo "work=$WORK"
  ...
  rc=$?
  set -e
  echo "train_done rc=$rc v4_6_dcfsb_adapter5_lr5e5_notest $(date --iso-8601=seconds)" | tee -a "$STATUS"
  if [[ "$rc" -eq 0 ]]; then
    echo "V4_6_DCFSB_ADAPTER5_LR5E5_NOTEST_TRAIN_OK" | tee -a "$STATUS"
  else
    echo "V4_6_DCFSB_ADAPTER5_LR5E5_NOTEST_TRAIN_FAILED" | tee -a "$STATUS"
  fi
  exit "$rc"
fi
...
echo "train_launch v4_6_dcfsb_adapter5_lr5e5_notest $(date --iso-8601=seconds) session=$SESSION gpu=$GPU_ID" | tee -a "$STATUS"
tmux new-session -d -s "$SESSION" "CUDA_VISIBLE_DEVICES=$GPU_ID bash '$SCRIPT' --inside"
echo "V4_6_DCFSB_ADAPTER5_LR5E5_NOTEST_LAUNCHED session=$SESSION" | tee -a "$STATUS"
