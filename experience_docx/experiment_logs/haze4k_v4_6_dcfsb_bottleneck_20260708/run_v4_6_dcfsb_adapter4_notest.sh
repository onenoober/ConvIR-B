...
SCRIPT=$EVID/run_v4_6_dcfsb_adapter4_notest.sh
...
SESSION=v46_dcfsb_train_adapter4
MODEL_NAME=ConvIR-Haze4K-v4A6-DCFSB-Bottleneck-adapter4-notest-seed3407-20260708
...
if [[ "${1:-}" == "--inside" ]]; then
  ...
  mkdir -p "$EVID"
  {
    echo "train_start v4_6_dcfsb_adapter4_notest $(date --iso-8601=seconds)"
    echo "work=$WORK"
    ...
    echo "locked_test_policy=no default validation; valid_freq=999 > stop_epoch=4"
  } | tee -a "$STATUS"
  cd "$ITS"
  set +e
  ...
  rc=$?
  set -e
  echo "train_done rc=$rc v4_6_dcfsb_adapter4_notest $(date --iso-8601=seconds)" | tee -a "$STATUS"
  if [[ "$rc" -eq 0 ]]; then
    echo "V4_6_DCFSB_ADAPTER4_NOTEST_TRAIN_OK" | tee -a "$STATUS"
  else
    echo "V4_6_DCFSB_ADAPTER4_NOTEST_TRAIN_FAILED" | tee -a "$STATUS"
  fi
  exit "$rc"
fi
...
echo "train_launch v4_6_dcfsb_adapter4_notest $(date --iso-8601=seconds) session=$SESSION gpu=$GPU_ID" | tee -a "$STATUS"
tmux new-session -d -s "$SESSION" "CUDA_VISIBLE_DEVICES=$GPU_ID bash '$SCRIPT' --inside"
echo "V4_6_DCFSB_ADAPTER4_NOTEST_LAUNCHED session=$SESSION" | tee -a "$STATUS"
