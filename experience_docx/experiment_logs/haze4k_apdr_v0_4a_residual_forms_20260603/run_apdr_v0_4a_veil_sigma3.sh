...
TAG=${TAG:-apdr_v0_4a_veil_sigma3_seed3407}
...
log_status "start_veil_sigma3 root=$ROOT selector=$SELECTOR tag=$TAG"
...
if "$PY" "$ROOT/experience_docx/tools/overfit_haze4k_apdr_v0_4a_lowfield.py" \
  ...
  log_status "gate_pass_veil_sigma3 $TAG"
else
  log_status "gate_fail_veil_sigma3 $TAG"
fi

log_status "complete_veil_sigma3 $TAG"
