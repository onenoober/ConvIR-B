...
FOLD_DIR=$EVID/folds/fold_${FOLD}
...
MODEL_NAME=ConvIR-Haze4K-v48-DCFSB-adapter4-fold${FOLD}-seed3407-20260708
OUT_DIR=$ITS/results/$MODEL_NAME
STATUS=$FOLD_DIR/status.txt
TRAIN_LOG=$FOLD_DIR/train_${MODEL_NAME}.log
mkdir -p "$FOLD_DIR"
{
  echo "train_start v48_fold${FOLD} $(date --iso-8601=seconds)"
...
if [ ! -x "$PY" ]; then echo "V48_FOLD${FOLD}_FAILED python_missing" | tee -a "$STATUS"; exit 2; fi
if [ ! -f "$TRAIN_SPLIT" ]; then echo "V48_FOLD${FOLD}_FAILED split_missing" | tee -a "$STATUS"; exit 2; fi
if [ ! -f "$A0" ]; then echo "V48_FOLD${FOLD}_FAILED a0_missing" | tee -a "$STATUS"; exit 2; fi
if [ -e "$OUT_DIR" ]; then echo "V48_FOLD${FOLD}_FAILED output_exists" | tee -a "$STATUS"; exit 3; fi
cd "$ITS"
set +e
...
if [ "$rc" -eq 0 ]; then
  ...
  echo "train_done rc=0 v48_fold${FOLD} $(date --iso-8601=seconds)" | tee -a "$STATUS"
  echo "V48_FOLD${FOLD}_TRAIN_OK" | tee -a "$STATUS"
else
  echo "state=FAILED_TRAIN" | tee -a "$STATUS"
  echo "train_done rc=$rc v48_fold${FOLD} $(date --iso-8601=seconds)" | tee -a "$STATUS"
  echo "V48_FOLD${FOLD}_TRAIN_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
