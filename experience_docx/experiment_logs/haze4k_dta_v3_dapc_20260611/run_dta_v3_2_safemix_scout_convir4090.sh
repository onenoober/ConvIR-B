...
INIT_CKPT=${INIT_CKPT:-$BASE/repos/ConvIR-B-dta-v3-dapc-finetune-taillite/Dehazing/ITS/results/ConvIR-Haze4K-DTA-v3-DAPC-DepthDirectTail-wg18_base_s008_b14-seed3407-f0-scout5full/Training-Results/Final.pkl}
...
case "$VARIANT" in
  c1_gate)
    TRAIN_SCOPE=dta_safemix_gate_only
    SAFE_PHYS=1.0
    SAFE_LEARNED=0.0
    SAFE_CLIP=0.08
    SAFE_GATE_BIAS=-2.5
...
RUN_ID=v32_safemix_${VARIANT}_seed${SEED}_f${FOLD}_${STAGE}
MODEL_NAME=ConvIR-Haze4K-DTA-v3-2-SafeMix-${VARIANT}-seed${SEED}-f${FOLD}-${STAGE}
TRAIN_LOG=$EVID/dta_v3_2_${RUN_ID}_train.log
mkdir -p "$EVID"
{
  echo "dta_v3_2_safemix_train_start run_id=$RUN_ID variant=$VARIANT stage=$STAGE $(date --iso-8601=seconds)"
  echo "work=$WORK"
  echo "python=$PY"
  echo "data=$DATA"
  echo "a0=$A0"
  ...
  echo "train_scope=$TRAIN_SCOPE safe_phys=$SAFE_PHYS safe_learned=$SAFE_LEARNED safe_clip=$SAFE_CLIP safe_gate_bias=$SAFE_GATE_BIAS"
...
if [[ ! -f "$INIT_CKPT" ]]; then
  echo "DTA_V3_2_SAFEMIX_MISSING_INIT $INIT_CKPT" | tee -a "$STATUS"
  exit 3
fi

cd "$WORK/Dehazing/ITS"
if [[ "${FORCE:-0}" != "1" && -f "results/$MODEL_NAME/Training-Results/Final.pkl" ]]; then
  echo "dta_v3_2_safemix_train_skip_existing model=$MODEL_NAME $(date --iso-8601=seconds)" | tee -a "$STATUS"
else
  set +e
  PYTHONUNBUFFERED=1 "$PY" main.py \
    ...
    --init_model_partial \
    --partial_new_prefixes DTA.trans_uncertainty_head.,DTA.safe_residual_head.,DTA.safe_gate_head. \
  ...
  set -e
  echo "dta_v3_2_safemix_train_done rc=$train_rc run_id=$RUN_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
  if [[ "$train_rc" -ne 0 ]]; then exit "$train_rc"; fi
fi

CANDIDATE=$WORK/Dehazing/ITS/results/$MODEL_NAME/Training-Results/Final.pkl
if [[ ! -f "$CANDIDATE" ]]; then
  echo "DTA_V3_2_SAFEMIX_MISSING_CHECKPOINT $CANDIDATE" | tee -a "$STATUS"
  exit 3
fi

cd "$WORK"
for A_MODE in fallback gt; do
  MANIFEST=$EVID/dta_v3_2_${RUN_ID}_${A_MODE}_matrix_manifest.json
  printf '{"runs":[\n' > "$MANIFEST"
  first=1
  for EVAL_MODE in invert zero shuffle normal; do
    EVAL_RUN=${RUN_ID}_${A_MODE}_${EVAL_MODE}
    COMPARE_DIR=$EVID/dta_v3_2_${EVAL_RUN}_compare
    mkdir -p "$COMPARE_DIR"
    set +e
    PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/eval_haze4k_checkpoint_compare.py \
      ...
      --candidate_name "DTA_v32_${EVAL_RUN}" \
      --dta_depth_cache_dir "$DEPTH" \
      --dta_eval_depth_split train \
      ...
      2>&1 | tee "$EVID/dta_v3_2_${EVAL_RUN}_eval.log"
    eval_rc=${PIPESTATUS[0]}
    set -e
    echo "dta_v3_2_safemix_eval_done rc=$eval_rc run_id=$EVAL_RUN $(date --iso-8601=seconds)" | tee -a "$STATUS"
  ...
  PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/aggregate_haze4k_dta_v3_controls.py \
    ...
    --baseline_label zero \
    --true_label true \
    2>&1 | tee "$EVID/dta_v3_2_${RUN_ID}_${A_MODE}_aggregate.log"
  agg_rc=${PIPESTATUS[0]}
  set -e
  echo "dta_v3_2_safemix_aggregate_done rc=$agg_rc run_id=$RUN_ID airlight=$A_MODE $(date --iso-8601=seconds)" | tee -a "$STATUS"
  if [[ "$agg_rc" -ne 0 ]]; then exit "$agg_rc"; fi
done

set +e
PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/summarize_haze4k_dta_v32_safemix_scouts.py \
  --evidence_dir "$EVID" \
  --output_json "$EVID/dta_v3_2_safemix_scout_summary.json" \
  --output_csv "$EVID/dta_v3_2_safemix_scout_summary.csv" \
  2>&1 | tee "$EVID/dta_v3_2_${RUN_ID}_summary.log"
summary_rc=${PIPESTATUS[0]}
set -e
echo "dta_v3_2_safemix_summary_done rc=$summary_rc run_id=$RUN_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [[ "$summary_rc" -ne 0 ]]; then exit "$summary_rc"; fi

CONTACT_DIR=$EVID/tail_regression_contact_sheet/${RUN_ID}_fallback
mkdir -p "$CONTACT_DIR"
true_csv=$(ls "$EVID"/dta_v3_2_${RUN_ID}_fallback_invert_compare/scout_eval_per_image_*.csv | head -n 1)
set +e
PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/make_haze4k_dta_contact_sheet.py \
  ...
  2>&1 | tee "$EVID/dta_v3_2_${RUN_ID}_contact_sheet.log"
contact_rc=${PIPESTATUS[0]}
set -e
echo "dta_v3_2_safemix_contact_sheet_done rc=$contact_rc run_id=$RUN_ID output=$CONTACT_DIR $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [[ "$contact_rc" -ne 0 ]]; then exit "$contact_rc"; fi

echo "DTA_V3_2_SAFEMIX_SCOUT_OK run_id=$RUN_ID checkpoint=$CANDIDATE" | tee -a "$STATUS"
