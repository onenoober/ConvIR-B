...
ROOT=${ROOT:-/sda/home/wangyuxin/ConvIR-B/repos/ConvIR-B-v3h-operator-site-context-audit}
...
LOG="$EVID/v3h_operator_context_audit.log"

mkdir -p "$EVID"
echo "v3h_context_start $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
export CUDA_VISIBLE_DEVICES="$GPU_ID"
export TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1
cd "$ROOT"
set +e
PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/chd_rm_v3h_operator_context_audit.py \
  ...
  --replay_max_candidates 8 \
  --fractions 0.25 0.5 0.75 \
  > "$LOG" 2>&1
rc=$?
set -e
echo "v3h_context_done rc=$rc $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V3H_OPERATOR_CONTEXT_AUDIT_OK" | tee -a "$STATUS"
else
  echo "V3H_OPERATOR_CONTEXT_AUDIT_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
