...
OUT=$REPO/experience_docx/experiment_logs/haze4k_v5_chd_rm_v2e_d7c_control_recall_audit_20260709/d7c_rp_full
LOG=$OUT/d7c_rp_full.log
STATUS=$OUT/status.txt
mkdir -p "$OUT"
cd "$REPO"
echo "RUNNING_D7C_RP_FULL start $(date --iso-8601=seconds) specs=5 epochs=6" | tee "$STATUS"
set +e
CUDA_VISIBLE_DEVICES=1 "$PY" experience_docx/tools/run_chd_rm_v2e_d7c_rp_micro_variant.py \
  ...
  --rp_specs \
    d7c_rp_lam05_r3_lr2e4:0.5:3:0.0002 \
    d7c_rp_lam10_r3_lr2e4:1.0:3:0.0002 \
    d7c_rp_lam15_r3_lr15e4:1.5:3:0.00015 \
    d7c_rp_lam20_r5_lr15e4:2.0:5:0.00015 \
    d7c_rp_lam30_r5_lr1e4:3.0:5:0.0001 \
  2>&1 | tee "$LOG"
rc=${PIPESTATUS[0]}
set -e
if [ "$rc" -eq 0 ]; then
  echo "RUN_DONE rc=0 $(date --iso-8601=seconds)" | tee -a "$STATUS"
  echo D7C_RP_FULL_OK | tee -a "$STATUS"
else
  echo "RUN_FAILED rc=$rc $(date --iso-8601=seconds)" | tee -a "$STATUS"
  echo D7C_RP_FULL_FAILED | tee -a "$STATUS"
fi
exit "$rc"
