...
mkdir -p "$OUT"
{
  echo "v14b_zero_init_start $(date --iso-8601=seconds)"
  echo "work=$WORK"
  echo "data=$DATA"
  echo "depth=$DEPTH"
  echo "a0=$A0"
  echo "fusion_mode=udp_bi active_adapters=dpfm1 components=all"
} | tee -a "$STATUS"
...
PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/check_haze4k_udp_lite_zero_init_equivalence.py \
  ...
  --tolerance 1e-6 \
  --output_json "$OUT/v14b_zero_init_equivalence.json" \
  > "$OUT/v14b_zero_init_equivalence.log" 2>&1

echo "v14b_zero_init_done rc=$? output=$OUT/v14b_zero_init_equivalence.json $(date --iso-8601=seconds)" | tee -a "$STATUS"
