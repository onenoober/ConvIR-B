...
LOG=$EVID/v2c_need_coverage_calibration_canary.log

mkdir -p "$EVID"
touch "$STATUS"

{
  echo "canary_start chdrm_v2c_need_coverage_calibration $(date --iso-8601=seconds)"
  echo "repo=$REPO"
...
PYTHONUNBUFFERED=1 "$PY" "$REPO/experience_docx/tools/run_chd_rm_v2c_need_coverage_calibration.py" \
  ...
  --output_dir "$EVID/canary" \
  --train_limit 128 \
  --val_limit 64 \
  --metric_sample_size 64 \
  --progress_every 32 \
  --variants d6c_ordinal_quantile d6s_shuffled_quantile \
  --methods identity affine_p10_p90 logit_mean_std quantile_map_1001 \
  2>&1 | tee "$LOG"
rc=${PIPESTATUS[0]}
set -e

echo "canary_done rc=$rc chdrm_v2c_need_coverage_calibration $(date --iso-8601=seconds)" | tee -a "$STATUS"
exit "$rc"
