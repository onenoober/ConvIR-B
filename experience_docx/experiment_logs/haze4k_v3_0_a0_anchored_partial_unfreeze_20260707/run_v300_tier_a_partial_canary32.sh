...
cd "$REPO"
STATUS=$E/status.txt
LOG=$E/runtime_logs/v300_tier_a_partial_canary32.log
trap 'echo "V300_TIER_A_FAILED time=$(date -Iseconds)" >> "$STATUS"' ERR
echo "V300_TIER_A_START time=$(date -Iseconds)" >> "$STATUS"
$PY experience_docx/tools/run_haze4k_v300_a0_partial_unfreeze.py   --mode canary32   --scope tier_a_partial   --repo-root "$REPO"   --data-dir "$DATA"   --checkpoint "$CKPT"   --split-source-summary "$SPLIT"   --evidence-root "$E"   --output-dir "$OUT"   --epochs 10   --batch-size 2   2>&1 | tee "$LOG"
echo "V300_TIER_A_OK time=$(date -Iseconds)" >> "$STATUS"
