...
STATUS=$E/status.txt
LOG=$E/runtime_logs/v300_tier_b_partial_canary32.log
trap 'echo "V300_TIER_B_FAILED time=$(date -Iseconds)" >> "$STATUS"' ERR
cd "$REPO"
echo "V300_TIER_B_START time=$(date -Iseconds)" >> "$STATUS"
$PY experience_docx/tools/run_haze4k_v300_a0_partial_unfreeze.py \
  --mode canary32 \
  --scope tier_b_partial \
  --repo-root "$REPO" \
  --data-dir "$DATA" \
  --checkpoint "$CKPT" \
  ...
  --batch-size 2 \
  2>&1 | tee "$LOG"
echo "V300_TIER_B_OK time=$(date -Iseconds)" >> "$STATUS"
