...
cd "$REPO"
STATUS=$E/status.txt
LOG=$E/runtime_logs/v300_stage0_preflight.log
trap 'echo "V300_STAGE0_FAILED time=$(date -Iseconds)" >> "$STATUS"' ERR
echo "V300_STAGE0_START time=$(date -Iseconds)" > "$STATUS"
echo "branch=$(git branch --show-current)" >> "$STATUS"
echo "commit=$(git rev-parse --short HEAD)" >> "$STATUS"
$PY experience_docx/tools/run_haze4k_v300_a0_partial_unfreeze.py   --mode preflight   --repo-root "$REPO"   --data-dir "$DATA"   --checkpoint "$CKPT"   --split-source-summary "$SPLIT"   --evidence-root "$E"   --output-dir "$OUT"   2>&1 | tee "$LOG"
echo "V300_STAGE0_OK time=$(date -Iseconds)" >> "$STATUS"
