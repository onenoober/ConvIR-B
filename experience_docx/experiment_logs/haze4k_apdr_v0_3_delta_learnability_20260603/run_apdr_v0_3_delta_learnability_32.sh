...
LOG_DIR="$ROOT/experience_docx/experiment_logs/haze4k_apdr_v0_3_delta_learnability_20260603"
STATUS="$LOG_DIR/status.txt"
TAG=${TAG:-apdr_v0_3_delta_learnability_32_seed3407}
...
log_status "start_delta_learnability root=$ROOT selector=$SELECTOR tag=$TAG"

if "$PY" "$ROOT/experience_docx/tools/overfit_haze4k_apdr_delta_learnability.py" \
  --data_dir "$DATA_DIR" \
  ...
  log_status "gate_pass_delta_learnability $TAG"
else
  log_status "gate_fail_delta_learnability $TAG"
fi

log_status "complete_delta_learnability $TAG"
