...
STATUS=$EVID/status_c2c.txt
LOG=$EVID/v20_c2c_mlp_router.log

mkdir -p "$EVID"
echo "===== v20_c2c_mlp_router_start $(date --iso-8601=seconds) =====" >> "$LOG"
{
  echo "v20_c2c_mlp_router_start $(date --iso-8601=seconds)"
...
cd "$REMOTE_ROOT"
set +e
"$PY" experience_docx/tools/audit_haze4k_v20_c2c_mlp_router.py \
...
rc=${PIPESTATUS[0]}
set -e
echo "v20_c2c_mlp_router_done rc=$rc $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V20_C2C_MLP_ROUTER_OK $(date --iso-8601=seconds)" | tee -a "$STATUS"
else
  echo "V20_C2C_MLP_ROUTER_FAILED rc=$rc $(date --iso-8601=seconds)" | tee -a "$STATUS"
fi
exit "$rc"
