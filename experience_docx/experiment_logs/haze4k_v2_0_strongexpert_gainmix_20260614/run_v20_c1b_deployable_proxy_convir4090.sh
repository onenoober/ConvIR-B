...
STATUS=$EVID/status_c1b.txt
LOG=$EVID/v20_c1b_deployable_proxy.log

mkdir -p "$EVID"
{
  echo "v20_c1b_deployable_proxy_start $(date --iso-8601=seconds)"
...
cd "$REMOTE_ROOT"
set +e
"$PY" experience_docx/tools/audit_haze4k_v20_c1b_deployable_proxy.py \
  --fulludp-eval-dir "$FULLUDP_EVAL" \
  --out-dir "$EVID" \
  2>&1 | tee "$LOG"
rc=${PIPESTATUS[0]}
set -e
echo "v20_c1b_deployable_proxy_done rc=$rc $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V20_C1B_DEPLOYABLE_PROXY_OK $(date --iso-8601=seconds)" | tee -a "$STATUS"
else
  echo "V20_C1B_DEPLOYABLE_PROXY_FAILED rc=$rc $(date --iso-8601=seconds)" | tee -a "$STATUS"
fi
exit "$rc"
