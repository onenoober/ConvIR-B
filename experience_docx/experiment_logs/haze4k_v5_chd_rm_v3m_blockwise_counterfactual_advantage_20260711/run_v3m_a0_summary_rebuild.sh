...
LOG="$EVID/v3m_a0_summary_rebuild_${STAMP}.log"
RAW="$EVID/cloud_only_raw_common_action"

echo "v3m_a0_summary_rebuild_start $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
echo "v3m_a0_summary_rebuild_log $LOG" | tee -a "$STATUS"

cd "$REMOTE_ROOT"
if [ "$(git branch --show-current)" != "$BRANCH" ]; then
  echo "V3M_A0_SUMMARY_REBUILD_PREFLIGHT_FAILED wrong_branch=$(git branch --show-current)" | tee -a "$STATUS"
  exit 2
fi
if [ -e "$EVID/v3m_a0_common_action_summary.json" ]; then
  echo "V3M_A0_SUMMARY_REBUILD_PREFLIGHT_FAILED summary_exists" | tee -a "$STATUS"
  exit 2
fi
for f in \
  v3l_a1_oracle_policy_rows_cloud_only.csv \
  v3l_a1_oracle_policy_oof_rows_cloud_only.csv \
  v3l_a1_oracle_policy_summary.csv
do
  test -s "$RAW/$f"
done
if grep -R --quiet 'ORACLE_PIXEL_SCALAR_CONTINUOUS' "$RAW"/*.csv "$RAW"/*.json; then
  echo "V3M_A0_SUMMARY_REBUILD_PREFLIGHT_FAILED pixel_policy_not_renamed" | tee -a "$STATUS"
  exit 2
fi
...
rc=$?
set -e
echo "v3m_a0_summary_rebuild_done rc=$rc $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V3M_A0_SUMMARY_REBUILD_OK" | tee -a "$STATUS"
else
  echo "V3M_A0_SUMMARY_REBUILD_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
