...
case "$MODE" in
  smoke)
    OUT="$EVID/${SMOKE_OUTPUT_DIR:-a3_smoke32_r1}"
    TAG=${SMOKE_TAG:-v3m_a3_smoke32_r1}
    MAX_TRAIN=32
    PROGRESS=8
    LOG="$EVID/v3m_a3_smoke32_${STAMP}.log"
    ;;
  formal)
    OUT="$EVID"
    TAG=v3m_a3
    MAX_TRAIN=1200
    PROGRESS=25
    LOG="$EVID/v3m_a3_formal_${STAMP}.log"
    ;;
  *)
    echo "V3M_A3_INVALID_MODE $MODE" | tee -a "$STATUS"
    exit 2
    ;;
esac

echo "v3m_a3_${MODE}_start $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
echo "v3m_a3_${MODE}_log $LOG gpu=$GPU" | tee -a "$STATUS"
cd "$REMOTE_ROOT"
test "$(git branch --show-current)" = "$BRANCH"
test -s "$V3J_EVID/fresh_route_confirm_split_manifest.json"
test -s "$PARENT_EVID/v3l_a0_canonical_operator_closeout.json"
test -s "$PARENT_EVID/v3l_a0_canonical_operator_artifact_manifest.json"
test -s "$EVID/cloud_only_raw_common_action/v3l_a1_oracle_policy_oof_rows_cloud_only.csv"
test -s "$EVID/v3m_a0_source_manifest.json"
test -s "$EVID/v3m_a2_summary.json"
test -s "$EVID/v3m_a2_source_manifest.json"
test -s "$EVID/v3m_a2_fold_summary.csv"
test -s "$EVID/v3m_a2_calibration_bins.csv"
if [ "$MODE" = smoke ]; then
  test ! -e "$OUT"
else
  SMOKE=${SMOKE:-$EVID/a3_smoke32_r1/v3m_a3_smoke32_r1_summary.json}
  test -s "$SMOKE"
  test ! -e "$EVID/v3m_a3_summary.json"
  "$PY" - "$SMOKE" <<'PY'
import json
import sys
summary = json.load(open(sys.argv[1], encoding="utf-8"))
if summary["decision"] != "V3M_A3_SMOKE_REPLAY_PASS_AUTHORIZE_FORMAL_OOF_ONLY":
    raise SystemExit("formal A3 requires passing smoke replay")
if not all(row["gate_fixed_replay_pass"] for row in summary["operator_summaries"]):
    raise SystemExit("formal A3 requires exact smoke fixed-alpha replay")
PY
fi
nvidia-smi -i "$GPU" --query-gpu=index,memory.free,utilization.gpu --format=csv,noheader

set +e
CUDA_VISIBLE_DEVICES="$GPU" PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/chd_rm_v3m_a3_frozen_policy_replay.py \
  ...
  --reference_oof_rows "$EVID/cloud_only_raw_common_action/v3l_a1_oracle_policy_oof_rows_cloud_only.csv" \
  --v3m_a0_source_manifest "$EVID/v3m_a0_source_manifest.json" \
  --a2_summary "$EVID/v3m_a2_summary.json" \
  --a2_source_manifest "$EVID/v3m_a2_source_manifest.json" \
  --a2_fold_summary "$EVID/v3m_a2_fold_summary.csv" \
  --a2_calibration_bins "$EVID/v3m_a2_calibration_bins.csv" \
  ...
  --expected_a2_summary_sha256 8233b2a8ec2534ac76d32fd46524baa718c0ad4ba76c3fbc74a8dd3276adcc01 \
  --expected_a2_source_manifest_sha256 2716db60d7b12e8c4682935c4cd37d32d28e5cfcedf9bed036231f6f193dc69b \
  --expected_a2_fold_summary_sha256 5ef263c41df5e2024084a75bc50bac4e45125fc797b102493c483408dc0aae1d \
  --expected_a2_calibration_bins_sha256 8ec92048058b754c99f4701c4abdfa7323973597060c0b13cea9820306483d62 \
  ...
  --minimum_mean_lift_ci95_low_db 0.05 --minimum_retention_ci95_low 0.45 --minimum_paired_lift_p10_db -0.02 \
  --bootstrap_draws 4000 --seed 3407 --progress_every "$PROGRESS" \
  > "$LOG" 2>&1
rc=$?
set -e
echo "v3m_a3_${MODE}_done rc=$rc $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V3M_A3_${MODE^^}_OK" | tee -a "$STATUS"
else
  echo "V3M_A3_${MODE^^}_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
