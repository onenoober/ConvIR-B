...
LOG="$EVID/v3m_a0_operator_agreement_rebuild_${STAMP}.log"
RAW="$EVID/cloud_only_raw_common_action"

echo "v3m_a0_operator_agreement_rebuild_start $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
echo "v3m_a0_operator_agreement_rebuild_log $LOG" | tee -a "$STATUS"

cd "$REMOTE_ROOT"
test "$(git branch --show-current)" = "$BRANCH"
test -f "$EVID/v3m_a0_common_action_summary.json"
test -f "$EVID/v3m_a0_operator_agreement.csv"
test ! -e "$EVID/v3m_a0_common_action_summary_pre_operator_agreement_fix.json"
test ! -e "$EVID/v3m_a0_operator_agreement_pre_operator_agreement_fix.csv"
for f in v3l_a1_oracle_policy_rows_cloud_only.csv v3l_a1_oracle_policy_oof_rows_cloud_only.csv v3l_a1_oracle_policy_summary.csv; do
  test -s "$RAW/$f"
done
if grep -R --quiet 'ORACLE_PIXEL_SCALAR_CONTINUOUS' "$RAW"/*.csv "$RAW"/*.json; then
  echo "V3M_A0_OPERATOR_AGREEMENT_REBUILD_PREFLIGHT_FAILED pixel_policy_not_renamed" | tee -a "$STATUS"
  exit 2
fi
...
PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/chd_rm_v3m_a0_common_action_oracle.py \
  ...
  --summarize_existing_raw \
  --rewrite_compact_summary_from_existing_raw \
  > "$LOG" 2>&1
rc=$?
set -e
echo "v3m_a0_operator_agreement_rebuild_done rc=$rc $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V3M_A0_OPERATOR_AGREEMENT_REBUILD_OK" | tee -a "$STATUS"
else
  echo "V3M_A0_OPERATOR_AGREEMENT_REBUILD_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
