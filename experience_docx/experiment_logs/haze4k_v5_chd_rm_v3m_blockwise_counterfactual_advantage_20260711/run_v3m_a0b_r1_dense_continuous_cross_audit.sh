...
LOG="$EVID/v3m_a0b_r1_dense_continuous_cross_audit_${STAMP}.log"

echo "v3m_a0b_r1_cross_audit_start $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
echo "v3m_a0b_r1_cross_audit_log $LOG" | tee -a "$STATUS"
...
for f in v3m_a0b_r1_quantization_gap_summary.csv v3m_a0b_r1_cross_audit.json v3m_a0b_r1_source_manifest.json; do
  test ! -e "$EVID/$f"
done
...
PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/chd_rm_v3m_a0b_dense_continuous_cross_audit.py \
  ...
  --grid_monotonic_tolerance_db 1e-6 \
  --bootstrap_draws 4000 \
  --seed 3407 \
  > "$LOG" 2>&1
rc=$?
set -e
echo "v3m_a0b_r1_cross_audit_done rc=$rc $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V3M_A0B_R1_DENSE_CONTINUOUS_CROSS_AUDIT_OK" | tee -a "$STATUS"
else
  echo "V3M_A0B_R1_DENSE_CONTINUOUS_CROSS_AUDIT_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
