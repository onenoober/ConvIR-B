...
case "$MODE" in
  ...
  formal)
    ...
    MAX_TRAIN=1200
    PROGRESS=25
    LOG="$EVID/v3m_a1_formal_${STAMP}.log"
    ;;
  *)
    echo "V3M_A1_INVALID_MODE $MODE" | tee -a "$STATUS"
    exit 2
    ;;
esac

echo "v3m_a1_${MODE}_start $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
echo "v3m_a1_${MODE}_log $LOG gpu=$GPU" | tee -a "$STATUS"
cd "$REMOTE_ROOT"
...
if summary["decision"] != "V3M_A1_SMOKE_REPLAY_PASS_AUTHORIZE_FORMAL_OOF_ONLY":
    raise SystemExit("formal A1 requires passing smoke replay")
if not all(row["fixed_alpha_replay_pass"] for row in summary["fixed_alpha_replay"]):
    raise SystemExit("formal A1 requires exact smoke replay")
PY
fi
nvidia-smi -i "$GPU" --query-gpu=index,memory.free,utilization.gpu --format=csv,noheader

set +e
CUDA_VISIBLE_DEVICES="$GPU" PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/chd_rm_v3m_a1_local_actuation_audit.py \
  ...
  --min_valid_image_fraction 0.80 --min_auc_ci95_low 0.56 --bootstrap_draws 4000 --seed 3407 --progress_every "$PROGRESS" \
  > "$LOG" 2>&1
rc=$?
set -e
echo "v3m_a1_${MODE}_done rc=$rc $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V3M_A1_${MODE^^}_OK" | tee -a "$STATUS"
else
  echo "V3M_A1_${MODE^^}_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
