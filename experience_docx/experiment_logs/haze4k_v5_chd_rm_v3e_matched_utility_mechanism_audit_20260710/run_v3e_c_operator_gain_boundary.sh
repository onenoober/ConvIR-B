...
LOG="$EVID/v3e_c_operator_gain_boundary.log"

mkdir -p "$EVID"
echo "v3e_c_start $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
export CUDA_VISIBLE_DEVICES="$GPU_ID"
export TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1
cd "$ROOT"
set +e
PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/chd_rm_v3e_mechanism_audit.py gain \
...
rc=$?
set -e
echo "v3e_c_done rc=$rc $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V3E_C_OPERATOR_GAIN_BOUNDARY_OK" | tee -a "$STATUS"
else
  echo "V3E_C_OPERATOR_GAIN_BOUNDARY_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
