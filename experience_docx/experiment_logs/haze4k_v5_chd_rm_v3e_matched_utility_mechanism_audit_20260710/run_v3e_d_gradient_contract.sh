...
LOG="$EVID/v3e_d_gradient_contract.log"

mkdir -p "$EVID"
echo "v3e_d_start $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
export CUDA_VISIBLE_DEVICES="$GPU_ID"
export TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1
cd "$ROOT"
set +e
PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/chd_rm_v3e_mechanism_audit.py grad \
  --a0_checkpoint "$BASE/checkpoints/official/Haze4K/haze4k-base.pkl" \
  --d7c_checkpoint "$OLD/Dehazing/ITS/results/ConvIR-Haze4K-v3d-rarm-fam2-adapteronly-e5frome1-seed3407-20260710/Training-Results/Final.pkl" \
  --control_checkpoint "$OLD/Dehazing/ITS/results/ConvIR-Haze4K-v3d-fam2modres-control-e5frome1-seed3407-20260710/Training-Results/Final.pkl" \
  --d7c_resume_checkpoint "$OLD/Dehazing/ITS/results/ConvIR-Haze4K-v3d-rarm-fam2-adapteronly-e1-seed3407-20260710/Training-Results/model.pkl" \
  --control_resume_checkpoint "$OLD/Dehazing/ITS/results/ConvIR-Haze4K-v3d-fam2modres-control-e1-seed3407-20260710/Training-Results/model.pkl" \
...
rc=$?
set -e
echo "v3e_d_done rc=$rc $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V3E_D_GRADIENT_CONTRACT_OK" | tee -a "$STATUS"
else
  echo "V3E_D_GRADIENT_CONTRACT_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
