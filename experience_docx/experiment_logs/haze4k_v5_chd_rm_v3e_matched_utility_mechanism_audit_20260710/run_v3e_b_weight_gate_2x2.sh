...
LOG="$EVID/v3e_b_weight_gate_2x2.log"

mkdir -p "$EVID"
echo "v3e_b_start $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
export CUDA_VISIBLE_DEVICES="$GPU_ID"
export TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1
cd "$ROOT"
set +e
PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/chd_rm_v3e_mechanism_audit.py replay \
...
rc=$?
set -e
echo "v3e_b_done rc=$rc $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V3E_B_WEIGHT_GATE_2X2_OK" | tee -a "$STATUS"
else
  echo "V3E_B_WEIGHT_GATE_2X2_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
