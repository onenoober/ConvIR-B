...
LOG="$EVID/v3i_c_counterfactual_response_probe.log"

mkdir -p "$EVID"
echo "v3i_c_start $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
export CUDA_VISIBLE_DEVICES="$GPU_ID"
export TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1
cd "$REMOTE_ROOT"
set +e
PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/chd_rm_v3i_c_counterfactual_response_probe.py \
...
rc=$?
set -e
echo "v3i_c_done rc=$rc $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V3I_C_COUNTERFACTUAL_RESPONSE_PROBE_OK" | tee -a "$STATUS"
else
  echo "V3I_C_COUNTERFACTUAL_RESPONSE_PROBE_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
