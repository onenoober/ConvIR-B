...
FOLD=${FOLD:-0}
SPLIT_JSON=${SPLIT_JSON:-$EVID/dta_v3_6_haze4k_oof_splits_seed3407.json}
...
mkdir -p "$EVID"
{
  ...
  echo "locked_test_policy_note=blocked by default; user relaxed one-shot override pending after fixed selector seal"
...
for p in "$PY" "$DATA" "$A0" "$DEPTH"; do
  if [[ ! -e "$p" ]]; then
    echo "DTA_V3_6_HRCS_CANDIDATE_MISSING_PATH $p" | tee -a "$STATUS"
    exit 3
  fi
done
...
if [[ ! -f "$CANDIDATE" ]]; then
  echo "DTA_V3_6_HRCS_CANDIDATE_MISSING_CHECKPOINT $CANDIDATE" | tee -a "$STATUS"
  exit 3
fi
...
if [[ "$RUN_TEST" == "1" ]]; then
  echo "DTA_V3_6_LOCKED_TEST_BLOCKED_BY_DEFAULT" | tee -a "$STATUS"
  exit 5
fi

echo "DTA_V3_6_HRCS_CANDIDATE_OK run_id=$RUN_ID checkpoint=$CANDIDATE" | tee -a "$STATUS"
