...
while IFS=$'\t' read -r seed fold mode job_gpu; do
  ...
  echo "multiseed_job_start run_id=$run_id gpu=$GPU \2026-06-11T13:08:26+08:00" | tee -a "$STATUS"
  set +e
  ...
  rc=$?
  set -e
  echo "multiseed_job_done rc=$rc run_id=$run_id gpu=$GPU \2026-06-11T13:08:26+08:00" | tee -a "$STATUS"
  if [[ "$rc" -ne 0 ]]; then
    echo "DTA_V2_MULTI_SEED_JOB_FAILED run_id=$run_id gpu=$GPU" | tee -a "$STATUS"
    exit "$rc"
  fi
done < "$JOB_FILE"
echo "DTA_V2_MULTI_SEED_WORKER_OK gpu=$GPU \2026-06-11T13:08:26+08:00" | tee -a "$STATUS"
