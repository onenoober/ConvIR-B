...
LOG=$EVID/v4_a1_sdfm_preflight.log
JSON_OUT=$EVID/v4_a1_sdfm_preflight.json
export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-2}
export TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1
mkdir -p "$EVID"
{
  echo "preflight_start v4_a1_sdfm $(date --iso-8601=seconds)"
  echo "work=$WORK"
  echo "data=$DATA"
  echo "a0=$A0"
  echo "python=$PY"
...
WORK = Path('/sda/home/wangyuxin/ConvIR-B/repos/ConvIR-B-haze4k-v4-1-sdfm-only')
...
JSON_OUT = WORK / 'experience_docx/experiment_logs/haze4k_v4_1_sdfm_only_20260707/v4_a1_sdfm_preflight.json'
...
route = build_sfad_sdfm_net('base', 'Haze4K', 'original')
...
result['sdfm_stats'] = route.collect_modulation_stats(input_img)

missing_ok = all(k.startswith('SFAD_') for k in partial['missing_new_modules'])
field_not_collapsed = all(
    stats.get('R_std', 0.0) > 1e-5
    and stats.get('R_lt_005', 1.0) < 0.95
    and stats.get('R_gt_095', 1.0) < 0.95
    for stats in result['sdfm_stats'].values()
)
...
result['pass'] = bool(
    ...
    and result['adapter_scope']['trainable_prefixes'] == ['SFAD_SDFM1', 'SFAD_SDFM2']
...
rc=$?
set -e
echo "preflight_done rc=$rc v4_a1_sdfm $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [[ "$rc" -eq 0 ]]; then
  echo "V4_A1_SDFM_PREFLIGHT_OK" | tee -a "$STATUS"
else
  echo "V4_A1_SDFM_PREFLIGHT_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
