...
task_existing_aggregate_done() {
  local variant=$1
  local fold=$2
  local seed=$3
  local run_id=v35_fdf_rcs_${variant}_seed${seed}_f${fold}_${STAGE}
  if [[ -n "$DTA_V37_RUN_BATCH_TAG" ]]; then run_id=${run_id}_${DTA_V37_RUN_BATCH_TAG}; fi
  [[ -s "$EVID/train_eval_depth_matrix_${run_id}_fallback_train.json" && -s "$EVID/r0_vs_rdepth_attribution_${run_id}_fallback_train.csv" ]]
}

task_idx=0
while [[ "$task_idx" -lt "${#TASKS[@]}" ]]; do
  IFS=',' read -r peek_variant peek_fold peek_seed <<< "${TASKS[$task_idx]}"
  if task_existing_aggregate_done "$peek_variant" "$peek_fold" "$peek_seed"; then
    peek_run_id=v35_fdf_rcs_${peek_variant}_seed${peek_seed}_f${peek_fold}_${STAGE}
    if [[ -n "$DTA_V37_RUN_BATCH_TAG" ]]; then peek_run_id=${peek_run_id}_${DTA_V37_RUN_BATCH_TAG}; fi
    echo "dta_v3_7_tau_queue_skip_existing_aggregate run_id=$peek_run_id $(date --iso-8601=seconds)" | tee -a "$STATUS"
    task_idx=$((task_idx + 1))
    continue
  fi
