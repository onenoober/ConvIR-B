...
LOCK_DIR=$EVID/dta_v3_7_tau_locks
mkdir -p "$LOCK_DIR"
LOCK_FILE=$LOCK_DIR/${RUN_ID}.lock
exec 9>"$LOCK_FILE"
echo "dta_v3_7_tau_lock_wait run_id=$RUN_ID lock=$LOCK_FILE $(date --iso-8601=seconds)" | tee -a "$STATUS"
flock 9
echo "dta_v3_7_tau_lock_acquired run_id=$RUN_ID lock=$LOCK_FILE $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [[ -s "$EVID/train_eval_depth_matrix_${RUN_ID}_fallback_train.json" && -s "$EVID/r0_vs_rdepth_attribution_${RUN_ID}_fallback_train.csv" ]]; then
  echo "dta_v3_7_tau_candidate_skip_existing_aggregate run_id=$RUN_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
  exit 0
fi
