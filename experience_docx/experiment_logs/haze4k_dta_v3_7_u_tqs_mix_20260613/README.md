...
This compact GitHub sync includes only the D8 status file and exact final
JSON/CSV closeout artifacts. Large raw output-difference feature tables, raw
action tables, checkpoints, candidate outputs, rendered images, launch logs, and
compare directories remain cloud-only by default.

Cloud-generated exact final artifacts included in this compact sync:
...
This compact GitHub sync includes the D9 status file, locked summary JSON,
aggregate CSV, per-outer CSV, and the locked fixed-policy evaluator script under
`experience_docx/tools/`. Raw selected-action CSVs, launch logs, tmux output,
and compact group-level logs/summaries remain cloud-only runtime artifacts by
default.
