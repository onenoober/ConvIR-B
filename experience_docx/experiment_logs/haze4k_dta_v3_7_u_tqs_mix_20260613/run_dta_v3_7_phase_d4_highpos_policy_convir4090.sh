...
OUTPUT_PREFIX=${OUTPUT_PREFIX:-v37_d4_highpos}
INCLUDE_RUN_SUBSTRING=${INCLUDE_RUN_SUBSTRING:-quick5full}

mkdir -p "$EVID"
{
  echo "dta_v3_7_phase_d4_highpos_policy_start $(date --iso-8601=seconds)"
  echo "state=RUNNING_AUDIT_D4_HIGH_POSITIVE_POLICY"
  echo "work=$WORK"
  echo "python=$PY"
...
for p in "$WORK" "$PY" "$SINGLE_ACTIONS" "$FEATURE_TABLE"; do
  if [[ ! -e "$p" ]]; then
    echo "DTA_V3_7_PHASE_D4_HIGHPOS_MISSING_PATH $p" | tee -a "$STATUS"
    exit 3
  fi
done
...
set +e
PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/train_haze4k_dta_v37_d3_high_positive_policy.py \
  --single_actions_csv "$SINGLE_ACTIONS" \
  --feature_action_table_csv "$FEATURE_TABLE" \
  --output_dir "$EVID" \
  --output_prefix "$OUTPUT_PREFIX" \
  --include_run_substring "$INCLUDE_RUN_SUBSTRING" \
  2>&1 | tee "$EVID/v37_phase_d4_highpos_policy.log"
rc=${PIPESTATUS[0]}
set -e
echo "dta_v3_7_phase_d4_highpos_policy_done rc=$rc $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [[ "$rc" -ne 0 ]]; then
  echo "DTA_V3_7_PHASE_D4_HIGHPOS_POLICY_FAILED $(date --iso-8601=seconds)" | tee -a "$STATUS"
  exit "$rc"
fi

echo "DTA_V3_7_PHASE_D4_HIGHPOS_POLICY_OK $(date --iso-8601=seconds)" | tee -a "$STATUS"
