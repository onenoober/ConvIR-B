...
LOG=$EVID/v21_c9_shifted_strong.log
mkdir -p "$EVID"
echo "===== v21_c9_shifted_start $(date --iso-8601=seconds) =====" >> "$LOG"
{
  echo "v21_c9_shifted_start $(date --iso-8601=seconds)"
...
cd "$REMOTE_ROOT"
set +e
"$PY" experience_docx/tools/audit_haze4k_v21_c9_shifted_strong_validation.py \
...
rc=${PIPESTATUS[0]}
set -e
echo "v21_c9_shifted_done rc=$rc $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V21_C9_SHIFTED_OK $(date --iso-8601=seconds)" | tee -a "$STATUS"
else
  echo "V21_C9_SHIFTED_FAILED rc=$rc $(date --iso-8601=seconds)" | tee -a "$STATUS"
fi
exit "$rc"
