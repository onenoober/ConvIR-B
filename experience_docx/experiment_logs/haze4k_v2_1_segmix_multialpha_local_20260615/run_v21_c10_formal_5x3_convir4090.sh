...
STATUS=$EVID/status_c10.txt
LOG=$EVID/v21_c10_formal_5x3.log
mkdir -p "$EVID"
echo "===== v21_c10_formal_start $(date --iso-8601=seconds) =====" >> "$LOG"
{
 echo "v21_c10_formal_start $(date --iso-8601=seconds)"
...
cd "$REMOTE_ROOT"
set +e
"$PY" experience_docx/tools/audit_haze4k_v21_c10_formal_5x3.py \
...
rc=${PIPESTATUS[0]}
set -e
echo "v21_c10_formal_done rc=$rc $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then echo "V21_C10_FORMAL_OK $(date --iso-8601=seconds)" | tee -a "$STATUS"; else echo "V21_C10_FORMAL_FAILED rc=$rc $(date --iso-8601=seconds)" | tee -a "$STATUS"; fi
exit "$rc"
