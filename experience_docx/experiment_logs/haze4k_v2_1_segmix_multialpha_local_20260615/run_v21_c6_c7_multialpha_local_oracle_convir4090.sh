...
STATUS=$EVID/status_c6_c7.txt
LOG=$EVID/v21_c6_c7_multialpha_local_oracle.log

mkdir -p "$EVID"
echo "===== v21_c6_c7_start $(date --iso-8601=seconds) =====" >> "$LOG"
{
  echo "v21_c6_c7_start $(date --iso-8601=seconds)"
...
cd "$REMOTE_ROOT"
set +e
"$PY" experience_docx/tools/audit_haze4k_v21_c6_c7_multialpha_local_oracle.py \
...
rc=${PIPESTATUS[0]}
set -e
echo "v21_c6_c7_done rc=$rc $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V21_C6_C7_OK $(date --iso-8601=seconds)" | tee -a "$STATUS"
else
  echo "V21_C6_C7_FAILED rc=$rc $(date --iso-8601=seconds)" | tee -a "$STATUS"
fi
exit "$rc"
