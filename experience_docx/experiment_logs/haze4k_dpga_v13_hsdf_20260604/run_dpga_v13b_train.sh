...
{
  echo "v13b_train_start $MODEL_NAME $(date --iso-8601=seconds)"
  echo "work=$WORK"
  echo "data=$DATA"
  echo "depth=$DEPTH"
  echo "a0=$A0"
  echo "split_json=$SPLIT_JSON"
  echo "active_adapters=shallow,bottleneck scale=0.25 bottleneck_scale_multiplier=2.0 hard_gate=bottleneck hard_gate_lambda=0.01"
  echo "mask=hard_selective hard_sample_lambda=0.20 hard_region_lambda=0.35 hard_region_error_threshold=0.010"
} | tee -a "$STATUS"
...
PYTHONUNBUFFERED=1 "$PY" main.py \
  ...
  --dpga_hard_gate_lambda 0.01 \
  --dpga_hard_gate_hard_target 1.0 \
  --dpga_hard_gate_medium_target 0.35 \
  --dpga_hard_gate_easy_target 0.0 \
...
echo "v13b_train_done rc=$? $MODEL_NAME $(date --iso-8601=seconds)" | tee -a "$STATUS"
