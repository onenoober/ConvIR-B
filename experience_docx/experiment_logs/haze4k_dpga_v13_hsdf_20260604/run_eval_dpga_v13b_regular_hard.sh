...
OUT=$LOG_DIR/v13b_eval_regular_hard
...
for ckpt in Best Final; do
  for split in val_regular val_hard; do
    PYTHONUNBUFFERED=1 "$PY" ../../experience_docx/tools/eval_haze4k_checkpoint_compare.py \
      ...
      --candidate_arch dpga \
      --candidate_mode "$ckpt" \
      --candidate_name "v13b_${ckpt,,}_${split}" \
      --candidate_dpga_active_adapters shallow,bottleneck \
      --candidate_dpga_scale_multiplier 0.25 \
      --candidate_dpga_bottleneck_scale_multiplier 2.0 \
      --candidate_dpga_hard_gate_mode bottleneck \
      --dpga_depth_cache_dir "$DEPTH" \
      --split_json "$SPLIT_JSON" \
      --split_name "$split" \
      --output_dir "$OUT" \
      --tag "v13b_${ckpt,,}_${split}_vs_a0" \
      > "$OUT/eval_v13b_${ckpt,,}_${split}.log" 2>&1
  done
done

"$PY" ../../experience_docx/tools/gate_haze4k_dpga_v13_regular_hard.py \
  --best_regular_compare_json "$OUT/scout_eval_compare_v13b_best_val_regular_vs_a0.json" \
  --best_hard_compare_json "$OUT/scout_eval_compare_v13b_best_val_hard_vs_a0.json" \
  --final_regular_compare_json "$OUT/scout_eval_compare_v13b_final_val_regular_vs_a0.json" \
  --final_hard_compare_json "$OUT/scout_eval_compare_v13b_final_val_hard_vs_a0.json" \
  --output "$OUT/dpga_v13b_gate_eval_regular_and_hard.json" \
  --stage "DPGA-v1.3B hard-gated bottleneck regular+hard gate" \
  --regular_mean_min 0.040 \
  --hard_bottom25_min 0.050 \
  --worst_budget 12 \
  --positive_ratio_min 0.62 \
  --strong_ratio_max 0.16 \
  > "$OUT/gate_v13b_regular_hard.log" 2>&1 || true

echo "v13b_eval_regular_hard_done output=$OUT $(date --iso-8601=seconds)" | tee -a "$STATUS"
