...
OUT=$LOG_DIR/runtime_diagnostics_val_regula
...
PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/audit_haze4k_dpga_runtime_variants.py \
  ...
  --output_dir "$OUT" \
  --run_module_ablation \
  > "$LOG_DIR/dpga_v13_runtime_ablation_val_regular.log" 2>&1

cp "$OUT/dpga_module_ablation_best_final.csv" "$OUT/dpga_v13_runtime_ablation_on_val_inner.csv"
echo "v13_runtime_ablation_done output=$OUT $(date --iso-8601=seconds)" | tee -a "$STATUS"
