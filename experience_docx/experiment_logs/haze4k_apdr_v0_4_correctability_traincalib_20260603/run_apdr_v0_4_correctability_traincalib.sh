...
LOG_DIR="$ROOT/experience_docx/experiment_logs/haze4k_apdr_v0_4_correctability_traincalib_20260603"
STATUS="$LOG_DIR/status.txt"
TAG=${TAG:-apdr_v0_4_correctability_traincalib_seed3407}
...
log_status "start_correctability_traincalib root=$ROOT selector=$SELECTOR tag=$TAG"
...
if "$PY" "$ROOT/experience_docx/tools/audit_haze4k_apdr_v0_4_correctability_traincalib.py" \
  ...
  log_status "gate_pass_correctability_traincalib $TAG"
else
  log_status "gate_fail_correctability_traincalib $TAG"
fi

log_status "complete_correctability_traincalib $TAG"
