...
LOG="$EVID/v3j_a_bounded_action_audit.log"

mkdir -p "$EVID"
echo "v3j_a_start $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
export CUDA_VISIBLE_DEVICES="$GPU_ID"
export TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1
cd "$REMOTE_ROOT"
set +e
PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/chd_rm_v3j_a_bounded_action_audit.py \
  ...
  --output_dir "$EVID" \
  --max_bound_calib_samples 600 \
  --max_samples 600 \
  --progress_every 25 \
  > "$LOG" 2>&1
rc=$?
set -e
echo "v3j_a_done rc=$rc $ROUTE_ID $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V3J_A_BOUNDED_ACTION_AUDIT_OK" | tee -a "$STATUS"
else
  echo "V3J_A_BOUNDED_ACTION_AUDIT_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
