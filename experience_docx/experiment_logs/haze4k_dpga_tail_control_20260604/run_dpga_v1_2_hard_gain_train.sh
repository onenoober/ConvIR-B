...
if not decision.get("launch_allowed"):
    print("echo 'v1.2 decision does not allow launch' >&2")
    print("exit 3")
    raise SystemExit(0)
...
{
  echo "v1_2_train_start $MODEL_NAME $(date --iso-8601=seconds)"
  echo "work=$WORK"
  echo "data=$DATA"
  echo "depth=$DEPTH"
  echo "a0=$A0"
...
PYTHONUNBUFFERED=1 "$PY" main.py \
  ...
  > "$TRAIN_LOG" 2>&1
rc=$?
echo "v1_2_train_done $MODEL_NAME rc=$rc $(date --iso-8601=seconds)" | tee -a "$STATUS"
exit "$rc"
