...
DECISION_JSON=$LOG_DIR/v1_2_decision/dpga_v1_2_training_decision.json
OUT_DIR=$LOG_DIR/v1_2_val_inner_eval
LOG=$LOG_DIR/eval_dpga_v1_2_val_inner.log
...
if [[ ! -f "$BEST" || ! -f "$FINAL" ]]; then
  echo "missing v1.2 checkpoints: best=$BEST final=$FINAL" >&2
  exit 3
fi
...
{
  echo "eval_dpga_v1_2_val_inner_start $(date --iso-8601=seconds)"
  ...
  "$PY" "$WORK/experience_docx/tools/eval_haze4k_checkpoint_compare.py" \
    ...
    --candidate_arch dpga \
    --candidate_mode original \
    --candidate_name dpga_v1_2_final \
    --dpga_depth_cache_dir "$DEPTH" \
    --dpga_eval_depth_split train \
    --split_json "$SPLIT_JSON" \
    --split_name val_inner \
    ...
    --candidate_dpga_adapter_residual_scale "$DPGA_ADAPTER_RESIDUAL_SCALE" \
    --output_dir "$OUT_DIR" \
    --tag v1_2_val_inner_final_vs_a0
  if "$PY" "$WORK/experience_docx/tools/gate_haze4k_dpga_v1_1_val.py" \
    --best_compare_json "$OUT_DIR/scout_eval_compare_v1_2_val_inner_best_vs_a0.json" \
    --final_compare_json "$OUT_DIR/scout_eval_compare_v1_2_val_inner_final_vs_a0.json" \
    --output "$OUT_DIR/gate_dpga_v1_2_val_inner.json" \
    --stage "DPGA-v1.2 val_inner scout"; then
    echo "gate_dpga_v1_2_val_inner_pass"
  else
    echo "gate_dpga_v1_2_val_inner_fail"
  fi
  echo "eval_dpga_v1_2_val_inner_done $(date --iso-8601=seconds)"
} 2>&1 | tee "$LOG"

{
  echo "v1_2_val_inner_eval_dir=$OUT_DIR"
  echo "v1_2_val_inner_gate=$OUT_DIR/gate_dpga_v1_2_val_inner.json"
} | tee -a "$STATUS"
