...
MODEL_NAME=ConvIR-Haze4K-v1.4A-UDP-Lite-DPFM123-adapter-only-seed3407-20260604
...
{
  echo "v14a_train_start $MODEL_NAME $(date --iso-8601=seconds)"
  echo "work=$WORK"
  echo "data=$DATA"
  echo "depth=$DEPTH"
  echo "a0=$A0"
  echo "split_json=$SPLIT_JSON"
  echo "fusion_mode=udp_lite active_adapters=dpfm components=all scale=1.0"
} | tee -a "$STATUS"
...
echo "v14a_train_done rc=$? $MODEL_NAME $(date --iso-8601=seconds)" | tee -a "$STATUS"
