...
mkdir -p "$EVID"
echo "v234_p0b_start $(date --iso-8601=seconds)" | tee -a "$STATUS"
cd "$WORK"
set +e
PYTHONUNBUFFERED=1 "$PY" experience_docx/tools/run_haze4k_v234_nopost_projection_audit.py \
  ...
  --sample_count 32 \
  --crop_size 256 \
  --balanced_min_table_gain 0.50 \
  > "$EVID/v234_p0b_balanced_canary.log" 2>&1
rc=$?
set -e
echo "v234_p0b_done rc=$rc $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [[ "$rc" -eq 0 ]]; then
  echo "V234_P0B_BALANCED_CANARY_OK" | tee -a "$STATUS"
else
  echo "V234_P0B_BALANCED_CANARY_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
