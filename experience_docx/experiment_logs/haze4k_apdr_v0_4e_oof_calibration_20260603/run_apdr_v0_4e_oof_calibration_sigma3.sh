...
TAG=${TAG:-apdr_v0_4e_oof_calibration_sigma3_seed3407}
...
log_status "start_v0_4e_oof_calibration root=$ROOT tag=$TAG folds=$FOLD_COUNT low_size=$LOW_SIZE k_values=$K_VALUES scales=$SCALES"

if "$PY" "$ROOT/experience_docx/tools/audit_haze4k_apdr_v0_4e_oof_calibration.py" \
  --data_dir "$DATA_DIR" \
  ...
  > "$LOG_DIR/v04e_oof_calibration_${TAG}.log" 2>&1; then
  log_status "complete_v0_4e_oof_calibration $TAG"
else
  log_status "error_v0_4e_oof_calibration $TAG"
  exit 1
fi
