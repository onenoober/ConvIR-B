...
CAND=$ITS/results/ConvIR-Haze4K-v4A2-GST-adapter-notest-seed3407-20260707/Training-Results/Final.pkl
STATUS=$EVID/status.txt
LOG=$EVID/audit_v4_a2_train128_final_vs_a0.log
SUMMARY=$EVID/a2_train128_compare_final_vs_a0.json
PER_IMAGE=$EVID/a2_train128_per_image_final_vs_a0.csv
MODULE_STATS=$EVID/a2_train128_module_stats_final.jsonl
export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-2}
export TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1
mkdir -p "$EVID"
{
  echo "audit_start v4_a2_train128_final_vs_a0 $(date --iso-8601=seconds)"
  echo "data=$DATA"
  echo "a0=$A0"
  echo "candidate=$CAND"
  echo "python=$PY"
...
from models.SFADGSTConvIR import build_sfad_gst_net
...
CAND = Path('/sda/home/wangyuxin/ConvIR-B/repos/ConvIR-B-haze4k-v4-2-gst-only/Dehazing/ITS/results/ConvIR-Haze4K-v4A2-GST-adapter-notest-seed3407-20260707/Training-Results/Final.pkl')
EVID = Path('/sda/home/wangyuxin/ConvIR-B/repos/ConvIR-B-haze4k-v4-2-gst-only/experience_docx/experiment_logs/haze4k_v4_2_gst_only_20260707')
SUMMARY = EVID / 'a2_train128_compare_final_vs_a0.json'
PER_IMAGE = EVID / 'a2_train128_per_image_final_vs_a0.csv'
MODULE_STATS = EVID / 'a2_train128_module_stats_final.jsonl'
...
route = build_sfad_gst_net('base', 'Haze4K', 'original').to(device).eval()
...
rc=$?
set -e
echo "audit_done rc=$rc v4_a2_train128_final_vs_a0 $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [[ "$rc" -eq 0 ]]; then
  echo "V4_A2_TRAIN128_AUDIT_OK" | tee -a "$STATUS"
else
  echo "V4_A2_TRAIN128_AUDIT_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
