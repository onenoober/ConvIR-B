...
OUT=$ROOT/experience_docx/experiment_logs/haze4k_rootcause_preexp_20260604/prior_predictability_full
...
{
  echo "prior_predictability_full_start $(date -Iseconds)"
  echo "root $ROOT"
  echo "out $OUT"
  echo "data $DATA"
  echo "depth $DEPTH"
} | tee "$OUT/status.txt"
/root/miniconda3/envs/convir-cu128/bin/python "$ROOT/experience_docx/tools/audit_haze4k_apdr_v0_4f_prior_predictability.py" \
  --data_dir "$DATA" \
  ...
  --output_dir "$OUT" \
  --tag apdr_v0_4f_prior_predictability_full_sigma3_seed3407 \
  --basis_num_images 0 \
  --eval_count 3000 \
  --seed 3407 \
  --num_workers 0 \
  ...
  --hook_paths Encoder.0,Encoder.1,Encoder.2,Decoder.0,Decoder.1,Decoder.2,Convs.0,Convs.1,APDR_1.context \
  --depth_split train \
  --progress_freq 100 \
  > "$OUT/prior_predictability_full.log" 2>&1
rc=$?
echo "prior_predictability_full_done rc=$rc $(date -Iseconds)" | tee -a "$OUT/status.txt"
exit $rc
