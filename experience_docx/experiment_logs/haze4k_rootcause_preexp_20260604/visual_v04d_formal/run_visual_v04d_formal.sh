...
OUT=$ROOT/experience_docx/experiment_logs/haze4k_rootcause_preexp_20260604/visual_v04d_formal
...
{
  echo "visual_v04d_formal_start $(date -Iseconds)"
  echo "root $ROOT"
  echo "out $OUT"
} | tee "$OUT/status.txt"
/root/miniconda3/envs/convir-cu128/bin/python "$ROOT/experience_docx/tools/audit_haze4k_apdr_v0_4_visual_regression_grids.py" \
  ...
  --output_dir "$OUT" \
  --tag visual_v04d_formal_sigma3_seed3407 \
  --basis_num_images 0 \
  --train_count 128 \
  --eval_count 256 \
  --seed 3407 \
  ...
  --max_per_group 12 \
  --progress_freq 64 \
  > "$OUT/visual_v04d_formal.log" 2>&1
rc=$?
echo "visual_v04d_formal_done rc=$rc $(date -Iseconds)" | tee -a "$OUT/status.txt"
exit $rc
