...
LOG="$EVID/v231_p2a_diagnostics.log"

mkdir -p "$EVID"
{
  echo "v231_p2a_start $(date --iso-8601=seconds)"
...
set +e
"$PY" experience_docx/tools/nopost_v231_action_value_identifiability_audit.py \
  --data-root "$DATA" \
...
if [ -f "$EVID/v231_p2a_closeout.json" ]; then
  decision=$("$PY" -c 'import json,sys; print(json.load(open(sys.argv[1])).get("decision","UNKNOWN"))' "$EVID/v231_p2a_closeout.json")
fi
echo "v231_p2a_done rc=$rc decision=$decision $(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -eq 0 ]; then
  echo "V231_P2A_COMMAND_OK" | tee -a "$STATUS"
else
  echo "V231_P2A_COMMAND_FAILED" | tee -a "$STATUS"
fi
exit "$rc"
