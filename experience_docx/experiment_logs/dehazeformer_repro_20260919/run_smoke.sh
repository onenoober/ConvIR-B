...
EVID="$ROOT/experience_docx/experiment_logs/dehazeformer_repro_20260919"
PY="$ROOT/envs/convir-cu121/bin/python"
SOTS="$ROOT/datasets/RESIDE/official/SOTS/indoor"
FIXTURE="$ROOT/datasets/RESIDE/derived/dehazeformer_smoke_20260919"
SAVE_ROOT="$ROOT/checkpoints/engineering-only/dehazeformer_smoke_20260919"
RESULT_ROOT="$ROOT/outputs/dehazeformer_smoke_20260919"
...
mkdir -p "$EVID"
printf 'PREFLIGHT_RUNNING run=dehazeformer_smoke_20260919 time=%s\n' "$(date --iso-8601=seconds)" | tee -a "$STATUS"

run_body() {
  printf 'run_id=dehazeformer_smoke_20260919\n'
...
cat "$LOG"
if [[ "$rc" -eq 0 ]]; then
  printf 'COMPLETED_GATE_PASS run=dehazeformer_smoke_20260919 rc=0 time=%s\n' "$(date --iso-8601=seconds)" | tee -a "$STATUS"
  printf 'DEHAZEFORMER_SMOKE_RUN_OK\n'
else
  printf 'PREFLIGHT_FAILED_ENGINEERING run=dehazeformer_smoke_20260919 rc=%s time=%s\n' "$rc" "$(date --iso-8601=seconds)" | tee -a "$STATUS"
