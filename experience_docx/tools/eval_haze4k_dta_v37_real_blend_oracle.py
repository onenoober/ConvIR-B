#!/usr/bin/env python3
"""Evaluate actual DTA-v3.7 soft-blend action-bank oracles on train folds.

Phase A used a linear delta proxy for alpha-shrunk actions. This tool does the
expensive but decisive check: render A0 and DTA candidate images, blend tensors
as A0 + alpha * (candidate - A0), and compute real PSNR/SSIM deltas.
"""
...
UTILITY_MODES = ("max_dpsnr", "tail_averse", "ssim_guarded")
...
def checkpoint_path(args: argparse.Namespace, variant: str) -> Path:
    model_name = (
        f"ConvIR-Haze4K-DTA-v3-5-FDF-RCS-{variant}-"
        f"seed{args.seed}_f{args.fold}-{args.stage}"
    )
    return (
...
def main() -> None:
    ...
    checkpoints = {variant: checkpoint_path(args, variant) for variant in VARIANT_ORDER}
    ...
    print(
        f"DTA_V3_7_REAL_BLEND_GROUP_START fold={args.fold} seed={args.seed} "
        f"device={device} max_images={args.max_images}",
        flush=True,
    )
    ...
    with torch.no_grad():
        for idx, data in enumerate(dataloader):
            ...
            if (idx + 1) % 50 == 0:
                ...
                print(
                    f"DTA_V3_7_REAL_BLEND_GROUP_PROGRESS fold={args.fold} seed={args.seed} "
                    f"images={idx + 1}/{len(dataloader)} elapsed_sec={elapsed:.1f}",
                    flush=True,
                )

    selected_csv = args.output_dir / f"v37_real_blend_selected_seed{args.seed}_f{args.fold}.csv"
    single_action_csv = args.output_dir / f"v37_real_blend_single_actions_seed{args.seed}_f{args.fold}.csv"
    ...
    summary = {
        ...
        "phase": "C1_real_soft_blend_group",
        "fold": args.fold,
        "seed": args.seed,
        "split_name": split_name,
    ...
    summary_path = args.output_dir / f"v37_real_blend_summary_seed{args.seed}_f{args.fold}.json"
    summary_path.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(
        f"DTA_V3_7_REAL_BLEND_GROUP_OK fold={args.fold} seed={args.seed} "
