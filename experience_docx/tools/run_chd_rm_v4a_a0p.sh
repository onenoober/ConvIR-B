...
RUNNER=$REMOTE_REPO/experience_docx/tools/chd_rm_v4a_a0p_audit.py
A0R_ROOT=$RUN_ROOT/a0r_r2
A0R_CLOSEOUT=$A0R_ROOT/v4a_a0r_closeout.json
A0R_TRACE=$A0R_ROOT/r1/trace
...
LOG=$RUN_ROOT/v4a_a0p_${STAMP}.log
echo "stage_start route=$ROUTE_ID stage=v4a-A0P-paired-applied-update-audit run=$RUN_ID time=$(date --iso-8601=seconds)" | tee -a "$STATUS"
...
CUDA_VISIBLE_DEVICES="$GPU" PYTHONUNBUFFERED=1 "$PY" "$RUNNER" audit \
  --v3z-root "$V3Z_ROOT" --a0r-closeout "$A0R_CLOSEOUT" --a0r-trace-dir "$A0R_TRACE" \
  --mode projected --output_dir "$OUT" --run_tag v4a_a0p "${COMMON_ARGS[@]}" 2>&1 | tee -a "$LOG"
rc=${PIPESTATUS[0]}
set -e

echo "stage_done route=$ROUTE_ID stage=v4a-A0P-paired-applied-update-audit run=$RUN_ID rc=$rc time=$(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -ne 0 ]; then
  echo V4A_A0P_FAILED_COMMAND_OR_INFRA | tee -a "$STATUS"
  exit "$rc"
fi

cp "$OUT/v4a_a0p_closeout.json" "$EVID_STAGE/"
cp "$OUT/v4a_a0p_step_summary.csv" "$EVID_STAGE/"
cp "$OUT/v4a_a0p_poststep_summary.csv" "$EVID_STAGE/"
cp "$OUT/v4a_a0p_window_assignment_manifest.json" "$EVID_STAGE/"
if grep -q '"state": "COMPLETED_GATE_PASS"' "$OUT/v4a_a0p_closeout.json"; then
  echo V4A_A0P_OK | tee -a "$STATUS"
else
  echo V4A_A0P_GATE_FAIL | tee -a "$STATUS"
fi
