#!/usr/bin/env python3
"""Complete the outcome-blind S0 scene and evidence-role ledger."""
...
ROUTE_ID = "daytime-dehazing-haze4k-identity-v1"
OPERATION_ID = "DAYTIME_DEHAZING_HAZE4K_IDENTITY_COMPLETE"
SUMMARY_NAME = "daytime_dehazing_haze4k_identity_v1_summary.json"
IDENTITY_NAME = "daytime_dehazing_haze4k_identity_v1_identity_summary.json"
ROLE_NAME = "daytime_dehazing_haze4k_identity_v1_role_summary.json"
REVIEW_FACTS_NAME = "daytime_dehazing_haze4k_identity_v1_review_facts.json"
RAW_LEDGER_NAME = "daytime_dehazing_haze4k_identity_v1_scene_role_ledger.jsonl"
...
def run(context_path: Path) -> None:
    ...
    if context.total_units != 5 or context.device != "cpu":
        raise RuntimeError("Haze4K identity-completion runtime contract mismatch")
    if any(context.protected_data_permissions.values()):
        raise RuntimeError("Haze4K identity completion forbids protected permissions")
    if load_completed_unit_ledger(context):
        raise RuntimeError("fresh identity-completion workload has completed units")
    ...
    summary = {
        ...
        "run_id": context.run_id,
        "program_id": "daytime_dehazing_spatially_adaptive_restoration_v1",
        "question_answered": "Whether the corrected Haze4K path completes the parent-bound Haze4K, ITS, OTS, and NH-Haze inventory sufficiently to support the unchanged program-local, scene-grouped, known-overlap-controlled four-role contract.",
        ...
        "marker": "DAYTIME_DEHAZING_HAZE4K_IDENTITY_V1_COMPLETE",
    }
