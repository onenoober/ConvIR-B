...
on_error() {
  rc=$?
  printf 'FINALIZATION_HISTORY_FETCH_FIX_CLOUD_FAILED line=%s command=%q rc=%s\n' \
    "$1" "$2" "$rc" >&2
  exit "$rc"
}
trap 'on_error "$LINENO" "$BASH_COMMAND"' ERR

branch=codex/finalization-history-fetch-fix
base=1b76328736824c899b3d46237c072b89de7a16b6
...
work=$(/usr/bin/mktemp -d /tmp/finalization-history-fetch-fix.XXXXXX)

cleanup() {
  case "$work" in
    /tmp/finalization-history-fetch-fix.*)
      /bin/rm -rf -- "$work"
      ;;
    *)
...
printf 'FINALIZATION_HISTORY_FETCH_FIX_STAGE=checkout\n'
...
printf 'FINALIZATION_HISTORY_FETCH_FIX_STAGE=compile\n'
"$python" -m py_compile \
  "$tools/convir_ops_mcp.py" \
  "$tests/test_convir_ops_mcp.py"

printf 'FINALIZATION_HISTORY_FETCH_FIX_STAGE=focused_regression\n'
"$python" -m unittest -v test_convir_ops_mcp

printf 'FINALIZATION_HISTORY_FETCH_FIX_STAGE=full_control_plane_regression\n'
...
printf 'FINALIZATION_HISTORY_FETCH_FIX_CLOUD_OK candidate=%s tests=%s gpu_access=0 dataset_access=0 protected_data_access=0 experiment_launch=0\n' \
