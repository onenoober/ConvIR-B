...
BRANCH=codex/experiment-spec-schema2-guard
...
RUN_ROOT=$BASE/runs/experiment-spec-schema2-guard/$REMOTE_COMMIT
REPO=$BASE/repos/experiment-spec-schema2-guard-$REMOTE_COMMIT
...
PYTHONPATH="$REPO/experience_docx/tools:$REPO/experience_docx/tools/tests" \
  ...
  test_route_runtime_contract 2>&1 | tee "$LOG"

grep -Fq 'test_schema1_is_read_only_and_cannot_write_a_new_bundle' "$LOG"
grep -Fq 'test_historical_manifest_schemas_remain_supported' "$LOG"
grep -Fq 'OK' "$LOG"
printf 'state=PASSED_ENGINEERING\nbranch=%s\ncommit=%s\nmarker=EXPERIMENT_SPEC_SCHEMA2_GUARD_CLOUD_OK\n' \
  "$BRANCH" "$REMOTE_COMMIT" > "$STATUS"
echo EXPERIMENT_SPEC_SCHEMA2_GUARD_CLOUD_OK
