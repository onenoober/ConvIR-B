#!/usr/bin/env python3
"""Run the frozen-BatchNorm CARL by consistency 2x2 factorial."""
...
ROUTE_ID = "daytime-dehazing-density-consistency-carl-multiseed-frozen-bn-stability-v1"
OPERATION_ID = "DAYTIME_DEHAZING_DENSITY_CONSISTENCY_CARL_MULTISEED_FROZEN_BN_STABILITY_QUALIFY"
...
PROBE_STEPS_PER_CELL = 512
PROBE_EVALUATION_SCENES = 4
PROBE_SEEDS = SEEDS
...
NUMERICAL_MODE = "cuda_tf32_disabled_checkpoint_frozen_batch_norm"
BATCH_NORM_POLICY = "checkpoint_running_statistics_frozen_affine_trainable"
FROZEN_BATCH_NORM_BUFFERS = (
    "running_mean",
    "running_var",
    "num_batches_tracked",
)
...
def configure_numerical_mode(torch: Any) -> None:
    ...
    if (
        ...
        raise RuntimeError("explicit CUDA TF32 disablement did not bind")


def finite_context(seed: int, arm: str, step: int, label: str) -> str:
    return f"non-finite {label} at seed={seed} arm={arm} step={step}"
...
def freeze_batch_norm_running_state(torch: Any, model: Any) -> None:
    references: dict[str, dict[str, Any]] = {}
    for module_name, module in model.named_modules():
        if not isinstance(module, torch.nn.modules.batchnorm._BatchNorm):
            continue
        if not module.track_running_stats:
            raise RuntimeError(f"BatchNorm lacks running-state tracking: {module_name}")
        module.eval()
        buffers: dict[str, Any] = {}
        for buffer_name in FROZEN_BATCH_NORM_BUFFERS:
            value = getattr(module, buffer_name, None)
            if value is None:
                raise RuntimeError(
                    f"BatchNorm running-state buffer is unavailable: "
                    f"{module_name}.{buffer_name}"
                )
            buffers[buffer_name] = value.detach().clone()
        if not bool(torch.isfinite(buffers["running_mean"]).all().item()):
            raise RuntimeError(
                f"checkpoint BatchNorm running_mean is non-finite: {module_name}"
            )
        if (
            not bool(torch.isfinite(buffers["running_var"]).all().item())
            or bool((buffers["running_var"] < 0).any().item())
        ):
            raise RuntimeError(
                f"checkpoint BatchNorm running_var is invalid: {module_name}"
            )
        references[module_name] = buffers
    if not references:
        raise RuntimeError("official ConvIR path exposes no BatchNorm running state")
    model.__dict__["_convir_frozen_batch_norm_reference"] = references


def require_frozen_batch_norm_state(
    torch: Any,
    model: Any,
    *,
    seed: int,
    ...
    references = model.__dict__.get("_convir_frozen_batch_norm_reference")
    if not isinstance(references, dict) or not references:
        raise RuntimeError("frozen BatchNorm reference is unavailable")
    modules = dict(model.named_modules())
    for module_name, expected_buffers in references.items():
        module = modules.get(module_name)
        if module is None or module.training:
            raise RuntimeError(
                f"frozen BatchNorm mode changed at seed={seed} arm={arm} "
                f"step={step} module={module_name} phase={label}"
            )
        for buffer_name, expected in expected_buffers.items():
            current = getattr(module, buffer_name, None)
            state_name = f"{module_name}.{buffer_name}"
            if current is None or current.shape != expected.shape:
                raise RuntimeError(
                    f"frozen BatchNorm identity changed at seed={seed} arm={arm} "
                    f"step={step} state={state_name} phase={label}"
                )
            if current.is_floating_point() and not bool(
                torch.isfinite(current.detach()).all().item()
            ):
                raise RuntimeError(
                    finite_context(
                        seed,
                        arm,
                        step,
                        f"{label}.frozen_batch_norm.{state_name}",
                    )
                )
            if not bool(torch.equal(current.detach(), expected)):
                raise RuntimeError(
                    f"frozen BatchNorm value changed at seed={seed} arm={arm} "
                    f"step={step} state={state_name} phase={label}"
                )
...
def load_convir(context: Any, torch: Any, *, trainable: bool) -> Any:
    ...
    freeze_batch_norm_running_state(torch, model)
    return model
...
def psnr(
    ...
    *,
    ...
    require_finite_tensor(
        ...
        arm=arm,
        step=step,
        label="evaluation_prediction",
    )
    mse_tensor = torch.mean(torch.square(prediction.clamp(0.0, 1.0) - target))
    require_finite_tensor(
        torch,
        mse_tensor,
        seed=seed,
        arm=arm,
        step=step,
        label="evaluation_mse",
    )
    mse = float(mse_tensor.item())
    score = 99.0 if mse <= 1.0e-12 else -10.0 * math.log10(mse)
    if not math.isfinite(score):
        raise RuntimeError(finite_context(seed, arm, step, "evaluation_psnr"))
    return score
...
def update_ema(teacher: Any, student: Any) -> None:
    ...
    references = student.__dict__.get("_convir_frozen_batch_norm_reference", {})
    frozen_buffer_names = {
    ...
    for name, target in teacher_state.items():
        ...
        if name in frozen_buffer_names:
            target.copy_(source)
...
def train_arm(
    ...
    require_finite_state(
        torch,
        model_state_tensors(model),
        seed=seed,
        arm=arm,
        step=0,
        label="initial_parameter_state",
    )
    require_frozen_batch_norm_state(
        torch,
        model,
        seed=seed,
        arm=arm,
        step=0,
        label="initial_student_state",
    )
    require_frozen_batch_norm_state(
        torch,
        teacher,
    ...
    for step in range(TRAINING_STEPS):
        ...
        require_finite_tensor(
            ...
            arm=arm,
            step=step_number,
            label="training_prediction",
        )
        reconstruction_loss = functional.l1_loss(prediction, clear)
        require_finite_tensor(
            torch,
            reconstruction_loss,
            seed=seed,
        ...
        if arm in {"b01_consistency", "b11_combined"}:
            ...
            require_finite_tensor(
                ...
                arm=arm,
                step=step_number,
                label="teacher_prediction",
            )
            consistency_loss = LAMBDA_CONSISTENCY * functional.l1_loss(
                prediction,
                teacher_prediction,
            )
            require_finite_tensor(
        ...
        if arm in {"b10_carl", "b11_combined"}:
            contrastive_loss = LAMBDA_CARL * carl_loss(
                torch,
                features,
                prediction,
                clear,
        ...
        loss = sum(loss_components[1:], loss_components[0])
        require_finite_tensor(
            torch,
            loss,
            seed=seed,
...
def load_trained_arm(
    ...
    require_finite_state(
        ...
        arm=arm,
        step=0,
        label="loaded_checkpoint_parameter_state",
    )
    return model.eval()
    ...
    with torch.no_grad():
        for stratum, groups in strata.items():
            for group in groups:
                ...
                for item in hazy:
                    evaluation_step += 1
                    prediction = unwrap_prediction(model(item))
                    require_frozen_batch_norm_state(
                        torch,
                        model,
                        seed=seed,
                ...
                if not math.isfinite(scene_score):
                    raise RuntimeError(
                        finite_context(seed, arm, evaluation_step, "scene_mean_psnr")
                    )
    ...
    for scene_id in scene_ids:
        ...
        for seed in completed_seeds:
            ...
            if any(not math.isfinite(value) for value in values.values()):
                raise RuntimeError(
                    finite_context(seed, "factorial", 0, "evaluation_arm_score")
                )
            combined = values["b11_combined"] - values["b00_baseline"]
            if not math.isfinite(combined):
                raise RuntimeError(
                    finite_context(seed, "factorial", 0, "raw_grouped_contrast")
                )
...
def structured_synthetic_batch(
    torch: Any,
    *,
    device: str,
    seed: int,
    ...
    scenes = []
    density_fields = []
    seed_phase = (seed % 1009) * (2.0 * math.pi / 1009.0)
    for batch_index in range(BATCH_SIZE):
        phase = seed_phase + step * 0.037 + batch_index * 1.173
        edge = (grid_x > 0.32 + 0.11 * batch_index).to(grid_x.dtype)
        red = 0.48 + 0.24 * torch.sin(2.0 * math.pi * grid_x + phase)
        green = 0.46 + 0.22 * torch.cos(2.0 * math.pi * grid_y - phase)
        blue = 0.44 + 0.20 * torch.sin(
            2.0 * math.pi * (grid_x + grid_y) + 0.5 * phase
        )
        scene = torch.stack((red + 0.10 * edge, green, blue - 0.08 * edge))
        scenes.append(scene.clamp(0.0, 1.0))
        density_fields.append(
            0.07
            * torch.sin(
                2.0 * math.pi * (grid_x - grid_y)
                + phase
                + batch_index * 0.31
            )
        )
    clear = torch.stack(scenes).to(device=device)
    density = torch.stack(density_fields).unsqueeze(1).to(device=device)
    airlight = torch.tensor(
        ...
        dtype=clear.dtype,
        device=device,
    ).view(BATCH_SIZE, 3, 1, 1)
    observations = []
    for base_transmission in (0.30, 0.45, 0.65, 0.85):
        transmission = (base_transmission + density).clamp(0.15, 0.95)
        observations.append(
            clear * transmission + airlight * (1.0 - transmission)
        )
    if bool(torch.equal(clear[0], clear[1])):
        raise RuntimeError("structured synthetic batch does not contain distinct scenes")
...
def synthetic_training_cell(
    ...
    *,
    ...
    axis = torch.linspace(0.0, 1.0, TRAIN_CROP, device=context.device)
    grid_y, grid_x = torch.meshgrid(axis, axis, indexing="ij")
    for cell_step in range(1, iterations + 1):
        clear, observations = structured_synthetic_batch(
            torch,
            device=context.device,
            seed=seed,
            step=cell_step,
        ...
        require_frozen_batch_norm_state(
            ...
            arm=arm,
            step=cell_step,
            label="synthetic_student_forward",
        )
        require_finite_tensor(
            torch,
            prediction,
        ...
        require_finite_tensor(
            ...
            arm=arm,
            step=cell_step,
            label="synthetic_reconstruction_loss",
        )
        loss_components = [reconstruction_loss]
        if arm in {"b01_consistency", "b11_combined"}:
            with torch.no_grad():
            ...
            require_frozen_batch_norm_state(
                ...
                arm=arm,
                step=cell_step,
                label="synthetic_teacher_forward",
            )
            require_finite_tensor(
                torch,
                teacher_prediction,
                ...
                arm=arm,
                step=cell_step,
                label="synthetic_teacher_prediction",
            )
            consistency_loss = LAMBDA_CONSISTENCY * functional.l1_loss(
                prediction,
                teacher_prediction,
            ...
            require_finite_tensor(
                ...
                arm=arm,
                step=cell_step,
                label="synthetic_consistency_loss",
            )
            loss_components.append(consistency_loss)
        if arm in {"b10_carl", "b11_combined"}:
            contrastive_loss = LAMBDA_CARL * carl_loss(
        ...
        require_finite_state(
            ...
            arm=arm,
            step=cell_step,
            label="synthetic_gradient_state",
        )
        optimizer.step()
        require_frozen_batch_norm_state(
            torch,
            ...
            arm=arm,
            step=cell_step,
            label="synthetic_optimizer_step",
        )
        require_finite_state(
            torch,
            model_state_tensors(model),
            ...
            arm=arm,
            step=cell_step,
            label="synthetic_parameter_state",
        )
        require_finite_optimizer(
            torch,
            optimizer,
    ...
    with torch.no_grad():
        for scene_index in range(PROBE_EVALUATION_SCENES):
            clear_batch, observation_batches = structured_synthetic_batch(
                torch,
                device=context.device,
                seed=seed,
                step=iterations + scene_index + 1,
                grid_x=grid_x,
                grid_y=grid_y,
            )
            clear = clear_batch[:1]
            observations = [value[:1] for value in observation_batches]
            observation_scores = []
            for observation_index, observation in enumerate(observations):
                ...
                prediction = unwrap_prediction(model(observation))
                require_frozen_batch_norm_state(
                    torch,
                    model,
                    seed=seed,
                    arm=arm,
                    step=evaluation_step,
                    label="synthetic_evaluation_forward",
                )
                observation_scores.append(
                    psnr(
                        torch,
...
def contract(context_path):
    ...
    configure_numerical_mode(torch)
    ...
    if completed_iterations != PROBE_ITERATIONS:
        raise RuntimeError("synthetic qualification did not cover the frozen probe map")
    write_contract_result(
        context,
        checks={
            ...
            "all_five_frozen_seeds_four_arm_path_exercised": True,
            "representative_512_step_cell_paths_exercised": True,
            "two_distinct_scene_batch_exercised": True,
            "structured_deterministic_haze_stress_fixture_exercised": True,
            ...
            "checkpoint_batch_norm_running_state_frozen": True,
            "post_forward_batch_norm_state_checks_exercised": True,
...
def run(context_path):
    ...
    write_scientific_review_facts(
        ...
        relpath=(
            ...
            "multiseed_frozen_bn_stability_v1_review_facts.json"
        ),
        facts=[
            build_primary_review_fact(
