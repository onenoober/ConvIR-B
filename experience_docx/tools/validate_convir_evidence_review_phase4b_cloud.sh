...
on_error() {
  rc=$?
  printf 'CONVIR_EVIDENCE_REVIEW_PHASE4B_CLOUD_FAILED line=%s command=%q rc=%s\n' \
    "$1" "$2" "$rc" >&2
  exit "$rc"
}
...
baseline=e4ddd62ef1e6b45bec6f70b5197ef6a72de43531
...
test -d "$runtime_root"
work=$(mktemp -d "$runtime_root/convir-evidence-review-phase4b.XXXXXX")
case "$work" in
  /sda/home/wangyuxin/ConvIR-B/runtime/convir-evidence-review-phase4b.*) ;;
...
bash -n "$tools/validate_convir_evidence_review_phase4b_cloud.sh"
...
assert info["inventorySourceSha256"] == hashlib.sha256(
    (server.parent / "convir_evidence_cloud_inventory.py").read_bytes()
).hexdigest()
...
assert summary["structuredContent"]["header"]["snapshot_commit"] == main_tip
...
trap - EXIT
cleanup_work
printf 'CONVIR_EVIDENCE_REVIEW_PHASE4B_CLOUD_OK candidate=%s github_main=%s tests=%s focused_tests=%s tools=4 real_run_root_access=0 model_calls=0 gpu_access=0 dataset_access=0 protected_data_access=0\n' \
