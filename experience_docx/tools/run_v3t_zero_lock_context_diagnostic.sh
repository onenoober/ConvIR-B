...
REMOTE_REPO=${REMOTE_REPO:-/sda/home/wangyuxin/ConvIR-B/repos/ConvIR-B-v3t-zero-lock-context-diagnostic-20260713}
RUN_ROOT=${RUN_ROOT:-/sda/home/wangyuxin/ConvIR-B/runs/haze4k_v5_chd_rm_v3t_zero_lock_context_diagnostic_20260713}
...
GPU=${GPU:-1}
MODE=${MODE:?set MODE=noop|factorial}
EXPECTED_ROUTE_COMMIT=${EXPECTED_ROUTE_COMMIT:?set the exact v3t route commit before launch}

ROUTE_ID=haze4k_v5_chd_rm_v3t_zero_lock_context_diagnostic_20260713
BRANCH=codex/haze4k-v5-v3t-zero-lock-context-diagnostic-20260713
...
case "$MODE" in
  noop)
    ...
    RUN_TAG=v3t_s0_noop32
    OUT=$RUN_ROOT/s0_noop32
    ;;
  factorial)
    STAGE=v3t-S1-four-cell-zero-lock-diagnostic
    RUN_TAG=v3t_s1_factorial32
    OUT=$RUN_ROOT/s1_factorial32
    ;;
  *)
    echo V3T_INVALID_MODE
    exit 2
    ;;
esac
...
if [ "$MODE" = factorial ]; then
  test -s "$EVID_STAGE/v3t_s0_noop32_closeout.json"
  "$PY" - "$EVID_STAGE/v3t_s0_noop32_closeout.json" <<'PY'
import json
import sys
closeout = json.load(open(sys.argv[1], encoding="utf-8"))
if closeout.get("decision") != "V3T_S0_NOOP_PASS_AUTHORIZE_FACTORIAL_ONLY":
    raise SystemExit("v3t factorial requires S0 no-op pass")
PY
fi
...
set +e
CUDA_VISIBLE_DEVICES="$GPU" PYTHONUNBUFFERED=1 "$PY" "$REMOTE_REPO/experience_docx/tools/chd_rm_v3t_zero_lock_context_diagnostic.py" "${ARGS[@]}" 2>&1 | tee "$LOG"
rc=${PIPESTATUS[0]}
set -e
echo "stage_done route=$ROUTE_ID stage=$STAGE rc=$rc time=$(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -ne 0 ]; then
  echo "V3T_${MODE^^}_FAILED" | tee -a "$STATUS"
  exit "$rc"
fi
...
echo "V3T_${MODE^^}_OK" | tee -a "$STATUS"
