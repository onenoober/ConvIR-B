#!/usr/bin/env python3
"""Run the Haze4K direct selective-controller learning-curve qualification."""
...
ROUTE_ID = "haze4k-direct-selective-controller-learning-curve-v1"
OPERATION_ID = "HAZE4K_DIRECT_SELECTIVE_CONTROLLER_QUALIFY"
PARENT_ROUTE_ID = "haze4k-observable-tail-factorial-v1"
PARENT_ROUTE_COMMIT = "b4ce59309a12b68e79dbfc8534a1b106e162110a"
PARENT_RECEIPT = "ce6d9f96bab31b3c46ea5d7024e259e8d016cc4362be30b65d94f57f8d61ecaa"
...
# Direct-controller contract. The older factorial helpers below are retained only
# for exact cache decoding and image-level replay from the verified parent route.
DIRECT_ARMS = ("joint", "separate")
DIRECT_ABLATION_ARM = "frozen"
DIRECT_TRAIN_SIZES = (120, 240, 360, 480)
DIRECT_EPOCHS = 24
DIRECT_TARGET_TEMPERATURE_DB = 0.05
DIRECT_TAIL_PENALTY = 4.0
DIRECT_HARM_PENALTY_DB = 0.25
DIRECT_POLICY_CE_WEIGHT = 1.0
DIRECT_EXPECTED_UTILITY_WEIGHT = 0.5
DIRECT_EXPECTED_TAIL_WEIGHT = 2.0
DIRECT_SEPARATE_RISK_WEIGHT = 0.5
DIRECT_ENTROPY_FLOOR_WEIGHT = 0.01
DIRECT_SUBSET_SALT = "haze4k-direct-selective-controller-learning-curve-v1-subset"
DIRECT_FOLD_SALT = "haze4k-direct-selective-controller-learning-curve-v1-fold"
DIRECT_SHUFFLE_SALT = "haze4k-direct-selective-controller-learning-curve-v1-shuffle"
DIRECT_BOOTSTRAP_SEED = 20260728
DIRECT_TRAINING_SEED = 20260728
DIRECT_BOOTSTRAP_RESAMPLES = 100_000
DIRECT_TRAINING_UNITS = (
    len(DIRECT_ARMS) * len(DIRECT_TRAIN_SIZES) * OOF_FOLDS * len(SEED_OFFSETS)
    + OOF_FOLDS * len(SEED_OFFSETS)
)
DIRECT_TOTAL_UNITS = DIRECT_TRAINING_UNITS + 2
DIRECT_FORMAL_COST_ITERATIONS = 1_053_000
DIRECT_PROBE_COST_ITERATIONS = 9
DIRECT_PARENT_SCENE_CACHE_COUNT = 750
DIRECT_PARENT_SCENE_CACHE_BYTES = 3_341_602_500
DIRECT_PARENT_SCENE_CACHE_DIGEST = "7d498d790b0f4d26fa87a901485423ba97a2d498bc96c6e4f21fef6a09284382"
DIRECT_PARENT_TEST_CACHE_COUNT = 400
DIRECT_PARENT_TEST_CACHE_BYTES = 1_726_230_000
DIRECT_PARENT_TEST_CACHE_DIGEST = "af72b61f572ee20c38cd72eeaf2df17ea2169f3a25898e7f8f2d900784fa8c54"
DIRECT_PARENT_RAW_INVENTORY_SHA256 = "9a5249f66c994f9c94245145cc65e02da643ada71cc047cd5144db475b3c66ff"
DIRECT_PARENT_CLOSEOUT_SHA256 = "de8771b362d8ead69cc536a8d4e490b464310821f80dcf99344abf9c8483c167"
DIRECT_PARENT_SUMMARY_SHA256 = "95a6deb9c765d1cb002cd3dcd1b0e967f704e21f9f68da57057b904b08ef433a"
DIRECT_PRIMARY_FAMILY_SIZE = 4
...
def _factorial_contract(context_path: Path) -> None:
    context = load_context(context_path, "contract")
...
def _factorial_run(context_path: Path) -> None:
    context = load_context(context_path, "run")
    if context.route_id != ROUTE_ID or context.operation_id != OPERATION_ID:
        raise RuntimeError("route or operation identity mismatch")
    prepare_phase_output(context)
...
def make_direct_controller(torch, arm: str):
    import torch.nn as nn

    if arm not in {*DIRECT_ARMS, DIRECT_ABLATION_ARM}:
        raise RuntimeError(f"unknown direct-controller arm: {arm}")

    class DirectController(nn.Module):
        def __init__(self):
            super().__init__()
            self.arm = arm
            if arm == DIRECT_ABLATION_ARM:
                self.trunk = nn.Sequential(
                ...
                self.trunk = nn.Sequential(
                    nn.Conv2d(FEATURE_CHANNELS + 80, 96, kernel_size=3, padding=1),
                    nn.GroupNorm(8, 96),
                    nn.GELU(),
                    nn.Conv2d(96, 64, kernel_size=3, padding=1),
                    nn.GroupNorm(8, 64),
            ...
            if arm == "separate":
                self.utility_head = nn.Conv2d(64, 2, kernel_size=1)
                self.risk_head = nn.Conv2d(64, 2, kernel_size=1)
            else:
                self.action_head = nn.Conv2d(64, 3, kernel_size=1)

        def forward(self, inputs):
            import torch.nn.functional as functional

            base = inputs["features"]
            if self.arm == DIRECT_ABLATION_ARM:
                hidden = self.trunk(base)
            else:
                shape = self.shape_encoder(inputs["shape_features"])
                shape = functional.avg_pool2d(
                    shape, kernel_size=4, stride=4, ceil_mode=True,
                ...
                if shape.shape[2:] != base.shape[2:]:
                    shape = functional.interpolate(
                        shape, size=base.shape[2:], mode="bilinear",
                        align_corners=False,
                    )
...
def direct_normalization(records: list[dict[str, Any]], arm: str):
    return feature_normalization(
        records, "R0" if arm == DIRECT_ABLATION_ARM else "R2",
    )


def direct_forward(torch, controller, record, normalization, device: str):
    return controller(normalized_inputs(torch, record, normalization, device))


def direct_targets(torch, target, harm):
    zero = torch.zeros_like(target[:, :1])
    utility = torch.cat([zero, target], dim=1)
    harm_all = torch.cat([zero, harm], dim=1)
    score = utility.clone()
    score[:, 1:] = score[:, 1:] - UTILITY_MARGIN_DB
    score = score - DIRECT_TAIL_PENALTY * torch.relu(
        -utility - PSNR_HARM_MARGIN_DB
    ) - DIRECT_HARM_PENALTY_DB * harm_all
    distribution = torch.softmax(
        score / DIRECT_TARGET_TEMPERATURE_DB, dim=1,
    )
    return utility, harm_all, distribution


def direct_loss(torch, output, target, harm, area, arm: str):
    import torch.nn.functional as functional

    if arm == "separate":
        utility_loss = functional.smooth_l1_loss(
            output["utility"], target, beta=UTILITY_HUBER_BETA_DB,
            reduction="none",
        ).mean(dim=1)
        risk_loss = functional.binary_cross_entropy_with_logits(
            output["risk_logits"], harm, reduction="none",
        ...
        risk_loss = risk_loss.mean(dim=1)
        loss_map = utility_loss + DIRECT_SEPARATE_RISK_WEIGHT * risk_loss
        return torch.sum(area * loss_map.squeeze(0)), {
            "policy_ce": 0.0,
            "expected_utility": float(
                torch.sum(area * utility_loss.squeeze(0)).detach().cpu()
            ),
            "tail": float(torch.sum(area * risk_loss.squeeze(0)).detach().cpu()),
        }

    utility, harm_all, target_distribution = direct_targets(
        torch, target, harm,
    )
    log_probability = torch.log_softmax(output["action_logits"], dim=1)
    probability = torch.softmax(output["action_logits"], dim=1)
    policy_ce = -torch.sum(target_distribution * log_probability, dim=1)
    expected_utility = -torch.sum(probability * utility, dim=1)
    expected_tail = torch.sum(
        probability * (
            torch.relu(-utility - PSNR_HARM_MARGIN_DB)
            + DIRECT_HARM_PENALTY_DB * harm_all
        ),
        dim=1,
    )
    ...
    loss_map = (
        DIRECT_POLICY_CE_WEIGHT * policy_ce
        + DIRECT_EXPECTED_UTILITY_WEIGHT * expected_utility
        + DIRECT_EXPECTED_TAIL_WEIGHT * expected_tail
        + DIRECT_ENTROPY_FLOOR_WEIGHT * entropy_term
    )
    return torch.sum(area * loss_map.squeeze(0)), {
        "policy_ce": float(torch.sum(area * policy_ce.squeeze(0)).detach().cpu()),
        "expected_utility": float(
            torch.sum(area * expected_utility.squeeze(0)).detach().cpu()
        ),
        "tail": float(torch.sum(area * expected_tail.squeeze(0)).detach().cpu()),
    }


def train_direct_controller(
    torch,
    records: list[dict[str, Any]],
    *,
    arm: str,
    ...
    if torch.cuda.is_available():
        ...
        torch.backends.cudnn.benchmark = False
        torch.backends.cudnn.deterministic = True
    controller = make_direct_controller(torch, arm).to(device)
    normalization = direct_normalization(records, arm)
    optimizer = torch.optim.AdamW(
        controller.parameters(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY,
    )
    history = []
    for epoch in range(epochs):
        order = np.random.default_rng(seed + epoch).permutation(len(records))
        ...
        for raw_index in order:
            ...
            area = torch.from_numpy(record["areas"].copy()).to(device)
            area = area / torch.sum(area)
            output = direct_forward(
                torch, controller, record, normalization, device,
            )
            loss, components = direct_loss(
                torch, output, target, harm, area, arm,
            )
            if not bool(torch.isfinite(loss).item()):
                raise RuntimeError("direct-controller loss became non-finite")
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(
                controller.parameters(), GRADIENT_CLIP_NORM,
            )
            optimizer.step()
        ...
        history.append({
            "epoch": epoch + 1,
            **{
                key: float(totals[key] / len(records))
                for key in ("loss", "policy_ce", "expected_utility", "tail")
            },
        })
        if progress_callback is not None:
            progress_callback(epoch + 1)
    return controller.eval(), normalization, history


def predict_direct(torch, controller, record, normalization, device: str):
    with torch.inference_mode():
        output = direct_forward(
            torch, controller, record, normalization, device,
        )
        if controller.arm == "separate":
            result = {
                "utility": output["utility"].squeeze(0).detach().cpu().numpy().astype(np.float64),
                "risk": torch.sigmoid(
                    output["risk_logits"]
    ...
    if not all(np.isfinite(value).all() for value in result.values()):
        raise RuntimeError("direct-controller prediction became non-finite")
    return result


def predict_direct_ensemble(torch, bundles, record, device: str):
    predictions = [
        predict_direct(
            torch, bundle["controller"], record,
            bundle["normalization"], device,
        )
        for bundle in bundles
    ]
    return {
        key: np.mean(np.stack([item[key] for item in predictions]), axis=0)
        for key in predictions[0]
    }


def direct_actions(prediction: dict[str, np.ndarray]) -> np.ndarray:
    if "probability" in prediction:
        return np.argmax(prediction["probability"], axis=0).astype(np.int64)
    utility = prediction["utility"]
    risk = prediction["risk"]
    score = utility - UTILITY_MARGIN_DB - DIRECT_HARM_PENALTY_DB * risk
    eligible = (score > 0.0) & (risk <= RISK_ACCEPTANCE_MAX)
    best = np.argmax(np.where(eligible, score, -np.inf), axis=0)
    return np.where(np.any(eligible, axis=0), best + 1, 0).astype(np.int64)


def direct_numpy_target(record: dict[str, Any]):
    utility = np.concatenate([
        np.zeros((1, *record["utility"].shape[1:]), dtype=np.float64),
        record["utility"].astype(np.float64),
    ], axis=0)
    harm = np.concatenate([
        np.zeros((1, *record["harm"].shape[1:]), dtype=np.float64),
        record["harm"].astype(np.float64),
    ], axis=0)
    score = utility.copy()
    score[1:] -= UTILITY_MARGIN_DB
    score -= (
        DIRECT_TAIL_PENALTY * np.maximum(-utility - PSNR_HARM_MARGIN_DB, 0.0)
        + DIRECT_HARM_PENALTY_DB * harm
    )
    shifted = score / DIRECT_TARGET_TEMPERATURE_DB
    shifted -= np.max(shifted, axis=0, keepdims=True)
    probability = np.exp(shifted)
    probability /= np.sum(probability, axis=0, keepdims=True)
    return utility, harm, score, probability


def evaluate_direct_actions(
    torch,
    record: dict[str, Any],
    hazy: np.ndarray,
    clear: np.ndarray,
    ...
    *,
    ...
    output = apply_spatial(hazy, prediction, actions)
    shuffled = np.random.default_rng(
        deterministic_seed(f"{DIRECT_SHUFFLE_SALT}|{scene}"),
    ).permutation(actions.reshape(-1)).reshape(actions.shape)
    shuffled_output = apply_spatial(hazy, prediction, shuffled)
    keep_psnr = psnr(prediction, clear)
    gain = psnr(output, clear) - keep_psnr
    shuffled_gain = psnr(shuffled_output, clear) - keep_psnr
    areas = record["areas"].astype(np.float64)
    total_area = float(np.sum(areas))
    selected = actions > 0
    ...
    selected_utility_sum = 0.0
    for action in range(2):
        mask = actions == action + 1
        selected_harm_area += float(
            np.sum(areas[mask] * record["harm"][action][mask])
        )
        selected_utility_sum += float(
            np.sum(areas[mask] * record["utility"][action][mask])
        )
    weights = areas / total_area
    result = {
        "scene": scene,
        ...
        "gain": gain,
        "keep_psnr": keep_psnr,
        "shuffled_gain": shuffled_gain,
        "gain_minus_shuffled": gain - shuffled_gain,
        "selected_area": selected_area,
        "selected_harm_area": selected_harm_area,
        "total_area": total_area,
        "action_area": selected_area / total_area,
        "selected_harm_fraction": (
            selected_harm_area / selected_area if selected_area > 0.0 else 0.0
        ),
        "selected_utility_mean": (
            selected_utility_sum / selected_area if selected_area > 0.0 else 0.0
        ),
        "active_scene": int(selected_area > 0.0),
        "material_scene": int(gain >= UTILITY_MARGIN_DB),
        "severe_harm_scene": int(gain <= -PSNR_HARM_MARGIN_DB),
        "keep_fraction": weighted_mean(actions == 0, weights),
        "weaken_fraction": weighted_mean(actions == 1, weights),
        "strengthen_fraction": weighted_mean(actions == 2, weights),
        "ssim_gain": None,
        "color_bias_delta": None,
    }
    if full_metrics:
        ssim = rgb_ssim(torch, [prediction, output], clear, device)
        result["ssim_gain"] = ssim[1] - ssim[0]
        result["color_bias_delta"] = (
            color_bias(output, clear) - color_bias(prediction, clear)
        )
    return result


def verify_parent_cache_class(
    ...
    *,
    ...
    directory = parent_run_root / relative_directory
    paths = sorted(path for path in directory.iterdir() if path.suffix == ".npz")
    rows = []
    total_bytes = 0
    for path in paths:
        ...
        total_bytes += size
        rows.append(
            f"{path.relative_to(parent_run_root).as_posix()}|{sha256_file(path)}|{size}"
        )
    digest = digest_lines(rows)
    return {
...
def direct_feasibility(
    calibration_records: list[dict[str, Any]],
    test_cache_root: Path,
) -> dict[str, Any]:
    entropies = []
    margins = []
    action_area = np.zeros(3, dtype=np.float64)
    total_area = 0.0
    effective_positive_scenes = 0
    effective_harm_scenes = 0
    frontiers = {
        threshold: Counter()
        for threshold in (0.0, 0.025, 0.05, 0.10, 0.20)
    }
    for record in calibration_records:
        utility, harm, score, probability = direct_numpy_target(record)
        areas = record["areas"].astype(np.float64)
        total = float(np.sum(areas))
        target = np.argmax(score, axis=0)
        selected = target > 0
        selected_area = float(np.sum(areas[selected]))
        if selected_area / total >= MIN_ACTION_AREA_FRACTION:
            effective_positive_scenes += 1
        if np.any((harm > 0.5) & (np.arange(3)[:, None, None] > 0)):
            effective_harm_scenes += 1
        entropy = -np.sum(
            probability * np.log(np.clip(probability, 1.0e-12, 1.0)),
            axis=0,
        )
        entropies.extend(entropy.reshape(-1).tolist())
        ordered = np.sort(score, axis=0)
        margins.extend((ordered[-1] - ordered[-2]).reshape(-1).tolist())
        for action in range(3):
            action_area[action] += float(np.sum(areas[target == action]))
        total_area += total
        best_action = np.argmax(score[1:], axis=0) + 1
        best_score = np.max(score[1:], axis=0)
        for threshold, counts in frontiers.items():
            chosen = np.where(best_score > threshold, best_action, 0)
            selected = chosen > 0
            counts["selected_area"] += float(np.sum(areas[selected]))
            counts["total_area"] += total
            for action in range(2):
                mask = chosen == action + 1
                counts["harm_area"] += float(
                    np.sum(areas[mask] * harm[action + 1][mask])
                )
                counts["utility_sum"] += float(
                    np.sum(areas[mask] * utility[action + 1][mask])
                )

    variant_actions: dict[str, list[int]] = defaultdict(list)
    variant_files = sorted(test_cache_root.glob("scene_*_variant_*.npz"))
    for path in variant_files:
        record = load_cache(path)
        _, _, score, _ = direct_numpy_target(record)
        actions = np.argmax(score, axis=0)
        areas = record["areas"].astype(np.float64)
        distribution = [
            float(np.sum(areas[actions == action])) for action in range(3)
        ]
        variant_actions[path.stem.rsplit("_variant_", 1)[0]].append(
            int(np.argmax(distribution))
        )
    flip_scenes = sum(
        len(set(actions)) > 1 for actions in variant_actions.values()
    )
    frontier_rows = []
    for threshold, counts in frontiers.items():
        selected_area = float(counts["selected_area"])
        frontier_rows.append({
            "threshold": threshold,
            "coverage": selected_area / float(counts["total_area"]),
            "selected_harm_ratio": (
                float(counts["harm_area"]) / selected_area
                if selected_area > 0.0 else None
            ),
            "selected_utility_mean": (
                float(counts["utility_sum"]) / selected_area
                if selected_area > 0.0 else None
            ),
        })
    action_distribution = {
        name: float(action_area[index] / total_area)
        for index, name in enumerate(("keep", "weaken", "strengthen"))
    }
    base_frontier = frontier_rows[0]
    favorable = (
        effective_positive_scenes >= 30
        and base_frontier["coverage"] >= MIN_ACTION_AREA_FRACTION
        and base_frontier["selected_harm_ratio"] is not None
        and base_frontier["selected_harm_ratio"] <= MAX_HARM_PREVALENCE
        and (
            action_distribution["weaken"]
            + action_distribution["strengthen"]
        ) >= MIN_ACTION_AREA_FRACTION
    )
    return {
        "schema_version": 1,
        "calibration_scene_count": len(calibration_records),
        "variant_scene_count": len(variant_actions),
        "variant_file_count": len(variant_files),
        "effective_positive_scene_count": effective_positive_scenes,
        "effective_harm_scene_count": effective_harm_scenes,
        "soft_target_entropy": {
            "mean": float(np.mean(entropies)),
            "median": float(np.median(entropies)),
            "p10": float(np.quantile(entropies, 0.10)),
            "p90": float(np.quantile(entropies, 0.90)),
        },
        "risk_adjusted_action_margin_db": {
            "mean": float(np.mean(margins)),
            "median": float(np.median(margins)),
            "p10": float(np.quantile(margins, 0.10)),
            "p90": float(np.quantile(margins, 0.90)),
        },
        "action_area_distribution": action_distribution,
        "variant_dominant_action_flip_rate": (
            flip_scenes / len(variant_actions) if variant_actions else None
        ),
        "oracle_selective_frontier": frontier_rows,
        "favorable": favorable,
    }


def lower_tail_mean(values: Iterable[float], fraction: float = 0.10) -> float:
    array = np.sort(np.asarray(list(values), dtype=np.float64))
    count = max(1, int(math.ceil(fraction * len(array))))
    return float(np.mean(array[:count]))


def direct_interval_rows(
    ensemble_rows: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    output = []
    combinations = sorted({
        (str(row["arm"]), int(row["train_size"])) for row in ensemble_rows
    })
    for index, (arm, train_size) in enumerate(combinations):
        rows = [
            row for row in ensemble_rows
            if row["arm"] == arm and int(row["train_size"]) == train_size
        ]
        interval = stratified_bootstrap_family(
            rows,
            ["gain"],
            family_size=len(combinations),
            seed=DIRECT_BOOTSTRAP_SEED + 100 + index,
        )["gain"]
        output.append({
            "arm": arm,
            "train_size": train_size,
            **interval,
            "cvar10_gain": lower_tail_mean(row["gain"] for row in rows),
            "severe_harm_prevalence": float(np.mean([
                row["severe_harm_scene"] for row in rows
            ])),
            "active_scene_prevalence": float(np.mean([
                row["active_scene"] for row in rows
            ])),
            "action_area": float(np.mean([row["action_area"] for row in rows])),
        })
    return output
...
def contract(context_path: Path) -> None:
    ...
    if context.device != "cuda":
        raise RuntimeError("direct-controller engineering contract requires CUDA")
    torch.cuda.reset_peak_memory_stats()
    entrypoint = asset_path(context, "controller_entrypoint", kind="file")
    runtime_environment = asset_path(context, "runtime_environment", kind="file")
    torch, model = load_official_model(context)
    generator = np.random.default_rng(DIRECT_TRAINING_SEED)
    hazy = generator.uniform(0.05, 0.95, size=(384, 512, 3)).astype(np.float32)
    clear = np.clip(
        hazy + generator.normal(scale=0.03, size=hazy.shape), 0.0, 1.0,
    ).astype(np.float32)
    ...
    record = {
        ...
        **{
            key: extracted[key]
            for key in (
                "features", "shape_features", "shape_areas",
                "disagreement", "utility", "harm", "areas",
            )
        },
    }
    parameter_counts = {}
    ...
    for arm in (*DIRECT_ARMS, DIRECT_ABLATION_ARM):
        controller, normalization, history = train_direct_controller(
            torch,
            [record],
            arm=arm,
            seed=DIRECT_TRAINING_SEED,
            device=context.device,
            epochs=2,
        )
        completed_iterations += 2
        prediction = predict_direct(
            torch, controller, record, normalization, context.device,
        )
        actions = direct_actions(prediction)
        evaluation = evaluate_direct_actions(
            torch, record, hazy, clear, extracted["prediction"], actions,
            scene="synthetic", fold=0, full_metrics=True,
            device=context.device,
        )
        completed_iterations += 1
        parameter_counts[arm] = sum(
            parameter.numel() for parameter in controller.parameters()
        )
        finite.append(
            all(math.isfinite(float(value)) for value in (
                history[0]["loss"], history[-1]["loss"],
                evaluation["gain"], evaluation["ssim_gain"],
                evaluation["color_bias_delta"],
            ))
        )
        write_contract_progress(
            context,
            completed_iterations=completed_iterations,
            total_iterations=DIRECT_PROBE_COST_ITERATIONS,
            stage=f"synthetic_direct_controller_{arm}",
        )
    matched_ratio = abs(
        parameter_counts["joint"] - parameter_counts["separate"]
    ) / max(parameter_counts["joint"], parameter_counts["separate"])
    checks = {
        "entrypoint_identity": (
            context.assets["controller_entrypoint"].sha256
        ...
        "official_graph_strict_loaded": (
            sum(parameter.numel() for parameter in model.parameters())
            == PARAMETER_COUNT
        ),
        "base_feature_shape": extracted["features"].shape[0] == FEATURE_CHANNELS,
        "shape_feature_shape": (
            extracted["shape_features"].shape[0] == SHAPE_CHANNELS
        ),
        "disagreement_feature_shape": (
            extracted["disagreement"].shape[0] == DISAGREEMENT_CHANNELS
        ),
        "three_action_target": direct_numpy_target(record)[2].shape[0] == 3,
        "all_arms_finite": all(finite),
        "joint_separate_parameter_match": matched_ratio <= 0.01,
        "probe_iteration_count": (
            completed_iterations == DIRECT_PROBE_COST_ITERATIONS
        ),
        "fixed_cost_contract": (
            context.engineering_contract["cost_contract"]["formal_iterations"]
            == DIRECT_FORMAL_COST_ITERATIONS
            and context.engineering_contract["cost_contract"]["probe_iterations"]
            == DIRECT_PROBE_COST_ITERATIONS
        ),
        "scientific_assets_absent": all(
            identifier not in context.assets
            for identifier in (
                "haze4k_train", "parent_run_root", "candidate_confirmation",
...
def run(context_path: Path) -> None:
    ...
    if (
        context.device != "cuda"
        or context.total_units != DIRECT_TOTAL_UNITS
        or context.evidence_role != "development_screening"
        or any(context.protected_data_permissions.values())
    ):
        raise RuntimeError("runtime device, role, unit, or permission contract changed")
    forbidden = {"candidate_confirmation", "nh_haze", "reside_its", "reside_ots"}
    if forbidden & set(context.assets):
        raise RuntimeError("forbidden scientific asset was delivered")
    if load_completed_unit_ledger(context):
        raise RuntimeError("fresh direct-controller route cannot preload units")
    import torch

    parent_root = asset_path(context, "parent_run_root", kind="directory")
    train_root = asset_path(context, "haze4k_train", kind="directory")
    parent_closeout_path = asset_path(context, "parent_closeout", kind="file")
    parent_summary_path = asset_path(context, "parent_summary", kind="file")
    parent_inventory_path = asset_path(
        context, "parent_raw_inventory", kind="file",
    )
    parent_closeout = read_json(parent_closeout_path)
    parent_summary = read_json(parent_summary_path)
    parent_inventory = read_json(parent_inventory_path)
    write_workload_progress(context, completed_units=0, stage="parent_identity")

    scene_cache_identity = verify_parent_cache_class(
        parent_root,
        "workload/scene_cache",
        expected_count=DIRECT_PARENT_SCENE_CACHE_COUNT,
        expected_bytes=DIRECT_PARENT_SCENE_CACHE_BYTES,
        expected_digest=DIRECT_PARENT_SCENE_CACHE_DIGEST,
    )
    test_cache_identity = verify_parent_cache_class(
        parent_root,
        "workload/test_development_cache",
        expected_count=DIRECT_PARENT_TEST_CACHE_COUNT,
        expected_bytes=DIRECT_PARENT_TEST_CACHE_BYTES,
        expected_digest=DIRECT_PARENT_TEST_CACHE_DIGEST,
    )
    train_groups, training_scenes, calibration_scenes, assignment = (
        enumerate_train_groups(train_root)
    )
    identity_checks = {
        "parent_closeout_sha256": (
            sha256_file(parent_closeout_path) == DIRECT_PARENT_CLOSEOUT_SHA256
        ),
        "parent_summary_sha256": (
            sha256_file(parent_summary_path) == DIRECT_PARENT_SUMMARY_SHA256
        ),
        "parent_inventory_sha256": (
            sha256_file(parent_inventory_path)
            == DIRECT_PARENT_RAW_INVENTORY_SHA256
        ),
        "parent_terminal": (
            parent_closeout.get("route_id") == PARENT_ROUTE_ID
            and parent_closeout.get("route_commit") == PARENT_ROUTE_COMMIT
            and parent_closeout.get("state") == "COMPLETED_GATE_FAIL"
            and parent_closeout.get("authorizes")
            == "NEW_ACTION_FAMILY_OR_END_TO_END_CONTROLLER_CONTRACT_AUTHORING_ONLY"
        ),
        "parent_selected_zero_action": (
            parent_summary.get("selected_oof", {}).get(
                "action_area", {},
            ).get("estimate") == 0.0
        ),
        "parent_scene_cache_inventory": scene_cache_identity["valid"],
        "parent_test_cache_inventory": test_cache_identity["valid"],
        "parent_inventory_class_match": (
            parent_inventory.get("classes", {}).get("scene_cache", {}).get(
                "inventory_sha256"
            ) == DIRECT_PARENT_SCENE_CACHE_DIGEST
            and parent_inventory.get("classes", {}).get(
                "test_scene_cache", {},
            ).get("inventory_sha256") == DIRECT_PARENT_TEST_CACHE_DIGEST
        ),
        "train_assignment_digest": assignment == TRAIN_ASSIGNMENT_DIGEST,
        "train_role_counts": (
            len(training_scenes) == TRAINING_SCENES
        ...
        "train_roles_disjoint": not (
            set(training_scenes) & set(calibration_scenes)
        ),
        "protected_assets_absent": not (forbidden & set(context.assets)),
    }
    if not all(identity_checks.values()):
        raise RuntimeError(
            "parent/data identity failed before scientific training: "
            + json.dumps(identity_checks, sort_keys=True)
        )

    cache_root = parent_root / "workload/scene_cache"
    records = {}
    training_set = set(training_scenes)
    for scene in training_scenes + calibration_scenes:
        role = "training" if scene in training_set else "calibration"
        path = cache_root / f"{role}_{scene[:24]}.npz"
        record = load_cache(path)
        records[scene] = {"scene": scene, "cache_path": path, **record}
    feasibility = direct_feasibility(
        [records[scene] for scene in calibration_scenes],
        parent_root / "workload/test_development_cache",
    )
    feasibility_path = output_file(
        context, "direct_controller_target_feasibility.json",
    )
    atomic_json(feasibility_path, {
        **feasibility,
        "identity_checks": identity_checks,
        "scene_cache_identity": scene_cache_identity,
        "test_cache_identity": test_cache_identity,
    })

    checkpoint_root = output_file(context, "controller_checkpoints")
    prediction_root = output_file(context, "raw_predictions")
    receipt_root = output_file(context, "unit_receipts")
    checkpoint_root.mkdir()
    prediction_root.mkdir()
    receipt_root.mkdir()
    inventory_items: list[dict[str, Any]] = []
    add_inventory_item(
        inventory_items, context, feasibility_path, "feasibility_receipt",
    )
    record_completed_unit(
        context,
        unit_id="target_feasibility",
        input_sha256=sha256_text("|".join([
            DIRECT_PARENT_SCENE_CACHE_DIGEST,
            DIRECT_PARENT_TEST_CACHE_DIGEST,
            assignment,
        ])),
        output_relpath="direct_controller_target_feasibility.json",
    )
    completed_units = 1
    write_workload_progress(
        context, completed_units=completed_units,
        stage="target_feasibility_complete",
    )

    fold_order = sorted(
        training_scenes,
        key=lambda scene: (
            sha256_text(f"{DIRECT_FOLD_SALT}|{scene}"), scene,
        ),
    )
    fold_assignment = {
        scene: index % OOF_FOLDS for index, scene in enumerate(fold_order)
    }
    if Counter(fold_assignment.values()) != {
        fold: TRAINING_SCENES // OOF_FOLDS for fold in range(OOF_FOLDS)
    }:
        raise RuntimeError("direct-controller OOF fold balance changed")
    ...
    for fold in range(OOF_FOLDS):
        evaluation_scenes = [
            scene for scene in training_scenes
            if fold_assignment[scene] == fold
        ]
        outer_training = [
            scene for scene in training_scenes
        ...
        nested_order = sorted(
            outer_training,
            key=lambda scene: (
                sha256_text(f"{DIRECT_SUBSET_SALT}|{fold}|{scene}"), scene,
            ),
        )
        payloads = []
        for scene in evaluation_scenes:
            hazy_path, clear_path = choose_one_variant(
                scene, train_groups[scene],
            )
            cache = load_cache(records[scene]["cache_path"], include_prediction=True)
            payloads.append({
                "scene": scene,
        ...
        schedules = [
            ...
            for train_size in DIRECT_TRAIN_SIZES
        ] + [(DIRECT_ABLATION_ARM, DIRECT_TRAIN_SIZES[-1])]
        for arm, train_size in schedules:
            subset = nested_order[:train_size]
            subset_digest = digest_lines(subset)
            train_records = [records[scene] for scene in subset]
            bundles = []
            for seed_index, offset in enumerate(SEED_OFFSETS):
                seed = DIRECT_TRAINING_SEED + fold + offset

                def progress(epoch, *, arm=arm, train_size=train_size,
                             fold=fold, seed_index=seed_index):
                    if epoch == DIRECT_EPOCHS:
                        write_workload_progress(
                            context,
                            completed_units=completed_units,
                            stage=(
                                f"trained_{arm}_{train_size}_"
                                f"fold_{fold}_seed_{seed_index}"
                            ),
                        )
                ...
                model_id = (
                    f"{arm}_n{train_size}_fold_{fold}_seed_{seed_index}"
                )
                checkpoint_path = checkpoint_root / f"{model_id}.pt"
                torch.save({
                    "schema_version": 1,
                ...
                add_inventory_item(
                    inventory_items, context, checkpoint_path,
                    "head_checkpoint",
                )
                seed_rows = []
                for payload in payloads:
                    prediction = predict_direct(
                        torch, controller, payload["record"],
                        normalization, context.device,
                    )
                    actions = direct_actions(prediction)
                    row = evaluate_direct_actions(
                        torch,
                        payload["record"],
                        payload["hazy"],
                        payload["clear"],
                ...
                raw_prediction_path = prediction_root / f"{model_id}.npz"
                np.savez(
                    raw_prediction_path,
                    scene=np.asarray(
                        [row["scene"] for row in seed_rows], dtype="U64",
                    ),
                    gain=np.asarray(
                        [row["gain"] for row in seed_rows], dtype=np.float64,
                    ),
                    action_area=np.asarray(
                        [row["action_area"] for row in seed_rows],
                        dtype=np.float64,
                    ),
                    selected_harm_fraction=np.asarray(
                        [row["selected_harm_fraction"] for row in seed_rows],
                        dtype=np.float64,
                    ),
                    active_scene=np.asarray(
                        [row["active_scene"] for row in seed_rows],
                        dtype=np.int8,
                    ),
                    severe_harm_scene=np.asarray(
                        [row["severe_harm_scene"] for row in seed_rows],
                        dtype=np.int8,
                    ),
                    keep_fraction=np.asarray(
                        [row["keep_fraction"] for row in seed_rows],
                        dtype=np.float64,
                    ),
                    weaken_fraction=np.asarray(
                        [row["weaken_fraction"] for row in seed_rows],
                        dtype=np.float64,
                    ),
                    strengthen_fraction=np.asarray(
                        [row["strengthen_fraction"] for row in seed_rows],
                        dtype=np.float64,
                    ),
                )
                add_inventory_item(
                    inventory_items, context, raw_prediction_path,
                    "unit_prediction",
                )
                aggregate = {
                    "mean_gain": float(np.mean([
                        row["gain"] for row in seed_rows
                    ])),
                    "cvar10_gain": lower_tail_mean(
                        row["gain"] for row in seed_rows
                    ),
                    "action_area": float(np.mean([
                        row["action_area"] for row in seed_rows
                    ])),
                    "active_scene_prevalence": float(np.mean([
                        row["active_scene"] for row in seed_rows
                ...
                receipt_path = receipt_root / f"{model_id}.json"
                atomic_json(receipt_path, {
                    ...
                    "subset_sha256": subset_digest,
                    "checkpoint_sha256": sha256_file(checkpoint_path),
                    "prediction_sha256": sha256_file(raw_prediction_path),
                    "evaluation_scene_count": len(seed_rows),
                    "aggregate": aggregate,
                })
                add_inventory_item(
                    inventory_items, context, receipt_path, "unit_receipt",
                )
                training_rows.append({
                    "model_id": model_id,
                    "arm": arm,
                    ...
                    "first_epoch_loss": history[0]["loss"],
                    "final_epoch_loss": history[-1]["loss"],
                    "final_policy_ce": history[-1]["policy_ce"],
                    "final_expected_utility": history[-1][
                        "expected_utility"
                    ],
                    "final_tail": history[-1]["tail"],
                    **aggregate,
                    "checkpoint_sha256": sha256_file(checkpoint_path),
                    "prediction_sha256": sha256_file(raw_prediction_path),
                })
                record_completed_unit(
                    context,
                    unit_id=f"train_{model_id}",
                    input_sha256=sha256_text("|".join([
                        arm, str(train_size), str(fold), str(seed),
                        subset_digest, DIRECT_PARENT_SCENE_CACHE_DIGEST,
                    ])),
                    output_relpath=f"unit_receipts/{model_id}.json",
                )
                completed_units += 1
                write_workload_progress(
                    context,
                    completed_units=completed_units,
                    stage=f"completed_{model_id}",
                )
                bundles.append({
            ...
            for payload in payloads:
                prediction = predict_direct_ensemble(
                    torch, bundles, payload["record"], context.device,
                )
                actions = direct_actions(prediction)
                row = evaluate_direct_actions(
                    torch,
                    ...
                    scene=payload["scene"],
                    fold=fold,
                    full_metrics=(train_size == DIRECT_TRAIN_SIZES[-1]),
                    device=context.device,
                )
    ...
    if completed_units != DIRECT_TRAINING_UNITS + 1:
        raise RuntimeError("direct-controller training-unit count changed")
    prefinal_ledger = load_completed_unit_ledger(context)
    primary_rows = [
        row for row in ensemble_rows
        if row["arm"] == "joint"
        and row["train_size"] == DIRECT_TRAIN_SIZES[-1]
    ]
    separate_rows = [
        row for row in ensemble_rows
        if row["arm"] == "separate"
    ...
    frozen_rows = [
        row for row in ensemble_rows
        if row["arm"] == DIRECT_ABLATION_ARM
    ]
    if not all(len(rows) == TRAINING_SCENES for rows in (
        primary_rows, separate_rows, frozen_rows,
    )):
        raise RuntimeError("100-percent OOF arm coverage changed")

    primary_intervals = stratified_bootstrap_family(
        primary_rows,
        ["gain", "action_area"],
        family_size=DIRECT_PRIMARY_FAMILY_SIZE,
        seed=DIRECT_BOOTSTRAP_SEED,
    )
    selected_harm = bootstrap_ratio_interval(
        primary_rows,
        "selected_harm_area",
        "selected_area",
        family_size=DIRECT_PRIMARY_FAMILY_SIZE,
        seed=DIRECT_BOOTSTRAP_SEED + 1,
    )
    active_scene = wilson(
        sum(row["active_scene"] for row in primary_rows),
        len(primary_rows),
        family_size=DIRECT_PRIMARY_FAMILY_SIZE,
    )
    material_scene = wilson(
        sum(row["material_scene"] for row in primary_rows),
        len(primary_rows),
    )
    image_safety = {
        "psnr": wilson(
            sum(row["gain"] <= -PSNR_HARM_MARGIN_DB for row in primary_rows),
            len(primary_rows),
            family_size=4,
        ),
        "ssim": wilson(
            sum(row["ssim_gain"] <= -SSIM_HARM_MARGIN for row in primary_rows),
            len(primary_rows),
            family_size=4,
        ),
        "color": wilson(
            sum(
                row["color_bias_delta"] >= COLOR_HARM_MARGIN
    ...
    by_identity = lambda rows: {row["scene"]: row for row in rows}
    joint_75 = by_identity([
        row for row in ensemble_rows
        if row["arm"] == "joint" and row["train_size"] == 360
    ])
    joint_100 = by_identity(primary_rows)
    slope_rows = [{
        "scene": scene,
        "fold": joint_100[scene]["fold"],
        "gain_difference": (
            joint_100[scene]["gain"] - joint_75[scene]["gain"]
        ),
    } for scene in sorted(joint_100)]
    slope_interval = stratified_bootstrap_family(
        slope_rows,
        ["gain_difference"],
        family_size=1,
        seed=DIRECT_BOOTSTRAP_SEED + 2,
    )["gain_difference"]
    separate_by_scene = by_identity(separate_rows)
    frozen_by_scene = by_identity(frozen_rows)
    comparison_rows = [{
        "scene": scene,
        "fold": joint_100[scene]["fold"],
        "joint_minus_separate": (
            joint_100[scene]["gain"] - separate_by_scene[scene]["gain"]
        ),
        "joint_minus_frozen": (
            joint_100[scene]["gain"] - frozen_by_scene[scene]["gain"]
        ),
    } for scene in sorted(joint_100)]
    matched_comparisons = stratified_bootstrap_family(
        comparison_rows,
        ["joint_minus_separate", "joint_minus_frozen"],
        family_size=2,
        seed=DIRECT_BOOTSTRAP_SEED + 3,
    )
    learning_curve = direct_interval_rows(ensemble_rows)

    keep_psnr = np.asarray(
        [row["keep_psnr"] for row in primary_rows], dtype=np.float64,
    )
    quartiles = np.quantile(keep_psnr, (0.25, 0.50, 0.75))
    subgroup_rows = []
    for index in range(4):
        ...
        rows = [
            ...
            if lower < row["keep_psnr"] <= upper
        ]
        subgroup_rows.append({
            "subgroup": f"keep_psnr_q{index + 1}",
            "scene_count": len(rows),
            "keep_psnr_lower": None if not math.isfinite(lower) else lower,
            "keep_psnr_upper": None if not math.isfinite(upper) else upper,
            ...
            "action_area": float(np.mean([
                row["action_area"] for row in rows
            ])),
            "selected_harm_ratio": (
                sum(row["selected_harm_area"] for row in rows)
                / max(sum(row["selected_area"] for row in rows), EPSILON)
            ),
        })

    coverage_checks = {
        "training_units": len(training_rows) == DIRECT_TRAINING_UNITS,
        "completed_unit_ledger_prefinal": (
            len(prefinal_ledger) == DIRECT_TOTAL_UNITS - 1
        ),
        "nested_subsets": all(
            len({
                row["subset_sha256"] for row in training_rows
                if row["fold"] == fold
                and row["arm"] == arm
                and row["train_size"] == train_size
            }) == 1
            for fold in range(OOF_FOLDS)
            for arm in (*DIRECT_ARMS, DIRECT_ABLATION_ARM)
            for train_size in (
                DIRECT_TRAIN_SIZES
                if arm in DIRECT_ARMS else (DIRECT_TRAIN_SIZES[-1],)
            )
        ),
        "five_fold_three_seed_grid": all(
            len([
        ...
        "ensemble_oof_grid": len(ensemble_rows) == (
            len(DIRECT_ARMS) * len(DIRECT_TRAIN_SIZES) * TRAINING_SCENES
            + TRAINING_SCENES
        ),
        "finite_training": all(
            math.isfinite(float(row["final_epoch_loss"]))
            and math.isfinite(float(row["mean_gain"]))
            for row in training_rows
        ),
    }
    identity_pass = all(identity_checks.values())
    coverage_pass = all(coverage_checks.values())
    target_outcome = "favorable" if feasibility["favorable"] else "unfavorable"
    gain_interval = primary_intervals["gain"]
    utility_outcome = (
        "favorable" if gain_interval["lower"] > UTILITY_MARGIN_DB
        else "unfavorable" if gain_interval["upper"] <= UTILITY_MARGIN_DB
        else "indeterminate"
    )
    action_interval = primary_intervals["action_area"]
    activation_outcome = (
        "favorable"
        if (
            action_interval["lower"] >= MIN_ACTION_AREA_FRACTION
            and active_scene["lower"] >= MIN_ACTIVE_SCENE_PREVALENCE
        )
        else "unfavorable"
        if (
            action_interval["upper"] < MIN_ACTION_AREA_FRACTION
            or active_scene["upper"] < MIN_ACTIVE_SCENE_PREVALENCE
        )
        else "indeterminate"
    )
    safety_intervals = [selected_harm, *image_safety.values()]
    safety_outcome = (
        "safe"
        if all(
            interval["upper"] is not None
            and interval["upper"] <= MAX_HARM_PREVALENCE
    ...
    precision_half_width = primary_intervals["gain"]["max_half_width"]
    gate_outcomes = {
        "evidence_identity": "pass" if identity_pass else "fail",
        "coverage": "pass" if coverage_pass else "fail",
        "target_feasibility": target_outcome,
        "actionable_utility": utility_outcome,
        "activation": activation_outcome,
        "tail_safety": safety_outcome,
        "precision": (
            "met" if precision_half_width <= PRECISION_TARGET_DB else "unmet"
        ),
        "data_scale_diagnostic": (
            "favorable" if slope_interval["lower"] > 0.0
            else "unfavorable" if slope_interval["upper"] <= 0.0
            else "indeterminate"
        ),
    }
    ...
    summary = {
        ...
        "operation_id": OPERATION_ID,
        "run_id": context.run_id,
        "scope": "development-only direct selective-controller learning curve",
        "independent_unit": "original_clear_scene",
        "data_roles": {
            "oof_scenes": TRAINING_SCENES,
            "target_feasibility_scenes": CALIBRATION_SCENES,
            "variant_stability_scenes": EXPECTED_TEST_SCENES,
            "candidate_confirmation_touched": False,
            "nh_haze_touched": False,
            "reside_its_ots_touched": False,
        },
        "design": {
            "arms": {
                "joint": (
                    "joint observable encoder and three-action policy optimized "
                    "with utility and tail harm in one decision objective"
                ),
                "separate": (
                    "parameter-matched utility/risk-head control with explicit "
                    "risk eligibility"
                ),
                "frozen": (
                    "100-percent-data base-feature direct-policy ablation"
                ),
            },
            "train_sizes": list(DIRECT_TRAIN_SIZES),
            "folds": OOF_FOLDS,
            "seeds": len(SEED_OFFSETS),
            "epochs": DIRECT_EPOCHS,
            "training_units": DIRECT_TRAINING_UNITS,
        },
        "identity_checks": identity_checks,
        ...
        "primary": {
            "gain": primary_intervals["gain"],
            "action_area": primary_intervals["action_area"],
            "active_scene": active_scene,
            "material_scene": material_scene,
            "selected_area_harm": selected_harm,
            "image_safety": image_safety,
            "cvar10_gain": lower_tail_mean(
                row["gain"] for row in primary_rows
            ),
            "action_distribution": {
                action: float(np.mean([
                    row[f"{action}_fraction"] for row in primary_rows
                ]))
                for action in ("keep", "weaken", "strengthen")
            },
        },
        "data_scale": {
            "paired_75_to_100_gain_difference": slope_interval,
            "interpretation": (
                "positive lower bound supports data limitation; nonpositive "
                "upper bound supports structural saturation; otherwise unresolved"
            ),
        },
        "matched_controls": matched_comparisons,
        "subgroup_tails": subgroup_rows,
        "seed_fold_stability": {
            "unit_gain_min": float(min(
                row["mean_gain"] for row in training_rows
            )),
            "unit_gain_max": float(max(
                row["mean_gain"] for row in training_rows
            )),
            "unit_gain_std": float(np.std([
                row["mean_gain"] for row in training_rows
            ], ddof=1)),
        },
        "precision": {
            "target_half_width_db": PRECISION_TARGET_DB,
            "observed_half_width_db": precision_half_width,
            "met": precision_half_width <= PRECISION_TARGET_DB,
        },
        "gate_outcomes": gate_outcomes,
        "raw_artifact_inventory": raw_inventory,
        "limitations": [
            "All outcomes are development-screening; candidate-confirmation, canary, locked-test, NH-Haze, ITS, and OTS were not read.",
            "Only 600 original-clear scenes are independent primary units; tiles, actions, folds, seeds, and variants never increase n.",
            "The 150 calibration-role scenes are used only for the frozen pre-training target-feasibility certificate.",
            "The 100 historical test-development scenes are used only for four-variant target-stability diagnostics.",
            "Raw parent caches, new checkpoints, and unit predictions remain cloud-only and are bound by SHA inventories.",
        ],
        "marker": "HAZE4K_DIRECT_SELECTIVE_CONTROLLER_LEARNING_CURVE_COMPLETE",
    }

    paths = {
        "summary": output_file(context, "direct_controller_summary.json"),
        "gate": output_file(context, "direct_controller_gate_summary.json"),
        "feasibility": feasibility_path,
        "curve": output_file(context, "direct_controller_learning_curve.csv"),
        "training": output_file(context, "direct_controller_training_diagnostics.csv"),
        "oof": output_file(context, "direct_controller_oof_scene_metrics.csv"),
        "subgroups": output_file(context, "direct_controller_subgroup_tails.csv"),
        "intervals": output_file(context, "direct_controller_intervals.json"),
        "inventory": output_file(context, "direct_controller_raw_inventory.json"),
    }
    atomic_json(paths["summary"], summary)
    atomic_json(paths["gate"], {
        "schema_version": 1,
        ...
        "data_scale": summary["data_scale"],
        "matched_controls": matched_comparisons,
    })
    write_csv(paths["curve"], learning_curve)
    write_csv(paths["training"], training_rows)
    write_csv(paths["oof"], [
        row for row in ensemble_rows
        if row["train_size"] == DIRECT_TRAIN_SIZES[-1]
    ])
    write_csv(paths["subgroups"], subgroup_rows)
    atomic_json(paths["intervals"], {
        "schema_version": 1,
        "primary_intervals": primary_intervals,
        "selected_area_harm": selected_harm,
        "active_scene": active_scene,
        ...
        "image_safety": image_safety,
        "learning_curve": learning_curve,
        "slope_75_to_100": slope_interval,
        "matched_controls": matched_comparisons,
    })
    atomic_json(paths["inventory"], raw_inventory)
    evidence_sha = {
    ...
    record_completed_unit(
        ...
        input_sha256=sha256_text("|".join([
            raw_inventory["inventory_sha256"],
            digest_lines(
                f"{key}:{value}" for key, value in evidence_sha.items()
            ),
        ])),
        output_relpath="finalization_unit.json",
    )
    if len(load_completed_unit_ledger(context)) != DIRECT_TOTAL_UNITS:
        raise RuntimeError("direct-controller completed-unit ledger is incomplete")
    write_workload_progress(
        context,
        completed_units=DIRECT_TOTAL_UNITS,
        stage="aggregate_gate_finalization",
    )
    write_gate_result(
        context,
        ...
        details={
            ...
            "variant_stability_scenes": EXPECTED_TEST_SCENES,
            "training_units": DIRECT_TRAINING_UNITS,
            "completed_unit_ledger_count": DIRECT_TOTAL_UNITS,
            "candidate_confirmation_touched": False,
            "primary_gain_db": primary_intervals["gain"]["estimate"],
            "primary_gain_lower_db": primary_intervals["gain"]["lower"],
            "primary_gain_upper_db": primary_intervals["gain"]["upper"],
            "summary_file": paths["summary"].name,
            "gate_summary_file": paths["gate"].name,
            "learning_curve_file": paths["curve"].name,
            "training_diagnostics_file": paths["training"].name,
            "oof_scene_metrics_file": paths["oof"].name,
            "subgroup_tails_file": paths["subgroups"].name,
            "intervals_file": paths["intervals"].name,
            "raw_inventory_file": paths["inventory"].name,
        },
    )
