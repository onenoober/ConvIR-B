...
REMOTE_REPO=${REMOTE_REPO:-/sda/home/wangyuxin/ConvIR-B/repos/ConvIR-B-v4a-a1r-representation-sufficiency-20260714}
...
A1F_ROOT=${A1F_ROOT:-$BASE/repos/ConvIR-B-v4a-a1f-deltau-action-feasibility-20260714}
...
EXPECTED_ROUTE_COMMIT=${EXPECTED_ROUTE_COMMIT:?set the exact A1R route commit}
EXPECTED_ROUTE_CARD_SHA256=${EXPECTED_ROUTE_CARD_SHA256:?set the validated A1R route-card SHA-256}
EXPECTED_A1F_ROOT_COMMIT=d4f8d0936869c822ae19b5c21172efc2eb973dd8
EXPECTED_A1F_R3_REVIEW_SHA256=a8b9064308710ac5fc890b9de0158c1faddb4d51f7d298d4991e9ddfb3616e1d
...
RUNNER=$REMOTE_REPO/experience_docx/tools/chd_rm_v4a_a1r_representation_sufficiency.py
A1F_EVID=$A1F_ROOT/experience_docx/experiment_logs/haze4k_v5_chd_rm_v4a_a1f_deltau_action_feasibility_20260714
A1F_R3_REVIEW=$A1F_EVID/v4a_a1f_r3_review.json
...
test "$(git -C "$REMOTE_REPO" branch --show-current)" = "codex/haze4k-v5-v4a-a1r-representation-sufficiency-20260714"
test "$(git -C "$REMOTE_REPO" rev-parse HEAD)" = "$EXPECTED_ROUTE_COMMIT"
test -z "$(git -C "$REMOTE_REPO" status --porcelain)"
test "$(git -C "$A1F_ROOT" rev-parse HEAD)" = "$EXPECTED_A1F_ROOT_COMMIT"
...
test -x "$PY"
test -f "$RUNNER"
test "$(sha256sum "$A1F_R3_REVIEW" | awk '{print $1}')" = "$EXPECTED_A1F_R3_REVIEW_SHA256"
...
for key, item in expected.items():
    if value.get(key) != item:
        raise SystemExit(f"A1F authorization mismatch: {key}={value.get(key)!r}")
print("V4A_A1R_PARENT_AUTHORIZATION_OK")
PY

if [ "$STAGE" = formal ]; then
  SMOKE_CLOSEOUT=${SMOKE_CLOSEOUT:?set the passed A1R smoke closeout}
...
expected = {
    ...
    "decision": "V4A_A1R_S0_INTEGRITY_PASS_AUTHORIZE_FORMAL_ONLY",
    "authorizes": "A1R_FORMAL_ONLY",
}
for key, item in expected.items():
    if value.get(key) != item:
        raise SystemExit(f"A1R smoke authorization mismatch: {key}={value.get(key)!r}")
print("V4A_A1R_SMOKE_AUTHORIZATION_OK")
PY
fi
...
LOG=$RUN_ROOT/v4a_a1r_${STAGE}_${STAMP}.log
echo "stage_start route=$ROUTE_ID stage=v4a-A1R-$STAGE run=$RUN_ID time=$(date --iso-8601=seconds)" | tee -a "$STATUS"
...
CUDA_VISIBLE_DEVICES="$GPU" PYTHONUNBUFFERED=1 "$PY" "$RUNNER" audit \
  --a1f-root "$A1F_ROOT" --expected-a1f-root-commit "$EXPECTED_A1F_ROOT_COMMIT" \
  --v3z-root "$V3Z_ROOT" --a1f-r3-review "$A1F_R3_REVIEW" \
  --expected-a1f-r3-review-sha256 "$EXPECTED_A1F_R3_REVIEW_SHA256" \
  --a0r-trace-dir "$A0R_TRACE" --a1r-stage "$STAGE" \
  --expected-route-commit "$EXPECTED_ROUTE_COMMIT" \
  --expected-route-card-sha256 "$EXPECTED_ROUTE_CARD_SHA256" --status-file "$STATUS" \
  --fresh-start-index 256 --fresh-count 512 --outer-folds 4 \
  ...
  --mode projected --output_dir "$OUT" --run_tag "$RUN_ID" "${COMMON_ARGS[@]}" \
  2>&1 | tee -a "$LOG"
rc=${PIPESTATUS[0]}
set -e

echo "stage_done route=$ROUTE_ID stage=v4a-A1R-$STAGE run=$RUN_ID rc=$rc time=$(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -ne 0 ]; then
  echo "V4A_A1R_${STAGE^^}_FAILED_RUNTIME_REQUIRES_CLASSIFICATION" | tee -a "$STATUS"
  exit "$rc"
fi

cp "$OUT/v4a_a1r_closeout.json" "$EVID_STAGE/v4a_a1r_${STAGE}_closeout.json"
cp "$OUT/v4a_a1r_source_manifest.json" "$EVID_STAGE/v4a_a1r_${STAGE}_source_manifest.json"
cp "$OUT/v4a_a1r_probe_state_manifest.json" "$EVID_STAGE/v4a_a1r_${STAGE}_probe_state_manifest.json"
cp "$OUT/v4a_a1r_fold_history.csv" "$EVID_STAGE/v4a_a1r_${STAGE}_fold_history.csv"
cp "$OUT/v4a_a1r_cell_operator_summary.csv" "$EVID_STAGE/v4a_a1r_${STAGE}_cell_operator_summary.csv"
cp "$OUT/v4a_a1r_bootstrap_summary.json" "$EVID_STAGE/v4a_a1r_${STAGE}_bootstrap_summary.json"
if grep -q '"state": "COMPLETED_GATE_PASS"' "$OUT/v4a_a1r_closeout.json"; then
  echo "V4A_A1R_${STAGE^^}_OK" | tee -a "$STATUS"
else
  echo "V4A_A1R_${STAGE^^}_GATE_FAIL" | tee -a "$STATUS"
fi
