#!/usr/bin/env python3
"""Haze4K v2.36 same-contract WLFBridge audit helpers."""
...
ROUTE_ID = "haze4k_v2_36_same_contract_wlfbridge_s4s6_generator_trainability_20260706"
...
def gate_p0_full600(stats: dict[str, Any]) -> bool:
    return bool(
        stats.get("sample_count") == 600
        and (stats.get("mean") is not None and stats["mean"] >= 0.30)
        ...
        and (
            stats.get("strong_reference_regression_rate") is not None
            and stats["strong_reference_regression_rate"] <= 0.02
        )
    )


def gate_p0b_projection(stats: dict[str, Any]) -> bool:
    return bool(
        stats.get("projection_ratio_vs_teacher") is not None
        and stats["projection_ratio_vs_teacher"] >= 0.50
        and stats.get("free_tensor_mean_delta") is not None
        and stats["free_tensor_mean_delta"] >= 1.00
        and stats.get("p05") is not None
        and stats["p05"] >= 0.10
        and stats.get("cvar5") is not None
        and stats["cvar5"] >= 0.00
        and stats.get("severe") == 0
        and stats.get("strong_reference_regression_rate") == 0
    )


def assign_full600_buckets(rows: list[dict[str, Any]]) -> None:
    sorted_rows = sorted(rows, key=lambda row: float(row["A0_same_context_psnr"]))
    bucket_count = max(1, len(sorted_rows) // 4)
    hard_ids = {id(row) for row in sorted_rows[:bucket_count]}
    easy_ids = {id(row) for row in sorted_rows[-bucket_count:]}
    strong_cut = percentile([float(row["A0_same_context_psnr"]) for row in rows], 75) or 0.0
    for row in rows:
        if id(row) in hard_ids:
            row["hardness_bucket"] = "hard"
        elif id(row) in easy_ids:
            row["hardness_bucket"] = "easy"
        else:
            row["hardness_bucket"] = "mid"
        row["easy_bucket"] = "easy_top25" if id(row) in easy_ids else "not_easy_top25"
...
def run_p0_full600(args: argparse.Namespace) -> None:
    source_rows = read_csv(args.v235_manifest)
    rows: list[dict[str, Any]] = []
    alpha_text = str(args.alpha)
    for rec in source_rows:
        rec_alpha = fnum(rec.get("blend_alpha"))
        if rec_alpha is None or abs(rec_alpha - args.alpha) > 1e-9:
            continue
        a0 = fnum(rec.get("A0_full_PSNR"))
        teacher = fnum(rec.get("blend_full_PSNR"))
        if a0 is None or teacher is None:
            continue
        image_id = rec.get("image_id") or Path(rec.get("image_name", "")).stem
        rows.append({
            "image_id": image_id,
            ...
            "context_contract": "full_image_same_context_a0p5",
            "teacher_alpha": args.alpha,
            "A0_same_context_psnr": a0,
            "teacher_same_context_psnr": teacher,
            "teacher_delta_vs_A0": teacher - a0,
            "input_sha256": rec.get("input_sha256", ""),
            "gt_sha256": rec.get("gt_sha256", ""),
            "A0_full_output_sha256": rec.get("A0_full_output_sha256", ""),
            "blend_output_sha256": rec.get("blend_output_sha256", ""),
            "eligible": teacher - a0 > 0,
            "mask_reason": "positive_same_contract_teacher_delta" if teacher - a0 > 0 else "nonpositive_same_contract_teacher_delta",
            "locked_test_touched": False,
        })
    rows.sort(key=lambda row: str(row["image_id"]))
    if len(rows) != 600:
        print(f"warning_p0_full600_expected_600_rows got={len(rows)} alpha={alpha_text}", flush=True)
    assign_full600_buckets(rows)
    ...
    write_csv(args.out_dir / "v236_p0_full600_same_contract_teacher_per_image.csv", rows, fields)
    fold_rows = [
        {
            "image_id": row["image_id"],
            "image_name": row["image_name"],
            "fold_id": row["fold_id"],
            "hardness_bucket": row["hardness_bucket"],
            "strong_reference_bucket": row["strong_reference_bucket"],
            "eligible": row["eligible"],
            "mask_reason": row["mask_reason"],
        }
        for row in rows
    ]
    write_csv(args.out_dir / "v236_p0_fold_split_manifest.csv", fold_rows)

    stats = summarize_values(rows, "teacher_delta_vs_A0")
    fold_summaries = []
    fold_pass_count = 0
    for fold_id in range(5):
        subset = [row for row in rows if row["fold_id"] == fold_id]
        fold_stats = summarize_values(subset, "teacher_delta_vs_A0")
        fold_pass = bool(
            (fold_stats.get("mean") is not None and fold_stats["mean"] >= 0.30)
            and (fold_stats.get("hard") is not None and fold_stats["hard"] >= 0.50)
            and (fold_stats.get("easy") is not None and fold_stats["easy"] >= -0.03)
            and (fold_stats.get("p05") is not None and fold_stats["p05"] >= -0.05)
            and (fold_stats.get("CVaR5") is not None and fold_stats["CVaR5"] >= -0.10)
            and (fold_stats.get("severe_rate") == 0)
            and (
                fold_stats.get("strong_reference_regression_rate") is not None
                and fold_stats["strong_reference_regression_rate"] <= 0.02
            )
        )
    ...
    cache_sha_coverage = (
        sum(
            ...
            and row.get("gt_sha256")
            and row.get("A0_full_output_sha256")
            and row.get("blend_output_sha256")
        )
        / len(rows)
        if rows
        else 0.0
    )
    gate_pass = gate_p0_full600(stats) and cache_sha_coverage == 1.0 and fold_pass_count == 5
    payload = {
        "route_id": ROUTE_ID,
        "phase": "P0 full-600 same-contract substrate expansion",
        ...
        "gate_pass": gate_pass,
        "decision": "P0_PASS_FULL600_SAME_CONTRACT_TEACHER" if gate_pass else "P0_FAIL_FULL600_SAME_CONTRACT_TEACHER",
        "p1_arch_identity_authorized": gate_pass,
        "p0b_context384_projection_authorized": gate_pass,
        "gate": {
            "image_count": 600,
    ...
    write_json(args.out_dir / "v236_p0_full600_same_contract_teacher_summary.json", payload)
    write_json(
        args.out_dir / "v236_p0_closeout.json",
        {
            "route_id": ROUTE_ID,
            "phase": "P0",
    ...
    print("V236_P0_FULL600_SAME_CONTRACT_TEACHER_OK")
...
def prepare_p4_samples(args: argparse.Namespace) -> list[dict[str, Any]]:
    if getattr(args, "context_size", ""):
        context_size = str(args.context_size)
        alpha = float(args.alpha)
    else:
        if not args.p3_summary:
            raise RuntimeError("Either --p3-summary or --context-size/--alpha is required")
    ...
    with torch.no_grad():
        for rec in p0c_rows:
            ...
            if context_size == "full_image_slice":
                ...
                x_for_model = x_full
                model_crop_top = crop_top
                model_crop_left = crop_left
                context_available = True
            else:
                context_int = int(context_size)
                origin = context_origin(inp.shape[1], inp.shape[2], crop_top, crop_left, args.loss_crop_size, context_int)
                if origin is None:
                    print(f"p4_skip_unavailable_context {context_size} {name}", flush=True)
                    continue
                top, left = origin
                x_ctx = inp[:, top:top + context_int, left:left + context_int].unsqueeze(0).to(device)
                a0_ctx = infer_official(official, x_ctx)
                model_crop_top = crop_top - top
                model_crop_left = crop_left - left
                a0_slice = a0_ctx[
                    :,
                    :,
                    model_crop_top:model_crop_top + args.loss_crop_size,
                    model_crop_left:model_crop_left + args.loss_crop_size,
                ]
                x_for_model = x_ctx
                context_available = True
            x_padded, _, _ = pad_to_factor(x_for_model, 32)
            base_psnr = psnr(a0_slice, y_crop)
            ...
            samples.append({
                ...
                "model_crop_left": model_crop_left,
                "student_context": context_size,
                "baseline_context": context_size,
...
def optimize_projection_sample(
    ...
    crop_top = int(sample.get("model_crop_top", sample["crop_top"]))
    crop_left = int(sample.get("model_crop_left", sample["crop_left"]))
    ...
    return {
        ...
        "crop_top": sample["crop_top"],
        "crop_left": sample["crop_left"],
        "model_crop_top": crop_top,
        "model_crop_left": crop_left,
        "student_context": sample.get("student_context", "full_image_slice"),
        "baseline_context": sample.get("baseline_context", "full_image_slice"),
...
def summarize_p4_projection(point: str, rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "loose_projection_gate_pass": loose_gate_pass,
    }
    payload["gate_pass"] = gate_p0b_projection(payload)
    return payload
...
def run_p4_projection(args: argparse.Namespace) -> None:
    ...
    for point in INSERTION_GROUPS:
        ...
        if device.type == "cuda":
            torch.cuda.empty_cache()
    prefix = getattr(args, "output_prefix", "v236_p4_same_contract_free_tensor_projection")
    write_csv(args.out_dir / f"{prefix}_per_image.csv", per_image_rows)
    write_csv(args.out_dir / f"{prefix}_by_insertion.csv", summary_rows)
    passing = [row for row in summary_rows if row["gate_pass"]]
    best = max(
        summary_rows,
        key=lambda row: float(row["projection_ratio_vs_teacher"] if row["projection_ratio_vs_teacher"] is not None else -999.0),
    )
    context_size = per_image_rows[0]["student_context"] if per_image_rows else ""
    payload = {
        "route_id": ROUTE_ID,
        "phase": getattr(args, "phase_label", "P4 same-contract free-tensor projection"),
        "locked_test_touched": False,
        "student_context": context_size,
        "baseline_context": context_size,
        "teacher_mode": "full_image_output_slice",
    ...
    closeout_name = getattr(args, "closeout_name", "v236_p4_closeout.json")
    write_json(args.out_dir / closeout_name, payload)
    print(json.dumps(payload, indent=2, sort_keys=True))
    print(getattr(args, "ok_marker", "V236_P4_SAME_CONTRACT_FREE_TENSOR_PROJECTION_OK"))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p4.add_argument("--p3-summary", type=Path, default=None)
    p4.add_argument("--context-size", default="")
    p4.add_argument("--alpha", type=float, default=0.5)
    ...
    p0b = sub.add_parser("p0b-projection")
    add_common(p0b)
    p0b.add_argument("--p0c-csv", type=Path, required=True)
    p0b.add_argument("--context-size", default="384")
    p0b.add_argument("--alpha", type=float, default=0.5)
    p0b.add_argument("--data-dir", type=Path, default=Path("/sda/home/wangyuxin/ConvIR-B/datasets/Haze4K/Haze4K"))
    p0b.add_argument("--checkpoint", type=Path, default=Path("/sda/home/wangyuxin/ConvIR-B/checkpoints/official/Haze4K/haze4k-base.pkl"))
    p0b.add_argument("--wdmamba-repo", type=Path, default=Path("/sda/home/wangyuxin/ConvIR-B/repos/external_experts/WDMamba"))
    p0b.add_argument("--wdmamba-checkpoint", type=Path, default=Path("/sda/home/wangyuxin/ConvIR-B/checkpoints/WDMamba_ckpts/haze4k_35.88.pth"))
    p0b.add_argument("--device", default="cuda")
    p0b.add_argument("--loss-crop-size", type=int, default=256)
    p0b.add_argument("--projection-steps", type=int, default=32)
    p0b.add_argument("--learning-rate", type=float, default=0.003)
    p0b.add_argument("--energy-weight", type=float, default=0.0001)
    p0b.add_argument("--grad-clip-norm", type=float, default=0.5)
    p0b.set_defaults(
        output_prefix="v236_p0b_context384_free_tensor_projection",
        closeout_name="v236_p0b_closeout.json",
        phase_label="P0B context384 free-tensor projection",
        ok_marker="V236_P0B_CONTEXT384_FREE_TENSOR_PROJECTION_OK",
    )
    return parser
...
def main() -> None:
    ...
    elif args.phase in {"p4-projection", "p0b-projection"}:
