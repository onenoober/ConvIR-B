#!/usr/bin/env python3
"""Audit DTA checkpoint internals against Haze4K transmission maps."""
...
def audit(args):
    ...
    model = build_dta(args).to(device)
    ...
    with torch.no_grad():
        for idx, data in enumerate(loader):
            ...
            outputs = model(padded, depth, airlight=airlight) if args.arch == "dta_v3" else model(padded, depth)
...
def main():
    ...
    parser.add_argument("--arch", default="dta_v2", choices=["dta_v2", "dta_v3"])
    parser.add_argument("--dta_variant", default="v2", choices=["v2", "v3"])
    ...
    print("DTA_TPRED_QUALITY_AUDIT_OK")
