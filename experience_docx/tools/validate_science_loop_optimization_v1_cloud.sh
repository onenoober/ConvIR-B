...
on_error() {
  rc=$?
  printf 'SCIENCE_LOOP_OPTIMIZATION_V1_CLOUD_FAILED line=%s command=%q rc=%s\n' \
    "$1" "$2" "$rc" >&2
  exit "$rc"
}
trap 'on_error "$LINENO" "$BASH_COMMAND"' ERR

branch=codex/science-loop-optimization-v1
base=5c9e352f608630054df82589a52c616c51d66d59
...
work=$(/usr/bin/mktemp -d /tmp/science-loop-optimization-v1.XXXXXX)

cleanup() {
  case "$work" in
    /tmp/science-loop-optimization-v1.*)
      /bin/rm -rf -- "$work"
      ;;
    *)
...
printf 'SCIENCE_LOOP_OPTIMIZATION_V1_STAGE=checkout\n'
...
printf 'SCIENCE_LOOP_OPTIMIZATION_V1_STAGE=compile\n'
"$python" -m py_compile \
...
printf 'SCIENCE_LOOP_OPTIMIZATION_V1_STAGE=policy_snapshot\n'
...
printf 'SCIENCE_LOOP_OPTIMIZATION_V1_STAGE=focused_regression\n'
"$python" -m unittest -v \
...
printf 'SCIENCE_LOOP_OPTIMIZATION_V1_STAGE=full_control_plane_regression\n'
...
printf 'SCIENCE_LOOP_OPTIMIZATION_V1_STAGE=machine_contract\n'
"$python" - "$work/repo/experience_docx/RULE_COMPATIBILITY.json" <<'PY'
import json
import sys
...
assert review.MAX_JSONRPC_RESPONSE_BYTES == 32 * 1024
assert set(review.TOOLS) == {
...
catalog_properties = review.TOOLS["convir_evidence_catalog_query"][
    "inputSchema"
]["properties"]
assert filters <= set(catalog_properties)
for name, tool in review.TOOLS.items():
    if name != "convir_evidence_catalog_query":
        assert filters.isdisjoint(tool["inputSchema"]["properties"])
...
assert compatibility["compatible_prior_rules_commits"] == [
    "7024906a5441d055ab4dbe8a12311ae9bdfb9766",
    "7866e061b8784d778d5b26fb1fead99dc65cbcc5",
]
PY

printf 'SCIENCE_LOOP_OPTIMIZATION_V1_CLOUD_OK candidate=%s rules_commit=%s tests=%s ops_tools=6 review_tools=6 gpu_access=0 dataset_access=0 protected_data_access=0 experiment_launch=0 historical_evidence_mutation=0\n' \
