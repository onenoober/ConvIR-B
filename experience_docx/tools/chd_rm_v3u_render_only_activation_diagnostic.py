#!/usr/bin/env python3
"""Render-only activation diagnostic for the v3s/v3t Delta-u zero lock."""
...
ROUTE_ID = "haze4k_v5_chd_rm_v3u_render_only_activation_diagnostic_20260713"
...
def delta_for(kind, model, sample, operator):
    if kind != "output":
        raise ValueError(f"v3u only permits output-side activation, got {kind}")
...
def train_cell(args, v3s, legacy, frozen, names, folds, kind, objective, model, device, label):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for start in range(0, len(names), args.risk_window):
            ...
            if objective != "render_only":
                raise ValueError(f"unexpected v3u objective: {objective}")
            total = means["render"]
        ...
        print(json.dumps({"ACTIVATION_PROGRESS": history[-1]}, sort_keys=True), flush=True)
...
def run_noop(args, v3s, legacy, frozen, names, folds, device, output_dir):
    ...
    closeout = {
        "route_id": ROUTE_ID,
        "run_id": args.run_tag,
        "stage": "v3u-S0-output-form-exact-noop",
        "state": "COMPLETED_GATE_PASS" if passed else "COMPLETED_GATE_FAIL",
        "gate_type": "structural_integrity",
        "decision": "V3U_S0_NOOP_PASS_AUTHORIZE_RENDER_ONLY_ACTIVATION_ONLY" if passed else "V3U_S0_NOOP_FAIL_STOP",
        "authorizes": "v3u-S1 output-side render-only fixed32 diagnostic only" if passed else "none",
...
def run_activation(args, v3s, legacy, frozen, names, folds, device, output_dir):
    models = import_v3u_models()
    first = frozen_output_sample(args, v3s, legacy, frozen, names[0], folds[names[0]], device)
    cells = build_cells(models, first, args, device)
    summaries = {}
    ...
    activity_pass = summaries["output_render_only"]["activity_pass"]
    decision = (
        "V3U_S1_RENDER_ONLY_ACTIVATION_PASS_AUTHORIZE_SAFETY_CURRICULUM_DESIGN_ONLY"
        if activity_pass
        else "V3U_S1_RENDER_ONLY_ACTIVATION_FAIL_STOP_DIRECT_LOCAL_RENDERED_FORM"
    )
    closeout = {
        "route_id": ROUTE_ID,
        "run_id": args.run_tag,
        "stage": "v3u-S1-output-side-render-only-activation",
        "state": "COMPLETED_GATE_PASS" if activity_pass else "COMPLETED_GATE_FAIL",
        "gate_type": "mechanism_activation",
        "decision": decision,
        "authorizes": (
            "new safety-curriculum training-contract design only; no training, policy, canary, or locked test"
            if activity_pass
            else "none; direct output-side render-only activation form is stopped"
        ),
        "metric_contract": "32 fixed OOF images, output-side Delta-u, real rendered alpha=.25 MSE only, no repair penalty",
...
def parse_args():
    ...
    parser.add_argument("--mode", required=True, choices=("noop", "activation"))
...
def main():
    ...
    if args.source_split.lower() != "train" or args.sample_count != 32:
        raise ValueError("v3u is fixed to the first 32 train-derived OOF names")
    if args.device == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("v3u requires the authorized CUDA runtime")
    if args.epochs <= 0 or args.risk_window <= 0:
        raise ValueError("invalid fixed activation budget")
    ...
    source = {
        ...
        "objective": {"render_high_mse_weight": 1.0, "repair_weight": 0.0},
        "names": list(names),
        "locked_test_touched": False,
        "canary_touched": False,
    }
    ...
    if args.mode == "noop":
        ...
        closeout = run_activation(args, v3s, legacy, frozen, names, folds, device, output_dir)
