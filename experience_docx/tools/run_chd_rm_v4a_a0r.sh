...
GPU=${GPU:-1}
EXPECTED_ROUTE_COMMIT=${EXPECTED_ROUTE_COMMIT:?set exact v4a route commit}
...
HISTORICAL_CLOSEOUT=$HISTORICAL_ROOT/s1_projected128/v3z_s1_projected128_closeout.json
HISTORICAL_HISTORY=$HISTORICAL_ROOT/s1_projected128/v3z_s1_projected128_history.csv
...
RUNNER=$REMOTE_REPO/experience_docx/tools/chd_rm_v4a_a0r_reconstruct.py
...
test -s "$HISTORICAL_CLOSEOUT"
test -s "$HISTORICAL_HISTORY"
...
COMMON_ARGS=(
  --v3s_root "$V3S_ROOT"
  --expected_v3s_commit "$EXPECTED_V3S_COMMIT"
  --v3p_root "$V3P_ROOT"
  --expected_v3p_commit "$EXPECTED_V3P_COMMIT"
...
set +e
CUDA_VISIBLE_DEVICES="$GPU" PYTHONUNBUFFERED=1 "$PY" "$RUNNER" reconstruct \
  --v3z-root "$V3Z_ROOT" --trace-dir "$OUT/noop_trace" --replicate-id noop \
  --mode noop --output_dir "$OUT/noop" --run_tag v4a_a0r_noop "${COMMON_ARGS[@]}" 2>&1 | tee -a "$LOG"
rc=${PIPESTATUS[0]}
if [ "$rc" -eq 0 ]; then
  CUDA_VISIBLE_DEVICES="$GPU" PYTHONUNBUFFERED=1 "$PY" "$RUNNER" reconstruct \
    --v3z-root "$V3Z_ROOT" --trace-dir "$OUT/r1/trace" --replicate-id r1 \
    --mode projected --output_dir "$OUT/r1" --run_tag v4a_a0r_r1 "${COMMON_ARGS[@]}" 2>&1 | tee -a "$LOG"
  rc=${PIPESTATUS[0]}
fi
if [ "$rc" -eq 0 ]; then
  CUDA_VISIBLE_DEVICES="$GPU" PYTHONUNBUFFERED=1 "$PY" "$RUNNER" reconstruct \
    --v3z-root "$V3Z_ROOT" --trace-dir "$OUT/r2/trace" --replicate-id r2 \
    --mode projected --output_dir "$OUT/r2" --run_tag v4a_a0r_r2 "${COMMON_ARGS[@]}" 2>&1 | tee -a "$LOG"
  rc=${PIPESTATUS[0]}
fi
if [ "$rc" -eq 0 ]; then
  "$PY" "$RUNNER" summarize \
    --r1-dir "$OUT/r1" --r2-dir "$OUT/r2" \
    --noop-closeout "$OUT/noop/v4a_a0r_noop_closeout.json" \
    --historical-closeout "$HISTORICAL_CLOSEOUT" --historical-history "$HISTORICAL_HISTORY" \
...
cp "$OUT/v4a_a0r_reproduction_summary.json" "$EVID_STAGE/"
cp "$OUT/v4a_a0r_closeout.json" "$EVID_STAGE/"
cp "$OUT/r1/v4a_a0r_r1_source_manifest.json" "$EVID_STAGE/"
cp "$OUT/r2/v4a_a0r_r2_source_manifest.json" "$EVID_STAGE/"
cp "$OUT/r1/v4a_a0r_r1_history.csv" "$EVID_STAGE/"
cp "$OUT/r2/v4a_a0r_r2_history.csv" "$EVID_STAGE/"

if grep -q '"state": "COMPLETED_GATE_PASS"' "$OUT/v4a_a0r_closeout.json"; then
  echo V4A_A0R_OK | tee -a "$STATUS"
else
  echo V4A_A0R_GATE_FAIL | tee -a "$STATUS"
fi
