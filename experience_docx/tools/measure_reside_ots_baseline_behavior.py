#!/usr/bin/env python3
"""Measure fixed official ConvIR-B behavior on qualified OTS scenes."""
...
PARENT_CLOSEOUT_SHA256 = "eb4fade9215db551d0fd602a5e0966736815cc0c900a4670fc23f938df5d8932"
PARENT_SCENES_SHA256 = "106d56154311b16af495ae1ccfe5ef960ddbeea2fc534baa7499ce207db566f1"
...
def export_depths(
    ...
    lines.append("disp('OTS_BASELINE_DEPTH_EXPORT_OK');")
    script_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    completed = subprocess.run(
        ["/usr/bin/octave", "--quiet", "--no-gui", str(script_path)],
        text=True, capture_output=True, timeout=timeout, check=False,
    )
    if completed.returncode or "OTS_BASELINE_DEPTH_EXPORT_OK" not in completed.stdout:
    ...
    for _, _, _, target_height, target_width, destination in entries:
        expected_bytes = target_height * target_width * 4
...
def load_official_model(context):
    ...
    if context.assets["official_checkpoint"].sha256 != CHECKPOINT_SHA256:
        raise RuntimeError("official checkpoint identity changed")
    if context.assets["model_source"].sha256 != MODEL_SOURCE_SHA256:
        raise RuntimeError("official model source identity changed")
    if context.assets["model_layers"].sha256 != MODEL_LAYERS_SHA256:
        raise RuntimeError("official model layers identity changed")
...
def contract(context_path: Path) -> None:
    ...
    if context.device != "cuda" or any(context.protected_data_permissions.values()):
        raise RuntimeError("baseline contract requires CUDA and no protected-data permission")
    ...
    prediction_tensor, ssim = infer(torch, model, hazy.astype(np.float32), context.device)
    ...
    details = variant_measurement(hazy, clear, prediction, depth, 0.10, ssim_value)
    ...
    atomic_json(output_file(context, "baseline_contract_details.json"), {
        "parameter_count": PARAMETER_COUNT,
        "fixture": {"batch": 1, "channels": 3, "height": height, "width": width},
        "measurement_keys": sorted(details),
    })
    write_contract_result(
...
def run(context_path: Path) -> None:
    ...
    if context.assets["parent_closeout"].sha256 != PARENT_CLOSEOUT_SHA256 \
        ...
        raise RuntimeError("parent compact-evidence identity changed")
    parent = json.loads(parent_closeout_path.read_text(encoding="utf-8"))
    parent_identity = (
        parent.get("state") == "COMPLETED_GATE_PASS"
        and parent.get("decision") == "OTS_OUTDOOR_MEASUREMENT_PASS"
        and parent.get("authorizes") == "OTS_BASELINE_BEHAVIOR_MEASUREMENT_DESIGN"
        and parent.get("details", {}).get("validation_scenes") == EXPECTED_SCENES
    )
    validation_ids = []
    for line in parent_scenes_path.read_text(encoding="utf-8").splitlines():
        parts = line.strip().split("\t")
        if len(parts) == 2 and parts[0] == "validation":
            validation_ids.append(parts[1])
            ...
            for target_beta in BETAS:
                if abs(beta - target_beta) < 1e-8:
                    if target_beta in selected_haze[scene_id]:
                        raise RuntimeError(f"duplicate selected OTS variant for {scene_id}, beta={target_beta}")
    ...
    if long_edge != 320:
        raise RuntimeError("frozen OTS long-edge setting changed")
    ...
    for scene_index, scene_id in enumerate(validation_ids, start=1):
        try:
            ...
            for beta in BETAS:
                ...
                prediction_tensor, _ = infer(torch, model, hazy, context.device)
    ...
    if not dataset_identity or not coverage_complete:
        state, decision, authorizes = (
            "COMPLETED_INCONCLUSIVE", "OTS_BASELINE_LOCAL_MISMATCH_INCONCLUSIVE",
            "OTS_BASELINE_MEASUREMENT_SUPPLEMENT_ONLY",
        )
        gate_reasons = ["parent, dataset, selected-variant, depth, or complete-scene identity/coverage failed"]
        ...
        gate_reasons = ["tau-aligned mismatch prevalence lower 95 percent bound met the frozen material margin"]
    elif float(mismatch_interval["upper"]) < MISMATCH_PREVALENCE_MARGIN:
        state, decision, authorizes = (
            "COMPLETED_GATE_FAIL", "OTS_BASELINE_LOCAL_MISMATCH_FAIL", "NONE",
        )
        gate_reasons = ["tau-aligned mismatch prevalence upper 95 percent bound was below the frozen material margin"]
    else:
        state, decision, authorizes = (
            "COMPLETED_INCONCLUSIVE", "OTS_BASELINE_LOCAL_MISMATCH_INCONCLUSIVE",
            "OTS_BASELINE_MEASUREMENT_SUPPLEMENT_ONLY",
        )
        gate_reasons = ["tau-aligned mismatch prevalence interval crossed the frozen material margin"]
    ...
    summary = {
        ...
        "scope": "fixed official Haze4K ConvIR-B on development-screening OTS synthetic outdoor scenes",
        "dataset_identity": {
        ...
        "limitations": [
            "This is L1 descriptive development-screening evidence for one fixed model instance on OTS synthetic outdoor optical thickness.",
            "The spatial negative control weakens a content-only explanation but does not identify tau as a causal mechanism.",
            "The official checkpoint was trained for Haze4K, so inadequate global OTS competence makes local behavior uninterpretable rather than a scientific failure.",
            "No candidate module, training, SOTS, Haze4K outcome, NH-HAZE, confirmation, canary, or locked-test evidence was used.",
        ],
        "marker": "RESIDE_OTS_BASELINE_BEHAVIOR_V1_COMPLETE",
    }
    ...
    write_workload_progress(context, completed_units=TOTAL_UNITS, stage="baseline_measurement_finalize")
    write_run_result(
