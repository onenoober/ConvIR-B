...
SNAPSHOT_RULES_COMMIT=23ea15ca840f0718c41db38d2e3aebfb53907dc5
TMP_ROOT=$(mktemp -d)
REPO=$TMP_ROOT/repo
...
/usr/bin/git -C "$REPO" checkout --quiet --detach "$REMOTE_SHA"
test -z "$(/usr/bin/git -C "$REPO" status --porcelain)"

"$PYTHON" "$REPO/experience_docx/tools/policy_snapshot.py" \
  --repo "$REPO" --rules-commit "$SNAPSHOT_RULES_COMMIT" --check
