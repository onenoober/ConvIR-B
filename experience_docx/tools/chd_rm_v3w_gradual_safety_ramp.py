#!/usr/bin/env python3
"""Fixed linear safety-ramp diagnostic after the v3v abrupt-switch failure."""
...
ROUTE_ID = "haze4k_v5_chd_rm_v3w_gradual_safety_ramp_20260713"
...
def delta_for(kind, model, sample, operator):
    if kind != "output":
        raise ValueError(f"v3w only permits output-side activation, got {kind}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for start in range(0, len(names), args.risk_window):
            ...
            if objective != "safety_curriculum":
                raise ValueError(f"unexpected v3w objective: {objective}")
            if epoch <= args.warmup_epochs:
                ...
                phase = "safety_ramp"
                safety_scale = (epoch - args.warmup_epochs) / (args.epochs - args.warmup_epochs)
        ...
        history.append({
            ...
            "epoch": epoch,
            "phase": phase,
            "safety_weight_scale": 0.0 if epoch <= args.warmup_epochs else safety_scale,
            "updates": updates,
            **{key: value / updates for key, value in totals.items()},
        })
        print(json.dumps({"RAMP_PROGRESS": history[-1]}, sort_keys=True), flush=True)
        if epoch == args.warmup_epochs:
...
def run_noop(args, v3s, legacy, frozen, names, folds, device, output_dir):
    ...
    closeout = {
        "route_id": ROUTE_ID,
        "run_id": args.run_tag,
        "stage": "v3w-S0-output-form-exact-noop",
        "state": "COMPLETED_GATE_PASS" if passed else "COMPLETED_GATE_FAIL",
        "gate_type": "structural_integrity",
        "decision": "V3W_S0_NOOP_PASS_AUTHORIZE_GRADUAL_RAMP_ONLY" if passed else "V3W_S0_NOOP_FAIL_STOP",
        "authorizes": "v3w-S1 output-side gradual-ramp fixed32 diagnostic only" if passed else "none",
...
def run_curriculum(args, v3s, legacy, frozen, names, folds, device, output_dir):
    ...
    if not summary["midpoint_activity_pass"]:
        decision = "V3W_S1_WARMUP_INACTIVE_STOP_GRADUAL_RAMP"
    elif not summary["final_activity_pass"]:
        decision = "V3W_S1_RAMP_LOSES_ACTIVITY_STOP_GRADUAL_RAMP"
    elif not summary["safety_nonworse_vs_v3u"]:
        decision = "V3W_S1_SAFETY_DIAGNOSTICS_NOT_NONWORSE_STOP_GRADUAL_RAMP"
    else:
        decision = "V3W_S1_RAMP_PASS_AUTHORIZE_SAFETY_TRAINING_CONTRACT_DESIGN_ONLY"
    passed = decision == "V3W_S1_RAMP_PASS_AUTHORIZE_SAFETY_TRAINING_CONTRACT_DESIGN_ONLY"
    closeout = {
        "route_id": ROUTE_ID,
        "run_id": args.run_tag,
        "stage": "v3w-S1-output-side-gradual-safety-ramp",
        "state": "COMPLETED_GATE_PASS" if passed else "COMPLETED_GATE_FAIL",
        ...
        "authorizes": (
            ...
            if passed
            else "none; this fixed gradual safety ramp is stopped"
        ),
        "metric_contract": "32 fixed OOF images, output-side Delta-u, 8-epoch render warmup then fixed linear safety ramp to full anchor/margin/harm/CVaR, no repair penalty",
...
def parse_args():
    ...
    parser.add_argument("--mode", required=True, choices=("noop", "ramp"))
