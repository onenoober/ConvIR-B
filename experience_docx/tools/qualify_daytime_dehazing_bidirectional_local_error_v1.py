...
ROUTE_ID = "daytime-dehazing-bidirectional-local-error-operationalization-v1"
OPERATION_ID = "DAYTIME_DEHAZING_BIDIRECTIONAL_LOCAL_ERROR_QUALIFY"
SUMMARY_NAME = "daytime_dehazing_bidirectional_local_error_v1_summary.json"
SCENE_METRICS_NAME = "daytime_dehazing_bidirectional_local_error_v1_scene_metrics.csv"
GATE_NAME = "daytime_dehazing_bidirectional_local_error_v1_gate_summary.json"
REVIEW_FACTS_NAME = "daytime_dehazing_bidirectional_local_error_v1_review_facts.json"
...
ALPHA_GRID = DENSE_GLOBAL_ALPHA_GRID
KEEP_ALPHA = 1.0
ALPHA_TIE_DB = 0.005
GLOBAL_ALPHA_TIE_DB = 0.0001
LOCAL_DIRECTION_MARGIN_DB = 0.05
KEEP_EQUIVALENCE_DB = 0.02
WITHIN_OBSERVATION_ALPHA_TOLERANCE = 0.10
KEEP_ALPHA_TOLERANCE = 0.05
CROSS_OBSERVATION_ALPHA_TOLERANCE = 0.10
PRIMARY_MARGIN_DB = 0.10
TRANSFER_MARGIN_DB = 0.10
...
CAPABILITY_HEIGHT = 992
CAPABILITY_WIDTH = 640
EPSILON = 1.0e-12
...
S1_V1_CLOSEOUT_SHA256 = "8272c22034ad827c39ac2ff7d7c1903f39a9872f8a7a81ca854bc95b045af98b"
S1_V1_SUMMARY_SHA256 = "5e3b678c8968a0c243f6bf79fe40b8ca80e860a376e475b61f90599eded0df1f"
PAIRING_V2_CLOSEOUT_SHA256 = "915d98b1dae5847cdb1166610e16ade96b5fb656373587d08895e2a7863492b9"
PAIRING_V2_SUMMARY_SHA256 = "d7e4900fab8701e7ae764775cd527510795cf0c357964a7e7cf0f1b2254c3699"
PAIRING_V2_IDENTITY_SHA256 = "123acad0dcf73b958d2d02845c8c51f0eac89023a469de551102ca40a714794a"
PAIRING_LEDGER_SHA256 = "d3e889b3cce92c0a11f25a6a49db0e3a3854b7eb427721b2410871ad65d39c25"
PAIRING_ROUTE_ID = "daytime-dehazing-reside-pairing-contract-v2"
PAIRING_OPERATION_ID = "DAYTIME_DEHAZING_RESIDE_PAIRING_CONTRACT_QUALIFY"
RESIDE_ROOT_BINDINGS = (
    (PurePosixPath("official/ITS/train/ITS_clear"), "its_train_clear"),
    (PurePosixPath("official/ITS/train/ITS_haze"), "its_train_haze"),
    (PurePosixPath("official/ITS/val/clear"), "its_val_clear"),
    (PurePosixPath("official/ITS/val/haze"), "its_val_haze"),
    (PurePosixPath("official/OTS_ALPHA/clear_images"), "ots_clear"),
    (PurePosixPath("official/OTS_ALPHA/OTS"), "ots_haze"),
)
...
def resolve_reside_ledger_path(
    context: Any, relative_value: Any,
) -> tuple[Path, str]:
    if not isinstance(relative_value, str):
        raise RuntimeError("pairing ledger path is not a string")
    relative = PurePosixPath(relative_value)
    if relative.is_absolute() or not relative.parts or ".." in relative.parts:
        raise RuntimeError("pairing ledger path escapes the frozen relative namespace")
    matches = [
        (prefix, asset_id)
        for prefix, asset_id in RESIDE_ROOT_BINDINGS
        if relative.parts[:len(prefix.parts)] == prefix.parts
    ]
    if len(matches) != 1:
        raise RuntimeError("pairing ledger path does not map to one frozen root")
    prefix, asset_id = matches[0]
    root = asset_path(context, asset_id, kind="directory").resolve(strict=True)
    candidate = root.joinpath(*relative.parts[len(prefix.parts):]).resolve(strict=True)
    if root not in candidate.parents or not candidate.is_file():
        raise RuntimeError("pairing ledger payload resolves outside its frozen root")
    return candidate, asset_id


def load_reside_pairing_ledger(
    context: Any,
    allowed_roles: dict[str, dict[str, str]],
) -> tuple[
    dict[str, dict[str, list[tuple[Path, Path]]]],
    dict[str, dict[str, Any]],
    dict[str, Any],
]:
    ledger_path = asset_path(context, "reside_pairing_ledger", kind="file")
    if sha256_file(ledger_path) != PAIRING_LEDGER_SHA256:
        raise RuntimeError("RESIDE pairing ledger identity changed")
    groups: dict[str, dict[str, list[tuple[Path, Path]]]] = {
        "ITS": {}, "OTS": {},
    }
    row_counts = Counter()
    ...
    with ledger_path.open("r", encoding="utf-8") as stream:
        for raw_line in stream:
            ...
            if row.get("schema_version") != 1 \
                    or row.get("route_id") != PAIRING_ROUTE_ID \
                    or row.get("operation_id") != PAIRING_OPERATION_ID \
                    or row.get("independent_unit") != "original_clear_scene" \
                    or dataset not in groups \
                    or not isinstance(scene, str):
                raise RuntimeError("RESIDE pairing ledger schema or route identity changed")
            scene_key = (dataset, scene)
            if scene_key in seen_scenes or scene not in allowed_roles[dataset]:
                raise RuntimeError("RESIDE pairing ledger scene membership is invalid")
            ...
            if not isinstance(checks, dict) or not checks or not all(
                ...
                raise RuntimeError("RESIDE pairing ledger contains a failed pairing check")

            content = row.get("content_identity")
            observations = row.get("formation_observations")
            if not isinstance(content, dict) or not isinstance(observations, list) \
                    or len(observations) != VARIANTS_PER_SCENE:
                raise RuntimeError("RESIDE pairing ledger row lacks the frozen pair")
            expected_clear_digest = allowed_roles[dataset][scene]
            if content.get("expected_s0_rgb_sha256") != expected_clear_digest \
                    or content.get("canonical_clear_rgb_sha256") != expected_clear_digest \
                    or not is_sha256(content.get("clear_file_sha256")):
                raise RuntimeError("RESIDE clear identity disagrees with S0")
            clear_path, clear_asset = resolve_reside_ledger_path(
                context, content.get("clear_relpath"),
            )
            if clear_asset not in {"its_train_clear", "its_val_clear", "ots_clear"} \
                    or sha256_file(clear_path) != content["clear_file_sha256"]:
                raise RuntimeError("RESIDE clear payload identity changed")
            full_clear = image_array(clear_path)
            height, width = full_clear.shape[:2]
            if canonical_rgb_digest(full_clear) != expected_clear_digest \
                    or row.get("width") != width \
                    or row.get("height") != height \
                    or row.get("padded_width") != ((width + 31) // 32) * 32 \
                    or row.get("padded_height") != ((height + 31) // 32) * 32 \
                    or row.get("scoring_crop_side") != (
                        min(height, width, MAX_CROP_SIDE) // 32
                    ) * 32:
                raise RuntimeError("RESIDE clear geometry or decoded identity changed")

            pairs: list[tuple[Path, Path]] = []
            haze_file_hashes: set[str] = set()
            haze_rgb_hashes: set[str] = set()
            source_tokens: set[str] = set()
            for ordinal, observation in enumerate(observations, start=1):
                if not isinstance(observation, dict) \
                        or observation.get("ordinal") != ordinal \
                        or not is_sha256(observation.get("haze_file_sha256")) \
                        or not is_sha256(observation.get("haze_rgb_sha256")) \
                        or not isinstance(observation.get("source_token"), str) \
                        or not observation["source_token"]:
                    raise RuntimeError("RESIDE formation observation identity is invalid")
                haze_path, haze_asset = resolve_reside_ledger_path(
                    context, observation.get("haze_relpath"),
                )
                if haze_asset not in {"its_train_haze", "its_val_haze", "ots_haze"} \
                        or sha256_file(haze_path) != observation["haze_file_sha256"]:
                    raise RuntimeError("RESIDE haze payload identity changed")
                full_haze = image_array(haze_path)
                if full_haze.shape != full_clear.shape \
                        or canonical_rgb_digest(full_haze) != observation["haze_rgb_sha256"]:
                    raise RuntimeError("RESIDE haze decoded identity or geometry changed")
                haze_file_hashes.add(observation["haze_file_sha256"])
                haze_rgb_hashes.add(observation["haze_rgb_sha256"])
                source_tokens.add(observation["source_token"])
                pairs.append((haze_path, clear_path))
                verified_file_count += 1
            if len(haze_file_hashes) != VARIANTS_PER_SCENE \
                    or len(haze_rgb_hashes) != VARIANTS_PER_SCENE \
                    or len(source_tokens) != VARIANTS_PER_SCENE:
                raise RuntimeError("RESIDE formation observations are not distinct")
            groups[dataset][scene] = pairs
            verified_file_count += 1

    coverage: dict[str, dict[str, Any]] = {}
    for dataset in ("ITS", "OTS"):
        planned = deterministic_rank(
            allowed_roles[dataset], f"{SCENE_SELECTION_SALT}|{dataset}",
        )[:PLANNED_SCENES[dataset]]
        coverage[dataset] = {
            "allowed_scene_count": len(allowed_roles[dataset]),
            "planned_scene_count": len(planned),
            "planned_scene_digest": sha256_text("\n".join(planned)),
            "selected_scene_count": len(groups[dataset]),
            "two_variants_per_scene": all(
                len(pairs) == VARIANTS_PER_SCENE
                for pairs in groups[dataset].values()
            ),
            "exact_planned_roster_retained": set(groups[dataset]) == set(planned),
            "canonical_clear_identity_match": True,
            "distinct_haze_payloads": True,
            "pairing_contract_verified": True,
        }
    if row_counts != Counter({"ITS": 150, "OTS": 150}) \
            or len(seen_scenes) != 300 \
            or verified_file_count != 900:
        raise RuntimeError("RESIDE pairing ledger coverage changed")
    summary = {
        "route_id": PAIRING_ROUTE_ID,
        "mapping_ledger_sha256": PAIRING_LEDGER_SHA256,
        "row_counts": dict(row_counts),
        "verified_file_count": verified_file_count,
        "all_paths_resolved_within_bound_roots": True,
        "all_file_and_decoded_rgb_identities_verified": True,
    }
    return groups, coverage, summary
...
def deterministic_crop_box(
    ...
    crop_side = (min(height, width, MAX_CROP_SIDE) // 32) * 32
    if crop_side < MIN_CROP_SIDE:
        raise RuntimeError(f"pair scoring crop outside frozen geometry for {scene[:24]}")
    seed = int(sha256_text(f"{CROP_SELECTION_SALT}|{scene}")[:16], 16)
    y0 = seed % (height - crop_side + 1)
    x0 = (seed // 65537) % (width - crop_side + 1)
    return y0, y0 + crop_side, x0, x0 + crop_side
...
def infer_scored_crop(
    ...
    height, width = hazy.shape[:2]
    full_prediction = infer_official(torch, model, hazy, device)
    return (
        hazy[y0:y1, x0:x1].copy(),
        clear[y0:y1, x0:x1].copy(),
        full_prediction[y0:y1, x0:x1].copy(),
        {
            "full_height": height,
            "full_width": width,
...
def _normalized_feature(value: np.ndarray) -> np.ndarray:
    ...
    scale = float(np.quantile(value, 0.95) - np.quantile(value, 0.05))
    if scale <= EPSILON:
        return np.zeros_like(value)
    return np.clip((value - float(np.quantile(value, 0.05))) / scale, 0.0, 1.0)


def _aligned_boundaries(energy: np.ndarray, length: int) -> list[int]:
    boundaries = [0]
    radius = max(8, length // 16)
    for index in range(1, GRID_SIDE):
        nominal = round(index * length / GRID_SIDE)
        lower = max(boundaries[-1] + length // 8, nominal - radius)
        upper = min(length - (GRID_SIDE - index) * (length // 8), nominal + radius)
        candidates = np.arange(lower, upper + 1, dtype=np.int64)
        candidate_energy = energy[candidates - 1]
        best = float(np.min(candidate_energy))
        eligible = candidates[np.isclose(candidate_energy, best, rtol=0.0, atol=1.0e-12)]
        boundaries.append(int(min(eligible, key=lambda value: (abs(value - nominal), value))))
    boundaries.append(length)
    return boundaries


def content_aligned_regions(
    hazy_observations: list[np.ndarray], prediction_observations: list[np.ndarray],
) -> list[tuple[slice, slice]]:
    if len(hazy_observations) != len(prediction_observations) or not hazy_observations:
        raise RuntimeError("content-aligned regions require matched observable observations")
    shape = hazy_observations[0].shape
    if any(value.shape != shape for value in hazy_observations + prediction_observations):
        raise RuntimeError("content-aligned observations must share one scored shape")
    features = []
    for hazy, prediction in zip(hazy_observations, prediction_observations):
        hazy_luminance = np.mean(hazy, axis=2)
        prediction_luminance = np.mean(prediction, axis=2)
        residual_magnitude = np.mean(np.abs(prediction - hazy), axis=2)
        features.append(
            _normalized_feature(hazy_luminance)
            + _normalized_feature(prediction_luminance)
            + _normalized_feature(residual_magnitude)
        )
    consensus = np.mean(features, axis=0)
    row_energy = np.mean(np.abs(np.diff(consensus, axis=0)), axis=1)
    column_energy = np.mean(np.abs(np.diff(consensus, axis=1)), axis=0)
    height, width = shape[:2]
    row_boundaries = _aligned_boundaries(row_energy, height)
    column_boundaries = _aligned_boundaries(column_energy, width)
    regions = []
    for row in range(GRID_SIDE):
        outer_y0, outer_y1 = row_boundaries[row], row_boundaries[row + 1]
        for column in range(GRID_SIDE):
            outer_x0, outer_x1 = column_boundaries[column], column_boundaries[column + 1]
            margin = max(4, math.floor(min(outer_y1 - outer_y0, outer_x1 - outer_x0) * 0.125))
            regions.append((
                slice(outer_y0 + margin, outer_y1 - margin),
                slice(outer_x0 + margin, outer_x1 - margin),
            ))
    return regions
...
def candidate_errors(
    ...
    sums, count = dense_global_errors(hazy, prediction, target, region, mask)
    return sums, np.full(len(ALPHA_GRID), count, dtype=np.float64)
...
def analyze_variant(
    ...
    regions: list[tuple[slice, slice]] | None = None,
) -> dict[str, Any]:
    height, width = hazy.shape[:2]
    ...
    regions = regions or content_aligned_regions([hazy], [prediction])
    if len(regions) != GRID_SIDE * GRID_SIDE:
        raise RuntimeError("content-aligned region count changed")
    for cell_index, region in enumerate(regions):
        ...
        selected_alphas = [float(ALPHA_GRID[index]) for index in selected_true]
        selected_directions = [direction(alpha) for alpha in selected_alphas]
        cross_halves = (
        ...
        if selected_directions[0] == selected_directions[1] != "keep" \
                and abs(selected_alphas[0] - selected_alphas[1]) <= WITHIN_OBSERVATION_ALPHA_TOLERANCE \
                and min(cross_halves) >= LOCAL_DIRECTION_MARGIN_DB:
            label = selected_directions[0]
            recommended_alpha = float(np.mean(selected_alphas))
            recommended_index = int(np.argmin(np.abs(ALPHA_GRID - recommended_alpha)))
        elif max(abs(alpha - KEEP_ALPHA) for alpha in selected_alphas) <= KEEP_ALPHA_TOLERANCE:
        ...
        tile_records.append({
            "cell": cell_index,
            "label": label,
            "selected_alpha_a": selected_alphas[0],
            "selected_alpha_b": selected_alphas[1],
            "recommended_alpha": (
...
def analyze_scene(
    ...
    variant_inputs: list[tuple[np.ndarray, np.ndarray, np.ndarray]] = []
    input_parts = ["bidirectional-local-error-operationalization-v1", dataset, scene]
    expected_shape = None
    inference_records = []
    for pair_index, (hazy_path, clear_path) in enumerate(pairs):
        ...
        hazy, clear, prediction, inference_record = infer_scored_crop(
        ...
        variant_inputs.append((hazy, clear, prediction))
    ...
    if len(variant_inputs) != VARIANTS_PER_SCENE:
        raise RuntimeError("scene analysis did not use exactly two nested variants")
    regions = content_aligned_regions(
        [item[0] for item in variant_inputs],
        [item[2] for item in variant_inputs],
    )
    variants = [
        analyze_variant(hazy, clear, prediction, regions)
        for hazy, clear, prediction in variant_inputs
    ]
    ...
    for index in range(GRID_SIDE * GRID_SIDE):
        ...
        alpha_tolerance_agreement.append(bool(
            repeated_labels[index] != "discordant"
            and abs(left["recommended_alpha"] - right["recommended_alpha"])
            <= CROSS_OBSERVATION_ALPHA_TOLERANCE
        ))
    ...
    alpha_tolerance_count = sum(alpha_tolerance_agreement)
    bidirectional_scene = (
    ...
    row = {
        ...
        "primary_clipped_low": transfer_raw < -PRIMARY_CLIP_DB,
        "primary_clipped_high": transfer_raw > PRIMARY_CLIP_DB,
        ...
        "alpha_tolerance_stable_region_count": alpha_tolerance_count,
        ...
        "alpha_tolerance_agreement_fraction": alpha_tolerance_count / (GRID_SIDE * GRID_SIDE),
        ...
        "maximum_full_image_padded_pixels": max(
            record["padded_height"] * record["padded_width"]
            for record in inference_records
        ),
        "direction_and_strength_stable_scene": (
            stable_cell_count >= STABLE_CELL_COUNT
            and alpha_tolerance_count >= STRENGTH_STABLE_CELL_COUNT
        ),
        ...
        "negative_tail_scene": transfer_raw < NEGATIVE_TAIL_DB,
...
def evidence_identity(context, include_ledger: bool) -> dict[str, bool]:
    ...
    s1_v1_closeout = asset_path(context, "s1_v1_closeout", kind="file")
    s1_v1_summary = asset_path(context, "s1_v1_summary", kind="file")
    pairing_closeout = asset_path(context, "pairing_v2_closeout", kind="file")
    pairing_summary = asset_path(context, "pairing_v2_summary", kind="file")
    pairing_identity = asset_path(context, "pairing_v2_identity_summary", kind="file")
    s0_terminal = read_json(closeout)
    s1_terminal = read_json(s1_v1_closeout)
    pairing_terminal = read_json(pairing_closeout)
    checks = {
        "s0_closeout_sha256": sha256_file(closeout) == S0_CLOSEOUT_SHA256,
        "s0_terminal": (
            s0_terminal.get("state") == "COMPLETED_GATE_PASS"
            and s0_terminal.get("decision") == "DAYTIME_DEHAZING_PROGRAM_FOUNDATION_PASS"
            and s0_terminal.get("authorizes") == "AUTHOR_S1_UTILITY_QUALIFICATION_CONTRACT_ONLY"
        ),
        "s0_role_summary_sha256": sha256_file(role_summary) == S0_ROLE_SUMMARY_SHA256,
        "s1_v1_closeout_sha256": sha256_file(s1_v1_closeout) == S1_V1_CLOSEOUT_SHA256,
        "s1_v1_summary_sha256": sha256_file(s1_v1_summary) == S1_V1_SUMMARY_SHA256,
        "s1_v1_pre_inference_validity_terminal": (
            s1_terminal.get("state") == "COMPLETED_INCONCLUSIVE"
            and s1_terminal.get("decision")
            == "DAYTIME_DEHAZING_LOCAL_RESTORATION_NEED_INCONCLUSIVE"
            and s1_terminal.get("authorizes")
            == "S1_MEASUREMENT_VALIDITY_OR_PRECISION_REVIEW_ONLY"
        ),
        "pairing_v2_closeout_sha256": sha256_file(pairing_closeout)
        == PAIRING_V2_CLOSEOUT_SHA256,
        "pairing_v2_summary_sha256": sha256_file(pairing_summary)
        == PAIRING_V2_SUMMARY_SHA256,
        "pairing_v2_identity_summary_sha256": sha256_file(pairing_identity)
        == PAIRING_V2_IDENTITY_SHA256,
        "pairing_v2_authorization": (
            pairing_terminal.get("state") == "COMPLETED_GATE_PASS"
            and pairing_terminal.get("decision")
            == "DAYTIME_DEHAZING_RESIDE_PAIRING_CONTRACT_PASS"
            and pairing_terminal.get("authorizes")
            == "AUTHOR_REVISED_S1_UTILITY_QUALIFICATION_CONTRACT_ONLY"
            and pairing_terminal.get("details", {}).get("mapping_ledger_sha256")
            == PAIRING_LEDGER_SHA256
            and pairing_terminal.get("details", {}).get("restoration_outcomes_accessed")
            is False
        ),
    }
    if include_ledger:
        ...
        pairing_ledger = asset_path(context, "reside_pairing_ledger", kind="file")
        checks["reside_pairing_ledger_sha256"] = sha256_file(pairing_ledger) \
            == PAIRING_LEDGER_SHA256
    return checks
...
def contract(context_path: Path) -> None:
    ...
    entrypoint = asset_path(context, "bidirectional_local_error_entrypoint", kind="file")
    anchor = context.assets.get("official_anchor_checkout")
    checks.update({
        "entrypoint_identity": context.assets["bidirectional_local_error_entrypoint"].sha256 == sha256_file(entrypoint),
        ...
        "run_only_assets_absent": all(
            ...
            for identifier in (
                "s0_scene_role_ledger", "haze4k_train", "reside_pairing_ledger",
    ...
    finite = True
    synthetic_inputs: list[tuple[np.ndarray, np.ndarray, np.ndarray]] = []
    result = None
    for index in range(PROBE_ITERATIONS):
        full_hazy = generator.uniform(
            0.05, 0.95, size=(CAPABILITY_HEIGHT, CAPABILITY_WIDTH, 3),
        ).astype(np.float32)
        full_clear = np.clip(
        ...
        synthetic_inputs.append((hazy, clear, prediction))
        if len(synthetic_inputs) == VARIANTS_PER_SCENE:
            regions = content_aligned_regions(
                [item[0] for item in synthetic_inputs],
                [item[2] for item in synthetic_inputs],
            )
            for synthetic_hazy, synthetic_clear, synthetic_prediction in synthetic_inputs:
                result = analyze_variant(
                    synthetic_hazy, synthetic_clear, synthetic_prediction, regions,
                )
        ...
        write_contract_progress(
            ...
            stage="synthetic_full_image_inference_and_consensus_region_measurement",
        )
    ...
    checks.update({
        ...
        "content_aligned_dense_local_crossfit_path": (
            result is not None
            and len(result["tiles"]) == GRID_SIDE * GRID_SIDE
            and len(ALPHA_GRID) == 1001
        ),
...
def run(context_path: Path) -> None:
    ...
    if all(identity_checks.values()):
        try:
            ...
            selected_groups["HAZE4K_TRAIN"], coverage["HAZE4K_TRAIN"] = \
                select_scene_pairs(
                    "HAZE4K_TRAIN", haze4k_groups, ledger_roles["HAZE4K_TRAIN"],
                )
            reside_groups, reside_coverage, pairing_summary = load_reside_pairing_ledger(
                context, ledger_roles,
            )
            for dataset in ("ITS", "OTS"):
                selected_groups[dataset] = reside_groups[dataset]
                coverage[dataset] = reside_coverage[dataset]
            ledger_summary["reside_pairing"] = pairing_summary
            input_roster_identities, selected_input_roster_digest = freeze_selected_input_roster(
                selected_groups,
            )
            selected_input_roster_digest = sha256_text(
                f"{PAIRING_LEDGER_SHA256}|{selected_input_roster_digest}"
            )
        except Exception as error:
            preflight_error = {
                "error_type": type(error).__name__,
    ...
    coverage_checks = {
        ...
        "reside_pairing_contract_verified": all(
            coverage.get(dataset, {}).get("pairing_contract_verified") is True
            for dataset in ("ITS", "OTS")
        ),
    ...
    if not identity_pass or not coverage_pass:
        gate_outcomes = {
            ...
            "measurement_null_artifact": "invalid",
    ...
    within_observation_utility = mean_family(
        "primary_local_minus_global_psnr_db", BOOTSTRAP_SEED,
    )
    ...
    primary = mean_family("cross_observation_transfer_psnr_db", BOOTSTRAP_SEED + 30)
    prevalence = {
    ...
    affirmative_null_artifact = (
        null["overall"]["lower"] >= NULL_UCB_MARGIN_DB
        or any(
            item["lower"] >= NULL_DATASET_UCB_MARGIN_DB
    ...
    utility_state = combine_states((
        mean_cross_dataset_state(
            within_observation_utility, PRIMARY_MARGIN_DB, 0.0, 0.0,
        ),
        mean_cross_dataset_state(
    ...
    repeatability_state = combine_states((
        mean_cross_dataset_state(primary, TRANSFER_MARGIN_DB, 0.0, 0.0),
        prevalence_cross_dataset_state(
    ...
    precision_met = (
        all(item["max_half_width"] <= PRECISION_HALF_WIDTH_DB for item in precision_intervals)
        and null_precision_met
    )
    gate_outcomes = {
        "evidence_identity": "pass",
        "development_scene_coverage": "pass",
        "measurement_null_artifact": "fail" if affirmative_null_artifact else "pass",
    ...
    atomic_json(aggregate_unit, {
        ...
        "within_observation_utility": within_observation_utility,
        "cross_observation_transfer": primary,
        "null": null,
    ...
    if not coverage_checks["completed_unit_ledger"]:
        ...
        for gate in (
            "measurement_null_artifact", "local_utility_over_global",
    ...
    terminal_summary(context, gate_outcomes, {
        ...
        "operation_id": OPERATION_ID,
        "run_id": context.run_id,
        "scope": "development-only privileged sequence-2 bidirectional local-error qualification",
        ...
        "intervention": {
            ...
            "local_alpha_grid": {
                "minimum": float(ALPHA_GRID[0]),
                "maximum": float(ALPHA_GRID[-1]),
            ...
            "regions": "16 shared observable content-aligned rectangular regions; three row and three column boundaries minimize two-observation consensus image/output/residual feature discontinuity inside frozen neighborhoods, and the central 75 percent of each side is scored without a taper",
            "selection": "the same 1001-point alpha grid for local and global actions, two-way 8x8 macroblock checkerboard cross-selection, opposite-half evaluation, within-observation alpha agreement within 0.10, and cross-observation strength agreement within 0.10",
            "inference_context": "the complete image is padded to a multiple of 32 and inferred once before the frozen square scoring crop is extracted",
        ...
        "null_control": {
            ...
            "affirmative_artifact": affirmative_null_artifact,
            "absence_precision_met": null_precision_met,
        },
        "prevalence": prevalence,
        "near_clear_fidelity": {
        ...
        "diagnostics": {
            ...
            "mean_alpha_tolerance_agreement_fraction": float(np.mean([
                row["alpha_tolerance_agreement_fraction"] for row in rows
            ])),
        ...
        "forbidden_activity_receipt": {
            ...
            "trainable_observable_signal_or_mechanism_selected": False,
        ...
        "limitations": [
            ...
            "Each original clear scene contributes once; haze variants, content-aligned regions, pixels, actions, and bootstrap draws remain nested.",
            "The shared content-aligned region partition uses only the two hazy observations and frozen official outputs; GT selects privileged alpha values but never defines region boundaries.",
            ...
            "RESIDE ITS and OTS use only the exact 300-scene, 600-observation SHA-bound pairing-v2 ledger; no runtime directory scan, filename rematching, eligibility backfill, or variant reselection is permitted.",
        ...
        "marker": "DAYTIME_DEHAZING_BIDIRECTIONAL_LOCAL_ERROR_V1_COMPLETE",
    }, rows)
    write_gate_result(
        context,
        gate_outcomes=gate_outcomes,
