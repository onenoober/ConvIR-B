...
from eval_udpnet_v15_phase0_repro import infer_one, load_convir_builders  # noqa: E402
...
def load_a0_model(build_convir_net, checkpoint: Path, device: torch.device, data_name: str):
    model = build_convir_net("base", data_name, "original").to(device)
...
def evaluate(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    a0 = load_a0_model(build_convir, Path(args.a0_checkpoint), device, args.a0_data)
    ...
    manifest = {
        "route": "haze4k_v2_8_nhhaze_official_weights_20260616",
        "mode": "evaluate_shard",
        "prefix": args.prefix,
    ...
    print(f"V28_NHHAZE_OFFICIAL_EVAL_SHARD_OK shard={args.shard_index} rows={len(rows)}")
    return rows
...
def aggregate(args: argparse.Namespace) -> None:
    ...
    for alpha in alphas:
        ...
        if alpha == 0.0:
            role = "a0_nhhaze_baseline"
        elif alpha == 1.0:
            role = "wdmamba_nhhaze_endpoint"
        elif alpha == PRIMARY_ALPHA:
            role = "inherited_fixed_alpha_diagnostic"
        else:
            role = "diagnostic_grid"
        alpha_rows.append({"alpha": alpha, "role": role, **summarize(sub)})
    ...
    if primary_positive and primary_tail_safer_than_full:
        decision = "V28_NHHAZE_OFFICIAL_WEIGHT_INHERITED_ALPHA_SUPPORTS_SHRINKAGE_DIAGNOSTIC"
    elif primary_positive:
        decision = "V28_NHHAZE_OFFICIAL_WEIGHT_INHERITED_ALPHA_POSITIVE_TAIL_MIXED"
    else:
    ...
    summary = {
        ...
        "a0_data": args.a0_data,
        "a0_checkpoint": str(args.a0_checkpoint) if args.a0_checkpoint else None,
        ...
        "primary_inherited_alpha_summary": primary,
        "wdmamba_nhhaze_endpoint_summary": endpoint,
        "a0_nhhaze_baseline_summary": a0,
        ...
        "primary_positive_diagnostic": primary_positive,
    ...
    decision_md = (
        ...
        f"Decision: `{decision}`\n\n"
        "This route evaluates NH-HAZE with NH-HAZE-specific ConvIR-B and WDMamba checkpoints. "
        "The `alpha=1.0` row is the official-weight WDMamba endpoint relative to the official-weight ConvIR-B A0 baseline. "
        "The `alpha=0.375` row reuses the Haze4K-discovered fixed shrinkage coefficient only as a predeclared diagnostic; "
        "the alpha grid is not used to tune NH-HAZE.\n\n"
    ...
    (out_dir / "v28_decision.md").write_text(decision_md, encoding="utf-8")
    ...
    print(f"V28_NHHAZE_OFFICIAL_AGGREGATE_OK decision={decision} rows={len(rows)}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prefix", default="v28_nhhaze_official_weights")
    parser.add_argument("--convir-its-dir", type=Path, default=Path("Dehazing/ITS"))
    parser.add_argument("--a0-data", default="NHR")
