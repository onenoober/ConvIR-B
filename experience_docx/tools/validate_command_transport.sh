...
readonly remote_url="https://github.com/onenoober/ConvIR-B.git"
...
for required in /usr/bin/awk /usr/bin/git /usr/bin/grep /usr/bin/mkdir \
  /usr/bin/mktemp /usr/bin/rm /usr/bin/tee /usr/bin/timeout /bin/bash \
  "${cloud_python}"; do
  if [[ ! -x "${required}" ]]; then
...
readonly run_root="${evidence_root}/${candidate_commit}"
readonly status_file="${run_root}/status.txt"
readonly log_file="${run_root}/cloud_test.log"
if [[ -e "${run_root}" ]]; then
  printf 'validation root already exists: %s\n' "${run_root}" >&2
  exit 3
fi
/usr/bin/mkdir -p "${run_root}"
printf 'state=RUNNING\ncandidate_commit=%s\nmodel_calls=0\n' "${candidate_commit}" >"${status_file}"

work_root=$(/usr/bin/mktemp -d "${evidence_root}/work.XXXXXX")
validation_state=RUNNING
cleanup() {
  exit_code=$?
  if [[ "${work_root}" == "${evidence_root}/work."* && -d "${work_root}" ]]; then
    /usr/bin/rm -rf -- "${work_root}"
  fi
  if [[ "${validation_state}" != "PASS" ]]; then
    printf 'state=FAILED\ncandidate_commit=%s\nexit_code=%s\nmodel_calls=0\n' \
      "${candidate_commit}" "${exit_code}" >"${status_file}"
  fi
}
trap cleanup EXIT
