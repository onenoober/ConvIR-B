...
def gpu_probe_body(context, gpu_index=None):
    ...
    return "\n".join([
        ...
        '        total++',
        '        if (shown < limit) {',
        '          summary_idx[shown]=sprintf("%d", idx + 0); summary_free[shown]=sprintf("%d", free + 0); summary_util[shown]=sprintf("%d", util + 0)',
        '          shown++',
        '        }',
        ...
        '        }',
        '        summary=""',
        '        for (i=0; i<shown; i++) summary = summary (i ? ";" : "") sprintf("%d:%d:%d", summary_idx[i], summary_free[i], summary_util[i])',
