...
REMOTE_REPO=${REMOTE_REPO:-/sda/home/wangyuxin/ConvIR-B/repos/ConvIR-B-v3v-safety-curriculum-activation-20260713}
RUN_ROOT=${RUN_ROOT:-/sda/home/wangyuxin/ConvIR-B/runs/haze4k_v5_chd_rm_v3v_safety_curriculum_activation_20260713}
...
GPU=${GPU:-1}
MODE=${MODE:?set MODE=noop|curriculum}
EXPECTED_ROUTE_COMMIT=${EXPECTED_ROUTE_COMMIT:?set the exact v3v route commit before launch}
...
case "$MODE" in
  noop)
    ...
    RUN_TAG=v3v_s0_noop32
    OUT=$RUN_ROOT/s0_noop32
    ;;
  curriculum)
    STAGE=v3v-S1-output-side-safety-curriculum
    RUN_TAG=v3v_s1_safety_curriculum32
    OUT=$RUN_ROOT/s1_safety_curriculum32
    ;;
  *)
    echo V3V_INVALID_MODE
    exit 2
    ;;
esac
...
if [ "$MODE" = curriculum ]; then
  test -s "$EVID_STAGE/v3v_s0_noop32_closeout.json"
  "$PY" - "$EVID_STAGE/v3v_s0_noop32_closeout.json" <<'PY'
import json
import sys
closeout = json.load(open(sys.argv[1], encoding="utf-8"))
if closeout.get("decision") != "V3V_S0_NOOP_PASS_AUTHORIZE_SAFETY_CURRICULUM_ONLY":
    raise SystemExit("v3v curriculum requires S0 no-op pass")
PY
fi
...
set +e
CUDA_VISIBLE_DEVICES="$GPU" PYTHONUNBUFFERED=1 "$PY" "$REMOTE_REPO/experience_docx/tools/chd_rm_v3v_safety_curriculum_activation.py" "${ARGS[@]}" 2>&1 | tee "$LOG"
rc=${PIPESTATUS[0]}
set -e
echo "stage_done route=$ROUTE_ID stage=$STAGE rc=$rc time=$(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -ne 0 ]; then
  echo "V3V_${MODE^^}_FAILED" | tee -a "$STATUS"
  exit "$rc"
fi
...
echo "V3V_${MODE^^}_OK" | tee -a "$STATUS"
