...
on_error() {
  rc=$?
  printf 'SCHEMA3_LIFECYCLE_GATE_RESULT_FIX_CLOUD_FAILED line=%s command=%q rc=%s\n' \
    "$1" "$2" "$rc" >&2
  exit "$rc"
}
trap 'on_error "$LINENO" "$BASH_COMMAND"' ERR

branch=codex/schema3-lifecycle-gate-result-fix
base=b9d777d2b1f4f68d1a4089f5c4c8260bce62f9e8
...
work=$(/usr/bin/mktemp -d /tmp/schema3-lifecycle-gate-result-fix.XXXXXX)

cleanup() {
  case "$work" in
    /tmp/schema3-lifecycle-gate-result-fix.*)
      /bin/rm -rf -- "$work"
      ;;
    *)
...
printf 'SCHEMA3_LIFECYCLE_GATE_RESULT_FIX_STAGE=checkout\n'
...
printf 'SCHEMA3_LIFECYCLE_GATE_RESULT_FIX_STAGE=compile\n'
"$python" -m py_compile \
  "$tools/route_lifecycle.py" \
  "$tests/test_route_lifecycle.py"

printf 'SCHEMA3_LIFECYCLE_GATE_RESULT_FIX_STAGE=focused_regression\n'
"$python" -m unittest -v test_route_lifecycle

printf 'SCHEMA3_LIFECYCLE_GATE_RESULT_FIX_STAGE=full_control_plane_regression\n'
...
printf 'SCHEMA3_LIFECYCLE_GATE_RESULT_FIX_CLOUD_OK candidate=%s tests=%s gpu_access=0 dataset_access=0 protected_data_access=0 experiment_launch=0\n' \
