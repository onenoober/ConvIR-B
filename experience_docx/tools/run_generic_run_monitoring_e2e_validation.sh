...
CLOSEOUT="$EVID_STAGE/generic_run_monitoring_e2e_closeout.json"
SUMMARY="$EVID_STAGE/generic_run_monitoring_e2e_summary.json"
...
test "$(sha256sum "$REMOTE_REPO/experience_docx/tools/run_generic_run_monitoring_e2e_validation.sh" | awk '{print $1}')" = "$RUNNER_SHA256"
...
failure_closeout() {
  ...
  echo "GENERIC_RUN_MONITORING_E2E_FAILED rc=$rc" | tee -a "$STATUS_PATH"
  exit "$rc"
}
trap failure_closeout ERR

"$PY" "$TELEMETRY" sidecar \
  --route-id "$ROUTE_ID" --run-id "$RUN_ID" --phase e2e_validation \
  --heartbeat "$HEARTBEAT_PATH" --parent-pid "$$" --interval-seconds 1 &

"$PY" "$TELEMETRY" event \
  --route-id "$ROUTE_ID" --run-id "$RUN_ID" --phase e2e_validation \
  --status "$STATUS_PATH" --event e2e_start --completed 0 --total 3
...
"$PY" "$TELEMETRY" event \
  --route-id "$ROUTE_ID" --run-id "$RUN_ID" --phase e2e_validation \
  --status "$STATUS_PATH" --event e2e_pass --completed 3 --total 3

write_closeout COMPLETED_GATE_PASS GENERIC_RUN_MONITORING_E2E_PASS GENERIC_RUN_MONITORING_ADOPTION true
trap - ERR
echo GENERIC_RUN_MONITORING_E2E_OK | tee -a "$STATUS_PATH"
