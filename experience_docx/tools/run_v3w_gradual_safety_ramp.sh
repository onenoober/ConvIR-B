...
REMOTE_REPO=${REMOTE_REPO:-/sda/home/wangyuxin/ConvIR-B/repos/ConvIR-B-v3w-gradual-safety-ramp-20260713}
RUN_ROOT=${RUN_ROOT:-/sda/home/wangyuxin/ConvIR-B/runs/haze4k_v5_chd_rm_v3w_gradual_safety_ramp_20260713}
...
GPU=${GPU:-1}
MODE=${MODE:?set MODE=noop|ramp}
EXPECTED_ROUTE_COMMIT=${EXPECTED_ROUTE_COMMIT:?set the exact v3w route commit before launch}

ROUTE_ID=haze4k_v5_chd_rm_v3w_gradual_safety_ramp_20260713
BRANCH=codex/haze4k-v5-v3w-gradual-safety-ramp-20260713
...
case "$MODE" in
  noop)
    ...
    RUN_TAG=v3w_s0_noop32
    OUT=$RUN_ROOT/s0_noop32
    ;;
  ramp)
    STAGE=v3w-S1-output-side-gradual-safety-ramp
    RUN_TAG=v3w_s1_gradual_ramp32
    OUT=$RUN_ROOT/s1_gradual_ramp32
    ;;
  *)
    echo V3W_INVALID_MODE
    exit 2
    ;;
esac
...
if [ "$MODE" = ramp ]; then
  test -s "$EVID_STAGE/v3w_s0_noop32_closeout.json"
  "$PY" - "$EVID_STAGE/v3w_s0_noop32_closeout.json" <<'PY'
import json
import sys
closeout = json.load(open(sys.argv[1], encoding="utf-8"))
if closeout.get("decision") != "V3W_S0_NOOP_PASS_AUTHORIZE_GRADUAL_RAMP_ONLY":
    raise SystemExit("v3w ramp requires S0 no-op pass")
PY
fi
...
set +e
CUDA_VISIBLE_DEVICES="$GPU" PYTHONUNBUFFERED=1 "$PY" "$REMOTE_REPO/experience_docx/tools/chd_rm_v3w_gradual_safety_ramp.py" "${ARGS[@]}" 2>&1 | tee "$LOG"
rc=${PIPESTATUS[0]}
set -e
echo "stage_done route=$ROUTE_ID stage=$STAGE rc=$rc time=$(date --iso-8601=seconds)" | tee -a "$STATUS"
if [ "$rc" -ne 0 ]; then
  echo "V3W_${MODE^^}_FAILED" | tee -a "$STATUS"
  exit "$rc"
fi
...
echo "V3W_${MODE^^}_OK" | tee -a "$STATUS"
