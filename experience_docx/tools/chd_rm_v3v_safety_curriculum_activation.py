#!/usr/bin/env python3
"""Two-phase safety-curriculum diagnostic after v3u render-only activation."""
...
ROUTE_ID = "haze4k_v5_chd_rm_v3v_safety_curriculum_activation_20260713"
OPERATORS = ("D_ref", "D_rep")
ALPHA_LOW = 0.125
ALPHA_HIGH = 0.25
V3U_FINAL_SAFETY = {
    "anchor": 6.74478656037536e-07,
    "harm": 2.2906834828972933e-06,
    "margin": 7.832882713110223e-06,
}
...
def frozen_output_sample(args, v3s, legacy, frozen, name, fold, device):
...
def build_cells(models, sample, args, device):
    return {
        "output_safety_curriculum": (
            "output",
            "safety_curriculum",
            models.DIRT_OutputDeltaU(args.channels, args.delta_bound).to(device),
        ),
    }
...
def delta_for(kind, model, sample, operator):
    if kind != "output":
        raise ValueError(f"v3v only permits output-side activation, got {kind}")
...
def evaluate_cell(args, v3s, legacy, frozen, names, folds, kind, model, device):
    ...
    with torch.no_grad():
        for name in names:
            sample = frozen_output_sample(args, v3s, legacy, frozen, name, folds[name], device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for start in range(0, len(names), args.risk_window):
            ...
            if objective != "safety_curriculum":
                raise ValueError(f"unexpected v3v objective: {objective}")
        ...
        print(json.dumps({"CURRICULUM_PROGRESS": history[-1]}, sort_keys=True), flush=True)
        if epoch == args.warmup_epochs:
            midpoint = evaluate_cell(args, v3s, legacy, frozen, names, folds, kind, model, device)
            print(json.dumps({"WARMUP_SNAPSHOT": midpoint}, sort_keys=True), flush=True)
    final = evaluate_cell(args, v3s, legacy, frozen, names, folds, kind, model, device)
    if midpoint is None:
        raise RuntimeError("curriculum did not produce a warmup snapshot")
    return initial, midpoint, final, history


def run_noop(args, v3s, legacy, frozen, names, folds, device, output_dir):
    models = import_v3v_models()
    first = frozen_output_sample(args, v3s, legacy, frozen, names[0], folds[names[0]], device)
    ...
    closeout = {
        "route_id": ROUTE_ID,
        "run_id": args.run_tag,
        "stage": "v3v-S0-output-form-exact-noop",
        "state": "COMPLETED_GATE_PASS" if passed else "COMPLETED_GATE_FAIL",
        "gate_type": "structural_integrity",
        "decision": "V3V_S0_NOOP_PASS_AUTHORIZE_SAFETY_CURRICULUM_ONLY" if passed else "V3V_S0_NOOP_FAIL_STOP",
        "authorizes": "v3v-S1 output-side safety-curriculum fixed32 diagnostic only" if passed else "none",
...
def run_curriculum(args, v3s, legacy, frozen, names, folds, device, output_dir):
    models = import_v3v_models()
    first = frozen_output_sample(args, v3s, legacy, frozen, names[0], folds[names[0]], device)
    cells = build_cells(models, first, args, device)
    summaries = {}
    ...
    for label, (kind, objective, model) in cells.items():
        initial, midpoint, final, history = train_cell(
        ...
        midpoint_activity = (
            midpoint["delta_abs"] >= args.activity_delta_abs
            and midpoint_reduction >= args.min_relative_render_reduction
        )
        final_activity = final["delta_abs"] >= args.activity_delta_abs and relative_reduction >= args.min_relative_render_reduction
        safety_nonworse = all(final[key] <= V3U_FINAL_SAFETY[key] for key in V3U_FINAL_SAFETY)
        summaries[label] = {
            "input_form": kind,
            "objective": objective,
            ...
            "midpoint_activity_pass": midpoint_activity,
            "final_activity_pass": final_activity,
            "safety_nonworse_vs_v3u": safety_nonworse,
            "v3u_final_safety_reference": V3U_FINAL_SAFETY,
        }
        histories.extend(history)
    summary = summaries["output_safety_curriculum"]
    if not summary["midpoint_activity_pass"]:
        decision = "V3V_S1_WARMUP_INACTIVE_STOP_SAFETY_CURRICULUM"
    elif not summary["final_activity_pass"]:
        decision = "V3V_S1_SAFETY_PHASE_LOSES_ACTIVITY_STOP_SAFETY_CURRICULUM"
    elif not summary["safety_nonworse_vs_v3u"]:
        decision = "V3V_S1_SAFETY_DIAGNOSTICS_NOT_NONWORSE_STOP_SAFETY_CURRICULUM"
    else:
        decision = "V3V_S1_CURRICULUM_PASS_AUTHORIZE_SAFETY_TRAINING_CONTRACT_DESIGN_ONLY"
    passed = decision == "V3V_S1_CURRICULUM_PASS_AUTHORIZE_SAFETY_TRAINING_CONTRACT_DESIGN_ONLY"
    closeout = {
        "route_id": ROUTE_ID,
        "run_id": args.run_tag,
        "stage": "v3v-S1-output-side-safety-curriculum",
        "state": "COMPLETED_GATE_PASS" if passed else "COMPLETED_GATE_FAIL",
        "gate_type": "mechanism_safety_curriculum",
        "decision": decision,
        "authorizes": (
            "new safety-training-contract design only; no training, policy, canary, or locked test"
            if passed
            else "none; this fixed safety curriculum is stopped"
        ),
        "metric_contract": "32 fixed OOF images, output-side Delta-u, 8-epoch render warmup then 8-epoch anchor/margin/harm/CVaR safety phase, no repair penalty",
        "warmup_epochs": args.warmup_epochs,
        "safety_weights": {
            "anchor": args.anchor_weight,
            "margin": args.margin_weight,
            "harm": args.harm_weight,
            "cvar": args.cvar_weight,
            "repair": 0.0,
        },
...
def parse_args():
    ...
    parser.add_argument("--mode", required=True, choices=("noop", "curriculum"))
...
def main():
    ...
    if args.source_split.lower() != "train" or args.sample_count != 32:
        raise ValueError("v3v is fixed to the first 32 train-derived OOF names")
    if args.device == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("v3v requires the authorized CUDA runtime")
    if args.epochs <= 0 or args.risk_window <= 0 or not 0 < args.warmup_epochs < args.epochs or not 0.0 < args.cvar_fraction <= 1.0:
        raise ValueError("invalid fixed curriculum budget")
    ...
    source = {
        ...
        "objective": {
            "warmup_render_high_mse_weight": 1.0,
            "safety_phase_anchor_weight": args.anchor_weight,
            "safety_phase_margin_weight": args.margin_weight,
            "safety_phase_harm_weight": args.harm_weight,
            "safety_phase_cvar_weight": args.cvar_weight,
            "repair_weight": 0.0,
            "warmup_epochs": args.warmup_epochs,
        },
        "v3u_final_safety_reference": V3U_FINAL_SAFETY,
    ...
    if args.mode == "noop":
        ...
        closeout = run_curriculum(args, v3s, legacy, frozen, names, folds, device, output_dir)
