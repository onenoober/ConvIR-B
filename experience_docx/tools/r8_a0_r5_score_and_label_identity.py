...
ROUTE_ID = "haze4k_v5_r8_r5_score_and_label_identity_20260719"
OPERATION_ID = "R8_A0_FROZEN_R5_SCORE_AND_LABEL_IDENTITY_AUDIT"
...
R7_ROUTE_ID = "haze4k_v5_r7_r5_score_construction_identity_20260719"
R7_OPERATION_ID = "R7_A0_FROZEN_R5_SCORE_CONSTRUCTION_IDENTITY_AUDIT"
R7_RUN_ID = "r7-a0-r5-score-identity-r1"
CELLS = (
    "P0_POOLED_DC_ONLY",
...
def reconstruct(
    ...
    for row in candidate_rows:
        ...
        if record["severe_label"] not in (0, 1):
            raise InputInconclusive("saved severe label is not binary")
    ...
    for cell in CELLS:
        for fold in FOLDS:
            names = sorted({key[2] for key in candidates if key[0] == cell and key[1] == fold})
            for name in names:
                for action in range(2):
                    rows = [candidates[(cell, fold, name, operator, action)] for operator in OPERATORS]
                    expected = int(any(float(row["target_gain_db"]) <= SEVERE_GAIN for row in rows))
                    labels = {int(row["severe_label"]) for row in rows}
                    label_mismatches += labels != {expected}
    return candidates, [summary[key] for key in sorted(summary)], label_mismatches == 0, label_mismatches
            ...
            for name_index in range(192):
                ...
                for operator_index, operator in enumerate(OPERATORS):
                    for action_index_value, action in enumerate(ACTIONS):
                        discordant_severe = name_index % 53 == 0 and action_index_value == 0
                        target = (
                            (-0.25 if operator_index == 0 else 0.05)
                            if discordant_severe
                            else 0.2 - 0.001 * name_index - 0.01 * action_index_value - 0.002 * operator_index
                        )
                        seed_values = []
                        ...
                        candidates.append({
                            ...
                            "severe_label": str(int(discordant_severe)),
...
def contract(context_path: Path) -> None:
    ...
    reconstructed, summary, labels_exact, label_mismatches = reconstruct(per_seed, candidates)
    ...
    checks = {
        ...
        "any_operator_label_semantics": labels_exact and label_mismatches == 0,
...
def write_inconclusive(context: Any, reason: str, started_wall: float, started_cpu: float) -> None:
    ...
    for filename in (
        "r8_a0_contract_summary.json", "r8_a0_provenance_and_access.json",
        "r8_a0_input_identity.json", "r8_a0_downstream_sensitivity.json",
        "r8_a0_gate_summary.json",
    ):
        atomic_json(context.phase_output_path / filename, base)
    placeholder = [{"status": "inconclusive", "reason": reason}]
    write_csv(context.phase_output_path / "r8_a0_score_reconstruction_summary.csv", placeholder)
    write_csv(context.phase_output_path / "r8_a0_policy_replay_summary.csv", placeholder)
    atomic_json(context.phase_output_path / "r8_a0_resource_summary.json", {
    ...
    write_run_result(
        ...
        decision="R8_A0_INPUT_IDENTITY_INCONCLUSIVE_STOP", authorizes="NONE",
        details={
            "reason": reason, "r5_terminal_changed": False,
            "r6_terminal_changed": False, "r7_terminal_changed": False,
        },
    )
...
def run(context_path: Path) -> None:
    ...
    try:
        ...
        r7_closeout = json.loads(asset_path(context, "r7_closeout", kind="file").read_text(encoding="utf-8"))
        if (r7_closeout.get("route_id"), r7_closeout.get("operation_id"), r7_closeout.get("run_id")) != (
            R7_ROUTE_ID, R7_OPERATION_ID, R7_RUN_ID
        ) or (r7_closeout.get("state"), r7_closeout.get("decision"), r7_closeout.get("authorizes")) != (
            "COMPLETED_GATE_INCONCLUSIVE", "R7_A0_INPUT_IDENTITY_INCONCLUSIVE_STOP", "NONE"
        ) or r7_closeout.get("details", {}).get("reason") != "saved severe label disagrees with frozen target threshold":
            raise InputInconclusive("R7 triggering closeout identity mismatch")
        ...
        reconstructed, score_summary, target_label_exact, label_mismatches = reconstruct(
            per_seed_rows, candidate_rows,
        )
        ...
        if passes:
            ...
            decision = "R8_A0_SCORE_AND_LABEL_IDENTITY_PASS"
            authorizes = "R8_NEXT_ATTRIBUTION_CONTRACT_REVIEW_ONLY"
        else:
            state = "COMPLETED_GATE_FAIL"
            decision = "R8_A0_SCORE_LABEL_OR_POLICY_IDENTITY_FAIL_STOP"
            authorizes = "NONE"
        contract_summary = {
        ...
        gate_summary = {
            ...
            "any_operator_severe_label_mismatches": label_mismatches,
            "policy_max_abs_difference": policy_max_difference,
            "r5_terminal_changed": False, "r6_terminal_changed": False,
            "r7_terminal_changed": False,
        }
        atomic_json(context.phase_output_path / "r8_a0_contract_summary.json", contract_summary)
        atomic_json(context.phase_output_path / "r8_a0_provenance_and_access.json", provenance)
        atomic_json(context.phase_output_path / "r8_a0_input_identity.json", input_identity)
        write_csv(context.phase_output_path / "r8_a0_score_reconstruction_summary.csv", score_summary)
        write_csv(context.phase_output_path / "r8_a0_policy_replay_summary.csv", policy_summary)
        atomic_json(context.phase_output_path / "r8_a0_downstream_sensitivity.json", sensitivity)
        atomic_json(context.phase_output_path / "r8_a0_gate_summary.json", gate_summary)
        atomic_json(context.phase_output_path / "r8_a0_resource_summary.json", {
