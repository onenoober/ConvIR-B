...
on_error() {
  rc=$?
  printf 'FINALIZATION_MAIN_CONTROL_PLANE_FIX_CLOUD_FAILED line=%s command=%q rc=%s\n' \
    "$1" "$2" "$rc" >&2
  exit "$rc"
}
trap 'on_error "$LINENO" "$BASH_COMMAND"' ERR

branch=codex/finalization-main-control-plane-fix
base=4061fa31ec82874ab2ab47fad06e8db23922ff79
...
work=$(/usr/bin/mktemp -d /tmp/finalization-main-control-plane-fix.XXXXXX)

cleanup() {
  case "$work" in
    /tmp/finalization-main-control-plane-fix.*)
      /bin/rm -rf -- "$work"
      ;;
    *)
...
printf 'FINALIZATION_MAIN_CONTROL_PLANE_FIX_STAGE=checkout\n'
...
printf 'FINALIZATION_MAIN_CONTROL_PLANE_FIX_STAGE=compile\n'
"$python" -m py_compile \
  "$tools/convir_ops_mcp.py" \
  "$tests/test_convir_ops_mcp.py"

printf 'FINALIZATION_MAIN_CONTROL_PLANE_FIX_STAGE=focused_regression\n'
"$python" -m unittest -v test_convir_ops_mcp

printf 'FINALIZATION_MAIN_CONTROL_PLANE_FIX_STAGE=full_control_plane_regression\n'
...
printf 'FINALIZATION_MAIN_CONTROL_PLANE_FIX_CLOUD_OK candidate=%s tests=%s gpu_access=0 dataset_access=0 protected_data_access=0 experiment_launch=0\n' \
