...
on_error() {
  rc=$?
  printf 'SCHEMA3_ROUTE_READY_WRITER_FIX_CLOUD_FAILED line=%s command=%q rc=%s\n' \
    "$1" "$2" "$rc" >&2
  exit "$rc"
}
trap 'on_error "$LINENO" "$BASH_COMMAND"' ERR

branch=codex/schema3-route-ready-writer-fix
base=4704fcd245523c88f5f011c06d48f1c81eb837f2
...
work=$(/usr/bin/mktemp -d /tmp/schema3-route-ready-writer-fix.XXXXXX)

cleanup() {
  case "$work" in
    /tmp/schema3-route-ready-writer-fix.*)
      /bin/rm -rf -- "$work"
      ;;
    *)
...
printf 'SCHEMA3_ROUTE_READY_WRITER_FIX_STAGE=checkout\n'
...
printf 'SCHEMA3_ROUTE_READY_WRITER_FIX_STAGE=compile\n'
"$python" -m py_compile \
  "$tools/validate_route_ready.py" \
  "$tests/test_validate_route_ready.py"

printf 'SCHEMA3_ROUTE_READY_WRITER_FIX_STAGE=focused_regression\n'
"$python" -m unittest -v test_validate_route_ready

printf 'SCHEMA3_ROUTE_READY_WRITER_FIX_STAGE=full_control_plane_regression\n'
...
printf 'SCHEMA3_ROUTE_READY_WRITER_FIX_CLOUD_OK candidate=%s tests=%s gpu_access=0 dataset_access=0 protected_data_access=0 experiment_launch=0\n' \
