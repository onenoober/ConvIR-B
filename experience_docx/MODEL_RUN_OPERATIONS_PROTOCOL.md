...
Cloud GPU allocation should be dynamic by default. Probe current GPU memory
before each job launch, use the maximum currently free GPUs within the route's
parallelism cap, and keep the queue progressing when only part of the machine is
available. Do not pause a route simply because other users are occupying some
GPUs. Only wait when no candidate GPU is below the route's free-memory
threshold.
