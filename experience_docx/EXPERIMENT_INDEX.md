...
| [StrongExpert-GainMix](family_summaries/strongexpert_gainmix_family_summary.md) | v2.1 sealed C10 `riskcap36_no075` passed formal 5x3 but failed its single locked one-shot. v2.2 C8 proved multi-expert complementarity; C9/C10 found fixed `WD0375 = A0 + 0.375*(WDMamba-A0)` was strong without router training; its locked one-shot then passed with mean `+1.442090`, hard `+1.529767`, easy `+1.182529`, positive `0.938000`, dSSIM `+0.00247093`, severe `25.80/600`. v2.3 C11 showed a minimal WD0375/FS050 selector is stronger on train-derived formal replay, but locked one-shot positive/severe risk regressed, so it should not replace WD0375. v2.4 C12 direct WD0375 distillation failed. v2.5 C13 A0-frozen residual distillation was learnable but not screen-ready. v2.6 then completed train-derived alpha curves: WDMamba has a safe interval through alpha `0.50` (`WD0375` mean/hard/easy `+2.512202/+3.505615/+1.189484`, positive `0.973333`, severe `11/600`, while full alpha has easy `-1.048537`, severe `124/600`); FSNet+UDP also has a safe interval through alpha `0.75`; MB-TaylorFormerV2-L is safe only at small alpha `0.125`. v2.7 tested Haze4K-weight WD0375 on NH-HAZE without tuning and failed zero-shot transfer. v2.8 used NH-specific ConvIR-B/WDMamba weights and found WDMamba_NH alpha `1.0` worse than A0_NH (mean/hard/easy `-2.197751/-1.093919/-2.327606`, severe `52/55`), while inherited alpha `0.375` also failed (`-0.133584/+0.122348/-0.155758`, severe `26/55`). | v2.1 remains `LOCKED_ONE_SHOT_FAIL_NO_TUNING`; v2.2 WD0375 remains the default strong locked-pass Haze4K baseline; v2.3 C11 is `LOCKED_C11_SELECTOR_ONE_SHOT_RECORDED_DO_NOT_PROMOTE_OVER_WD0375`; v2.4 C12 is `C12_SCREEN_FAIL_KEEP_WD0375_TEACHER`; v2.5 C13 is `C13_INTERMEDIATE_GATE_FAIL_NO_B_SCREEN_LOCKED_UNTOUCHED`; v2.6 is `V26_ALPHA_CURVES_COMPLETED_LOCKED_UNTOUCHED`; v2.7 is `V27_NHHAZE_HAZE4K_WEIGHT_ZERO_SHOT_TRANSFER_NOT_SUPPORTED`; v2.8 is `V28_NHHAZE_OFFICIAL_WEIGHT_INHERITED_ALPHA_NOT_SUPPORTED`. The residual-shrinkage claim is strong for Haze4K train-derived WDMamba/FSNet and useful on Haze4K locked WD0375, but fixed-alpha cross-dataset transfer is not established. Future external-dataset claims need data-specific weight audits plus separate validation/OOF calibration or adaptive alpha. Locked output is evidence only and must not tune alpha, features, checkpoints, profiles, actions, experts, thresholds, or distillation targets. |

## Route Summary

| Route | Status | Main result | Decision | Card | Evidence root | Source after cleanup |
| --- | --- | --- | --- | --- | --- | --- |
| Haze4K v2.8 NH-HAZE Official-Weight Evaluation | Completed official-weight diagnostic; Haze4K locked untouched | v2.8 evaluated NH-HAZE-specific checkpoints: A0_NH `nhhaze-base.pkl` sha256 `aab6a72613781900a23c3922ad2dd60f6b0d563018e33ae75162bcf3338f5bac`, WDMamba_NH `NH_20.83.pth` sha256 `e097524f466b24f32843867911f9cbd47be8d51e61e5e345f8a27c22c73d5c5a`, ConvIR-B data arg `NHR`, and WDMamba `DENet(3,4)`. Final audit passed with `55` unique images, complete seven-alpha grid, and no NH-HAZE alpha tuning. WDMamba_NH endpoint alpha `1.0` was worse than A0_NH: mean/hard/easy dPSNR `-2.197751/-1.093919/-2.327606`, dSSIM `-0.06118223`, positive `0.054545`, severe `52/55`, worst `-4.730593`. Inherited alpha `0.375` also failed: `-0.133584/+0.122348/-0.155758`, dSSIM `-0.00598572`, positive `0.309091`, severe `26/55`. Alpha `0.125` was diagnostic-positive only (`+0.086285/+0.124708/+0.082227`, severe `0/55`) and must not be treated as selected without validation/OOF. | `V28_NHHAZE_OFFICIAL_WEIGHT_INHERITED_ALPHA_NOT_SUPPORTED`; do not claim Haze4K `alpha=0.375` transfers to NH-HAZE. Under this protocol, A0_NH is the stronger endpoint than WDMamba_NH; any NH alpha selection needs a separate validation/OOF route. | [card](experiment_cards/2026-06-16-haze4k-v2-8-nhhaze-official-weights.md) | [logs](experiment_logs/haze4k_v2_8_nhhaze_official_weights_20260616/) | `codex/haze4k-v2-8-nhhaze-official-weights` |
...
- Haze4K v2.8 completes the NH-HAZE official-weight diagnostic. It uses NH-specific ConvIR-B and WDMamba weights, fixes WDMamba to the strict-loading `DENet(3,4)` NH architecture, and confirms that WDMamba_NH is not a stronger endpoint than A0_NH under this local protocol: alpha `1.0` gives mean/hard/easy `-2.197751/-1.093919/-2.327606`, positive `0.054545`, severe `52/55`. The inherited Haze4K alpha `0.375` also fails (`-0.133584/+0.122348/-0.155758`, severe `26/55`). Alpha `0.125` is test-set diagnostic-positive only and cannot be promoted without separate validation/OOF calibration.
