...
| Haze4K DTA-v3 DAPC fine-tune | DTA-v3.2 CTDG-SafeMix audits planned | `convir-4090` preflight passed and deterministic eval shuffle was fixed. R0 routes failed. Tail-lite `wg18_base_s008_b14` kept true-depth mechanism surplus but failed SSIM/tail. DTA-v3.1 confirmed output semantics, found GT/oracle A did not rescue quality, light hinge only moved mean to `+0.025084` with worst still `76/600`, and same-fold risk selection fixed tail only at low coverage. The next no/low-training queue adds oracle coverage upper bound, alpha-blend sweep, GT-transmission/t-error audit, corrected selector metrics, and nested fold0 selector smoke before any C0-C4 training. | `PLANNED_CTDG_SAFEMIX_AUDITS_LOCKED_TEST_BLOCKED`; no 5-fold formal validation from B0-B4; locked test blocked. | [card](experiment_cards/2026-06-11-haze4k-dta-v3-dapc.md) | [logs](experiment_logs/haze4k_dta_v3_dapc_20260611/) | `github/codex/haze4k-dta-v3-dapc-finetune` |
