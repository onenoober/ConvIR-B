...
| [Depth-transmission adapters](family_summaries/dta_family_summary.md) | DTA-v2 is positive but not depth-attributed enough. DTA-v3 R0 failed; tail-lite zero-R0 depthDirect `wg18_base` now passes true-vs-zero/shuffle/normal mechanism thresholds, but SSIM/tail still fail. | Continue with DTA-v3.1 WG18-RiskSelect-AConsistent: output semantics audit, train/eval airlight consistency, OOF-style risk selection, and light tail/SSIM hinge scout around `wg18_base`; locked test remains blocked. |
...
| Haze4K DTA-v3 DAPC fine-tune | Mechanism-positive tail-lite wide-gate; DTA-v3.1 follow-up planned | `convir-4090` preflight passed and deterministic eval shuffle was fixed. R0 routes failed. Baseline zero-R0 depthDirect train=`invert` gave true-vs-zero surplus `+0.032286 dB` but tail failed. Strong wide-gate tailguard reduced worst regressions but collapsed mean/surplus. Tail-lite restored the mechanism: best `wg18_base_s008_b14` had mean `+0.024404`, hard `+0.006360`, true-vs-zero `+0.036631`, true-vs-shuffle `+0.032141`, true-vs-normal `+0.033084`, but dSSIM `-0.00002331`, pos ratio `0.6050`, and worst `76/600`. DTA-v3.1 adds airlight consistency audits, risk selection, and a light hinge scout without touching locked test. | `COMPLETED_MECHANISM_POSITIVE_TAIL_FAIL_TAILLITE_WIDE_GATE`; next state `PLANNED_CODE_READY_PENDING_CLOUD_RUN`; locked test blocked. | [card](experiment_cards/2026-06-11-haze4k-dta-v3-dapc.md) | [logs](experiment_logs/haze4k_dta_v3_dapc_20260611/) | `github/codex/haze4k-dta-v3-dapc-finetune` |
