...
- `github/codex/haze4k-dta-v3-7-u-tqs-mix`: active DTA U-TQS-Mix route;
  ...
  locked test remains blocked until a fixed train-derived confirmation seals a
  policy without post-test selection.
...
| [Depth-transmission adapters](family_summaries/dta_family_summary.md) | DTA-v3.6 proved the key bottleneck: conservative FDF/DTA actions have strict-pass oracle headroom, but hard reject loses too many positives. DTA-v3.7 U-TQS-Mix is the authorized mainline; D6 now gives the first staged deployable strict pass by adding actual output-difference / quality features to soft-shrink policy. | Do not continue v3.6 hard-reject threshold search as the main strategy; do not add broad router/residual capacity first; freeze a D6 strict row for train-derived confirmation, and keep locked test/raw D1 full `5x3` blocked until that confirmation authorizes them. |
...
| Haze4K DTA-v3.7 U-TQS-Mix | D6 output-difference deployable policy strict-passed on staged quick5full scope; no locked test yet | Phase A passed soft-oracle headroom; Phase C1 and D3 actual real-blend oracles passed. D1 integrated T/A/U candidates are mechanism-positive, and D3 confirmed actual D1 micro-shrink oracle headroom (`22/24` strict rows; best mean `+0.136562`, hard `+0.120391`, positive `0.7071`, worst `0/600`). D4/D5 deployable policies strict-failed, but D6 actual candidate-vs-A0 output-difference / quality features produced `8/522` strict rows. Best strict row: `outputdiff_plus_Q / micro_shrink / pred_gain`, mean `+0.078596`, hard `+0.085328`, dSSIM `+0.00001913`, positive `0.6583`, worst `46/600`, max outer worst `52/600`, true-vs-zero `+0.083899`. | `D6_OUTPUTDIFF_POLICY_STRICT_PASS_LOCKED_TEST_UNTOUCHED`; freeze D6 for train-derived fixed-policy confirmation. Do not run locked test or raw D1 full `5x3` until the fixed confirmation protocol authorizes the next scope. | [card](experiment_cards/2026-06-13-haze4k-dta-v3-7-u-tqs-mix.md) | [logs](experiment_logs/haze4k_dta_v3_7_u_tqs_mix_20260613/) | `github/codex/haze4k-dta-v3-7-u-tqs-mix` |
