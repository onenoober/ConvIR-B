...
- `github/codex/haze4k-dta-v3-7-u-tqs-mix`: active DTA U-TQS-Mix route;
  ...
  contention; after the D1 repair, dynamic probes should consider memory and
  utilization and launch one job per fresh probe to reduce GPU-race OOMs. D6
  output-difference / quality policy is the first staged deployable strict pass,
  and D7 fixed confirmation sealed the staged primary policy candidate; locked
  test remains blocked until a broader train-derived formal confirmation passes
  without post-test selection.
...
| [Depth-transmission adapters](family_summaries/dta_family_summary.md) | DTA-v3.6 proved the key bottleneck: conservative FDF/DTA actions have strict-pass oracle headroom, but hard reject loses too many positives. DTA-v3.7 U-TQS-Mix is the authorized mainline; D6/D7 now give the first staged deployable strict pass and fixed-policy confirmation using actual output-difference / quality features. | Do not continue v3.6 hard-reject threshold search as the main strategy; do not add broad router/residual capacity first; use the sealed D7 policy for broader train-derived formal confirmation, and keep locked test blocked until that formal confirmation passes. |

## Route Summary

| Route | Status | Main result | Decision | Card | Evidence root | Source after cleanup |
| --- | --- | --- | --- | --- | --- | --- |
| Haze4K DTA-v3.4 FDF-TSR fine-tune | Train-derived triage failed after one-shot test override | `E2=e2_tiny_residual` one-shot test was depth-attributed but failed absolute quality (`-0.014802` mean, `0.489` positive ratio, `257/1000` worst). Follow-up train-derived triage on `convir-5090` ran E1-E4 across folds `0/1` and seeds `3407/3411`; the best mean row was `e4_plus_film` (`+0.081446` mean, `+0.057209` hard), but all variants failed with positive ratio below `0.630` and worst around `116-128/600`. | `TRIAGE_GATE_FAIL_LOCKED_TEST_BLOCKED`; no formal 5-fold x 3-seed and no additional locked-test variant. | [card](experiment_cards/2026-06-12-haze4k-dta-v3-4-fdf-tsr.md) | [logs](experiment_logs/haze4k_dta_v3_dapc_20260611/) | `github/codex/haze4k-dta-v3-4-fdf-tsr-finetune` |
| Haze4K DTA-v3.5 FDF-RCS-Lite | Completed relaxed train-derived diagnostic | `convir-4090` queue completed L1-L5 across folds `0/1` and seeds `3407/3411`, repaired L0 A0 sanity under the five-GPU cap, and generated OOF action/oracle/nested-selector reports. L1 had best mean (`+0.071183`) and positive ratio (`0.6308`) but worst `82.5/600`; L2 had best all-image tail (`60.5/600`) with mean `+0.052706`; all non-L0 variants passed relaxed flow but failed strict triage. Oracle risk-coverage is strong, but nested selector coverage remains too low. | `COMPLETED_RELAXED_FLOW_PASS_STRICT_FAIL_SELECTOR_DIAGNOSTIC_LOCKED_TEST_BLOCKED`; no 5-fold formal validation and no locked Haze4K test. | [card](experiment_cards/2026-06-12-haze4k-dta-v3-5-fdf-rcs-lite.md) | [logs](experiment_logs/haze4k_dta_v3_5_fdf_rcs_lite_20260612/) | `github/codex/haze4k-dta-v3-5-fdf-rcs-lite` |
| Haze4K DTA-v3.6 HRCS | Formal train-derived validation completed; relaxed pass, strict fail | `convir-4090` formal queue completed L1/L2/L3 across folds `0..4` and seeds `3407/3411/2026`: `45` train runs, `180` depth-control evals, `45` aggregates, and `27000` formal OOF rows. Best deployable rows are relaxed-pass but strict-fail: L3 logistic `deployable_all` target `0.93` has coverage `0.8887`, mean `+0.065404`, positive ratio `0.5817`, worst `47.07/600`, max outer worst `59.33/600`; L2 logistic has best tail (`39.47/600`) but mean/positive ratio fail; L1 keeps high mean (`+0.075380`) but tail fails. Oracle L1/L3 high-coverage rows strict-pass, and oracle action bank is very strong, so the remaining bottleneck is deployable risk calibration/features. | `FORMAL_COMPLETED_RELAXED_PASS_STRICT_FAIL_FIXED_POLICY_READY_LOCKED_TEST_UNTOUCHED`; fixed relaxed one-shot policy, if authorized, is L3 logistic `deployable_all` target `0.93`; no promotion claim and no locked-test tuning. | [card](experiment_cards/2026-06-13-haze4k-dta-v3-6-hrcs.md) | [logs](experiment_logs/haze4k_dta_v3_6_hrcs_20260613/) | `github/codex/haze4k-dta-v3-6-hrcs` |
| Haze4K DTA-v3.7 U-TQS-Mix | D8 fixed formal confirmation running for sealed D7 primary policy; no locked test yet | Phase A passed soft-oracle headroom; Phase C1 and D3 actual real-blend oracles passed. D1 integrated T/A/U candidates are mechanism-positive, and D3 confirmed actual D1 micro-shrink oracle headroom (`22/24` strict rows; best mean `+0.136562`, hard `+0.120391`, positive `0.7071`, worst `0/600`). D4/D5 deployable policies strict-failed, but D6 output-difference / quality features produced `8/522` strict rows. D7 froze the top D6 rows and confirmed both: primary `outputdiff_plus_Q / micro_shrink / pred_gain / target=1.00` has mean `+0.078596`, hard `+0.085328`, dSSIM `+0.00001913`, positive `0.6583`, worst `46/600`, max outer worst `52/600`; backup `outputdiff_only` has worst `44/600`. | `D8_FIXED_FORMAL_RUNNING_LOCKED_TEST_UNTOUCHED`; D8 is active on convir-4090 with sealed primary policy only, architecture unchanged, partial sync `6/45` candidates launched and `0` failure markers; locked test remains blocked until D8 passes. | [card](experiment_cards/2026-06-13-haze4k-dta-v3-7-u-tqs-mix.md) | [logs](experiment_logs/haze4k_dta_v3_7_u_tqs_mix_20260613/) | `github/codex/haze4k-dta-v3-7-u-tqs-mix` |
...
| Haze4K DTA-v3 DAPC fine-tune | DTA-v3.3 RouterFusion triage failed | `convir-4090` triage completed from commit `bc28db8` with locked test untouched. D1/D2 improved mean/hard/surplus (`D1` mean `+0.067711`, hard `+0.069012`, true-vs-zero `+0.055552`; `D2` mean `+0.065951`, hard `+0.067494`, true-vs-zero `+0.058310`) but failed the tail gate with worst around `80/600`. D3 RouterFusion aggregate was weaker (mean `+0.032786`, positive ratio `0.5713`, true-vs-zero `+0.029598`, worst `75.5/600`) and diagnostics show suppressive/misrouted routing. | `TRIAGE_GATE_FAIL_LOCKED_TEST_BLOCKED`; no formal 5-fold x 3-seed and no locked test from D1/D2/D3. | [card](experiment_cards/2026-06-11-haze4k-dta-v3-dapc.md) | [logs](experiment_logs/haze4k_dta_v3_dapc_20260611/) | `github/codex/haze4k-dta-v3-dapc-finetune` |
...
| `experiment_logs/haze4k_dta_v3_dapc_20260611/` | 1077 | DTA-v3 DAPC convir-4090 setup/preflight, R0/conservative R0 evidence, baseline depthDirect, strong tailguard, tail-lite wide-gate, DTA-v3.1 airlight/risk diagnostics, DTA-v3.2 CTDG/SafeMix evidence, and DTA-v3.3 RouterFusion triage summaries, train/eval matrices, attribution CSV/JSON, deterministic shuffle audits, diagnostic reports, contact-sheet logs/manifests, launch scripts, status, README, and tmux transcripts. |
| `experiment_logs/haze4k_dta_v3_5_fdf_rcs_lite_20260612/` | 514 | DTA-v3.5 FDF-RCS-Lite convir-4090 scripts, status, relaxed train-derived triage summaries, per-run train/eval/aggregate logs, depth-control matrices, OOF action table, oracle risk-coverage curve, nested selector calibration reports, and evidence README. |
| `experiment_logs/haze4k_dta_v3_6_hrcs_20260613/` | 526 | DTA-v3.6 HRCS route README, Phase A and formal convir-4090 launch/status/logs, formal train/eval matrices, formal OOF action table, v3.5-style nested selector reports, high-coverage rejection curves, risk-feature AUC reports, reliability bins, selector error tables, action-bank oracle-vs-selector tables, best configs, and selector summary JSON. |
| `experiment_logs/haze4k_dta_v3_7_u_tqs_mix_20260613/` | 343 | DTA-v3.7 U-TQS-Mix route README, convir-4090 Phase A/B/B2/C1/D1-D7 scripts/status/logs, positive-loss budget, soft action-bank oracle grids, false reject/accept taxonomy, feature ablation AUC reports, T/A/Q/U preflight JSON, image quality feature table, TAU staged-screen evidence, D2-D6 nested policy reports, D6 output-difference feature-group summary, D7 fixed-policy confirmation summary, aggregate/action tables, and summaries. Large raw D6/D7 feature or selected-action tables remain cloud-only runtime artifacts by default. |
