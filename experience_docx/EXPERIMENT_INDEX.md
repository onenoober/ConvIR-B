...
| [StrongExpert-GainMix](family_summaries/strongexpert_gainmix_family_summary.md) | v2.1 sealed C10 `riskcap36_no075` passed formal 5x3 but failed its single locked one-shot: locked mean `+0.290049`, hard `+0.121385`; no locked-informed tuning is allowed. v2.2 C8-Mini proved multi-expert complementarity. v2.2 C9 then found fixed `WD0375 = A0 + 0.375*(WDMamba-A0)` is already strong: mean `+2.512202`, hard `+3.505615`, easy `+1.189484`, positive `0.973333`, severe `11/600`; C9-C group-min passed and C10 formal 5x3 table replay passed. | v2.1 decision remains `LOCKED_ONE_SHOT_FAIL_NO_TUNING`; v2.2 C10 decision is `C10_FORMAL_5X3_WD0375_PASS_AUTHORIZE_LOCKED_ONE_SHOT_REVIEW`. Locked Haze4K test remains untouched in C9/C10, and any locked replay must be one-shot fixed `WD0375` only with no locked-informed tuning or distillation target selection. |

## Route Summary

| Route | Status | Main result | Decision | Card | Evidence root | Source after cleanup |
| --- | --- | --- | --- | --- | --- | --- |
| Haze4K v2.2 C9 Fixed-Strong-Expert Baseline + C10 Formal WD0375 | Completed train-derived fixed-profile validation and formal table replay; locked untouched | C9 used C8 train-derived per-image tables only. Fixed `WD0375 = A0 + 0.375*(WDMamba-A0)` passed C9-A with mean `+2.512202 dB`, hard `+3.505615 dB`, easy `+1.189484 dB`, positive `0.973333`, dSSIM `+0.00167334`, and severe `11/600`. C9-B router was intentionally not run because fixed WD0375 passed. C9-C group-min shifted validation passed all fixed bins; worst-bin min mean/hard/positive and max severe were `+1.124603/+1.552796/0.900000/40 per 600`. C10 formal 5x3 table replay passed: fold-worst mean/hard/easy/positive/severe `+2.311024/+3.347410/+0.857374/0.948276/21.818182 per 600`. | `C10_FORMAL_5X3_WD0375_PASS_AUTHORIZE_LOCKED_ONE_SHOT_REVIEW`; this does not run locked test. A separate fixed one-shot locked replay may be reviewed for sealed WD0375 only; no locked-informed tuning or distillation target selection. | [card](experiment_cards/2026-06-15-haze4k-v2-2-c9-fixed-wdmamba-router.md) | [logs](experiment_logs/haze4k_v2_2_c9_fixed_wdmamba_router_20260615/) | `codex/haze4k-v2-2-c9-fixed-wdmamba-router` |
...
- Haze4K v2.2 C9 shows the simplest fixed strong-expert profile is enough: `WD0375` passed C9-A and C9-C without router training, and C10 formal 5x3 table replay passed with fold-worst mean/hard/easy/positive/severe `+2.311024/+3.347410/+0.857374/0.948276/21.818182 per 600`. Locked remains untouched; next action is at most a separately sealed one-shot locked replay review for WD0375.

| `experiment_logs/haze4k_v2_2_c9_fixed_wdmamba_router_20260615/` | completed | v2.2 C9/C10 route README, fixed WD0375 C9-A summaries, split/group-min shifted C9-C reports, bootstrap/Wilson bounds, C9 decision/summary, sealed WD0375 C10 formal 5x3 fold/group/summary tables, no-locked status, metric parity/provenance reports, runtime log, and command script. |
