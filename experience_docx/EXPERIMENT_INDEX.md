...
| Haze4K DTA-v3.7 U-TQS-Mix | Phase B2 enriched table policy strict fail; real blend next | Phase A passed soft-oracle headroom (`13/18` strict rows; best oracle mean `+0.143298`, positive ratio `0.6623`, worst `0/600`). Phase B current-feature TQS strict-failed. Phase B2 extracted quality/color/edge/dark-channel features for all `3000` train hazy images and reran nested TQS; strict pass remained `0`. Best `T_pred` still has low gain (mean `+0.015792`), while enriched `deployable_TQAU_action_all` improves mean to `+0.021754` and dSSIM `+0.00000301` but positive ratio collapses to `0.5128`. | `PHASE_B2_ENRICHED_TABLE_POLICY_STRICT_FAIL`; tail control is easy but deployable policy cannot preserve high positive action from tables alone. Continue to real soft-blend verification and integrated T/A/U supervised candidate training; do not return to hard-reject threshold search. | [card](experiment_cards/2026-06-13-haze4k-dta-v3-7-u-tqs-mix.md) | [logs](experiment_logs/haze4k_dta_v3_7_u_tqs_mix_20260613/) | `github/codex/haze4k-dta-v3-7-u-tqs-mix` |
