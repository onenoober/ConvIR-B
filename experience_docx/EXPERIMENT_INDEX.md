...
| [StrongExpert-GainMix](family_summaries/strongexpert_gainmix_family_summary.md) | v2.1 sealed C10 `riskcap36_no075` passed formal 5x3 but failed its single locked one-shot. v2.2 C8 proved multi-expert complementarity; C9/C10 found fixed `WD0375 = A0 + 0.375*(WDMamba-A0)` was strong without router training; its locked one-shot then passed with mean `+1.442090`, hard `+1.529767`, easy `+1.182529`, positive `0.938000`, dSSIM `+0.00247093`, severe `25.80/600`. v2.3 C11 showed a minimal WD0375/FS050 selector is stronger on train-derived formal replay, but locked one-shot positive/severe risk regressed, so it should not replace WD0375. v2.4 C12 direct WD0375 distillation failed. v2.5 C13 A0-frozen residual distillation was learnable but not screen-ready. v2.6 then completed train-derived alpha curves: WDMamba has a safe interval through alpha `0.50` (`WD0375` mean/hard/easy `+2.512202/+3.505615/+1.189484`, positive `0.973333`, severe `11/600`, while full alpha has easy `-1.048537`, severe `124/600`); FSNet+UDP also has a safe interval through alpha `0.75`; MB-TaylorFormerV2-L is safe only at small alpha `0.125`. v2.7 tested Haze4K-weight WD0375 on NH-HAZE without tuning and failed zero-shot transfer. v2.8 used NH-specific weights but was later audited as an all-55 mixed train/val/test aggregate, invalid for official NH-HAZE reproduction. v2.8b re-aggregated the existing per-image results on official test `51-55`: A0_NH `20.6636/0.7968`, WDMamba_NH `20.8307/0.8182`, inherited `alpha=0.375` diagnostic `+0.515796` mean dPSNR with severe `0/5`. | v2.1 remains `LOCKED_ONE_SHOT_FAIL_NO_TUNING`; v2.2 WD0375 remains the default strong locked-pass Haze4K baseline; v2.3 C11 is `LOCKED_C11_SELECTOR_ONE_SHOT_RECORDED_DO_NOT_PROMOTE_OVER_WD0375`; v2.4 C12 is `C12_SCREEN_FAIL_KEEP_WD0375_TEACHER`; v2.5 C13 is `C13_INTERMEDIATE_GATE_FAIL_NO_B_SCREEN_LOCKED_UNTOUCHED`; v2.6 is `V26_ALPHA_CURVES_COMPLETED_LOCKED_UNTOUCHED`; v2.7 is `V27_NHHAZE_HAZE4K_WEIGHT_ZERO_SHOT_TRANSFER_NOT_SUPPORTED`; v2.8 all-55 is `V28_ALL55_MIXED_SPLIT_REPRO_INVALID_FOR_OFFICIAL_BENCHMARK`; v2.8b is `V28B_NHHAZE_OFFICIAL_TEST51_55_REPRO_ALIGNED_WITH_EXPECTED_BASELINES`. The residual-shrinkage claim is strong for Haze4K train-derived WDMamba/FSNet and useful on Haze4K locked WD0375, but NH-HAZE alpha claims remain only five-image official-test diagnostics until a separate validation/OOF protocol selects alpha. Locked output is evidence only and must not tune alpha, features, checkpoints, profiles, actions, experts, thresholds, or distillation targets. |

## Route Summary

| Route | Status | Main result | Decision | Card | Evidence root | Source after cleanup |
| --- | --- | --- | --- | --- | --- | --- |
| Haze4K v2.8b NH-HAZE Official-Test Split Audit | Completed split audit; original all-55 result relabeled invalid for official benchmark | v2.8b re-aggregated existing v2.8 per-image metrics without new model runtime. The original v2.8 all-55 aggregate used NH-specific ConvIR-B/WDMamba weights but mixed official-style train `01-45`, validation `46-50`, and test `51-55`, explaining the inflated all-55 A0 `26.1047/0.9296`. Official test `51-55` gives A0_NH `20.6636/0.7968` and WDMamba_NH `20.8307/0.8182`, aligning with ConvIR-B README NH-HAZE base `20.66/0.802` and WDMamba checkpoint `NH_20.83.pth`. On `51-55`, inherited alpha `0.375` is diagnostic-positive (`+0.515796` mean dPSNR, dSSIM `+0.02203439`, positive `1.0`, severe `0/5`, worst `+0.078772`), but this is only a five-image post-run diagnostic. | `V28B_NHHAZE_OFFICIAL_TEST51_55_REPRO_ALIGNED_WITH_EXPECTED_BASELINES`; v2.8 all-55 is `V28_ALL55_MIXED_SPLIT_REPRO_INVALID_FOR_OFFICIAL_BENCHMARK`. Do not use all-55 numbers for official NH-HAZE claims, and do not select NH alpha without separate validation/OOF. | [card](experiment_cards/2026-06-16-haze4k-v2-8-nhhaze-official-weights.md) | [audit](experiment_logs/haze4k_v2_8b_nhhaze_official_test_split_audit_20260616/), [v2.8 logs](experiment_logs/haze4k_v2_8_nhhaze_official_weights_20260616/) | `codex/haze4k-v2-8-nhhaze-official-weights` |
...
- Haze4K v2.8 was audited after an anomalously high NH-HAZE reproduction. The run used NH-specific ConvIR-B and WDMamba weights, but the flat local dataset was evaluated as all `55` pairs, mixing official-style train `01-45`, validation `46-50`, and test `51-55`. That all-55 aggregate is invalid for official benchmark claims. v2.8b re-aggregated the existing per-image metrics on official test `51-55`, giving A0_NH `20.6636/0.7968` and WDMamba_NH `20.8307/0.8182`, which aligns with ConvIR-B README NH-HAZE base `20.66/0.802` and WDMamba checkpoint `NH_20.83.pth`. Inherited `alpha=0.375` is positive on `51-55` (`+0.515796`, severe `0/5`) but remains a five-image post-run diagnostic, not a selected NH-HAZE alpha.

| `experiment_logs/haze4k_v2_8b_nhhaze_official_test_split_audit_20260616/` | completed | v2.8b split audit summary, decision, absolute metrics for all/train/val/test, official-test alpha grid, and official-test per-image table. It relabels v2.8 all-55 as mixed-split invalid for official NH-HAZE benchmark reproduction. |
| `experiment_logs/haze4k_v2_8_nhhaze_official_weights_20260616/` | completed | v2.8 NH-HAZE official-weight route README, final decision, final audit JSON, preflight smoke evidence, parallel run and monitor scripts, status files, runtime logs, shard manifests/per-image tables, alpha grid, compact alpha comparison, group metrics/group-min tables, and summary JSON. |
