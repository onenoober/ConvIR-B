...
| [StrongExpert-GainMix](family_summaries/strongexpert_gainmix_family_summary.md) | v2.0 C4 formal 5x3 passed the screen gate but failed the strong formal target (`hard=+0.256389 dB`, `positive=0.68`). v2.1 is opened on `codex/haze4k-v2-1-segmix-multialpha-local` to run C5 failure forensic, C6 exact risk-bounded multi-alpha OOF router, and C7 patch-level alpha oracle. | Locked test and distillation remain blocked. Continue only with train-derived hard/positive improvements; if C6 strong-candidate gate passes, proceed to C9 shifted-strong validation before any C10 formal replay. |

## Route Summary

| Route | Status | Main result | Decision | Card | Evidence root | Source after cleanup |
| --- | --- | --- | --- | --- | --- | --- |
| Haze4K v2.1 SEG-Mix Multi-Alpha / Local-Alpha | Planned cloud C5-C7; locked blocked | Opens the next StrongExpert-GainMix stage after v2.0 C4. C5 will decompose the hard/positive gap without tuning. C6 will render exact alphas `0.125/0.25/0.375/0.50/0.75` and run a train-only risk-bounded multi-alpha OOF router. C7 will compute patch-level alpha oracle capacity from the same render pass. | `PLANNED_C5_C6_C7_NO_LOCKED`; C6 strong-candidate pass is required before C9 shifted-strong validation. | [card](experiment_cards/2026-06-15-haze4k-v2-1-segmix-multialpha-local.md) | [logs](experiment_logs/haze4k_v2_1_segmix_multialpha_local_20260615/) | `github/codex/haze4k-v2-1-segmix-multialpha-local` |
