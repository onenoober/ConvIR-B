...
- `github/codex/haze4k-dta-v3-7-u-tqs-mix`: active DTA U-TQS-Mix route;
  ...
  D7 fixed confirmation sealed the staged primary policy candidate, and D8
  fixed formal confirmation strict-passed the broader train-derived scope. The
  next permitted step is exactly one fixed locked-test confirmation, with no
  post-test tuning.
...
| [Depth-transmission adapters](family_summaries/dta_family_summary.md) | DTA-v3.6 proved the key bottleneck: conservative FDF/DTA actions have strict-pass oracle headroom, but hard reject loses too many positives. DTA-v3.7 U-TQS-Mix is the authorized mainline; D6/D7/D8 give the first deployable strict pass, fixed-policy confirmation, and broader train-derived formal strict pass using actual output-difference / quality features. | Do not continue v3.6 hard-reject threshold search as the main strategy; do not add broad router/residual capacity first; use the sealed D8 primary policy for exactly one locked Haze4K confirmation, and do not tune anything from locked-test feedback. |
...
| Haze4K DTA-v3.7 U-TQS-Mix | D8 fixed formal confirmation strict-passed; locked test untouched | Phase A passed soft-oracle headroom; Phase C1 and D3 actual real-blend oracles passed. D1 integrated T/A/U candidates are mechanism-positive, and D3 confirmed actual D1 micro-shrink oracle headroom (`22/24` strict rows; best mean `+0.136562`, hard `+0.120391`, positive `0.7071`, worst `0/600`). D4/D5 deployable policies strict-failed, D6 output-difference / quality features produced `8/522` strict rows, D7 froze the top rows, and D8 strict-passed the sealed primary policy on `3 variants x 5 folds x 3 seeds`: mean `+0.078297`, hard `+0.085281`, dSSIM `+0.00001904`, positive `0.65875`, worst `46.5/600`, max outer worst `52/600`, true-vs-zero `+0.083620`. | `D8_FIXED_FORMAL_STRICT_PASS_LOCKED_TEST_UNTOUCHED`; one fixed one-shot locked Haze4K confirmation is now eligible when `convir-4090` is reachable, with no threshold/feature/action/checkpoint/code tuning from locked feedback. | [card](experiment_cards/2026-06-13-haze4k-dta-v3-7-u-tqs-mix.md) | [logs](experiment_logs/haze4k_dta_v3_7_u_tqs_mix_20260613/) | `github/codex/haze4k-dta-v3-7-u-tqs-mix` |
...
| `experiment_logs/haze4k_dta_v3_7_u_tqs_mix_20260613/` | 350+ | DTA-v3.7 U-TQS-Mix route README, convir-4090 Phase A/B/B2/C1/D1-D8 scripts/status/logs, positive-loss budget, soft action-bank oracle grids, false reject/accept taxonomy, feature ablation AUC reports, T/A/Q/U preflight JSON, image quality feature table, TAU staged-screen evidence, D2-D6 nested policy reports, D6 output-difference feature-group summary, D7 fixed-policy confirmation summary, and D8 fixed formal closeout summary/recovered aggregate. Large raw D6/D7/D8 feature or selected-action tables remain cloud-only runtime artifacts by default. |
