...
| [StrongExpert-GainMix](family_summaries/strongexpert_gainmix_family_summary.md) | v2.1 C5-C7 completed. C6 exact multi-alpha OOF passes screen with mean `+0.422839`, hard `+0.479300`, easy `+0.447305`, severe `46.0/600`, but misses strong-candidate positive ratio (`0.698333` < `0.70`). C7 risk-capped patch oracle is strong (mean `+0.876923`, hard `+0.756983`, positive `0.995000`). | Locked test and distillation remain blocked. C7b local-alpha prototype is a near miss: mean `+0.376111`, hard `+0.360949`, positive `0.793333`, but severe `50.0/600` exceeds the `48/600` limit. C7c severe-risk tightening passes the strong OOF gate with `riskcap42_no075`: mean `+0.354799`, hard `+0.322247`, positive `0.790000`, severe `43.0/600`. C9 shifted stress passed 8/9 dimensions but failed `diff_signed_q4` with severe `50.0/600`; C10 remains blocked. C9b fixed conservative `riskcap36_no075` stress passed all dimensions (mean `+0.341530`, hard `+0.310932`, positive `0.786667`, severe `37.0/600`). Proceed to C10 formal 5x3; locked remains blocked until C10 passes. |

## Route Summary

| Route | Status | Main result | Decision | Card | Evidence root | Source after cleanup |
| --- | --- | --- | --- | --- | --- | --- |
| Haze4K v2.1 SEG-Mix Multi-Alpha / Local-Alpha C5-C7 | C6 screen pass, strong target not yet; C7 local-alpha signal | C5 forensic completed with no tuning. C6 exact multi-alpha OOF improved C4 hard from `+0.256389` to `+0.479300` and mean to `+0.422839`, while easy stayed positive and severe stayed `46.0/600`; strong-candidate gate failed only on positive ratio `0.698333`. C7 patch oracle shows large local-alpha capacity, with risk-capped mean `+0.876923` and hard `+0.756983`. | `C6_MULTIALPHA_OOF_SCREEN_PASS_STRONG_TARGET_NOT_YET_START_C7_C8__C7_PATCH_ALPHA_ORACLE_STRONG_SIGNAL_START_LOCAL_ALPHA`; proceed to C7b local-alpha prototype, no locked. | [card](experiment_cards/2026-06-15-haze4k-v2-1-segmix-multialpha-local.md) | [logs](experiment_logs/haze4k_v2_1_segmix_multialpha_local_20260615/) | `github/codex/haze4k-v2-1-segmix-multialpha-local` |
