...
| Haze4K DTA-v3 DAPC fine-tune | Mechanism-positive tail-lite wide-gate; tail/SSIM fail | `convir-4090` preflight passed and deterministic eval shuffle was fixed. R0 routes failed. Baseline zero-R0 depthDirect train=`invert` gave true-vs-zero surplus `+0.032286 dB` but tail failed. Strong wide-gate tailguard reduced worst regressions but collapsed mean/surplus. Tail-lite restored the mechanism: best `wg18_base_s008_b14` had mean `+0.024404`, hard `+0.006360`, true-vs-zero `+0.036631`, true-vs-shuffle `+0.032141`, true-vs-normal `+0.033084`, but dSSIM `-0.00002331`, pos ratio `0.6050`, and worst `76/600`. | `COMPLETED_MECHANISM_POSITIVE_TAIL_FAIL_TAILLITE_WIDE_GATE`; continue only with mild tail/risk selection around `wg18_base`; locked test blocked. | [card](experiment_cards/2026-06-11-haze4k-dta-v3-dapc.md) | [logs](experiment_logs/haze4k_dta_v3_dapc_20260611/) | `github/codex/haze4k-dta-v3-dapc-finetune` |
