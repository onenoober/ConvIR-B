...
| [StrongExpert-GainMix](family_summaries/strongexpert_gainmix_family_summary.md) | v2.1 sealed C10 `riskcap36_no075` passed formal 5x3 but failed its single locked one-shot. v2.2 C8 proved multi-expert complementarity; C9/C10 found fixed `WD0375 = A0 + 0.375*(WDMamba-A0)` was strong without router training; its locked one-shot then passed with mean `+1.442090`, hard `+1.529767`, easy `+1.182529`, positive `0.938000`, dSSIM `+0.00247093`, severe `25.80/600`. v2.3 C11 showed a minimal WD0375/FS050 selector is stronger on train-derived formal replay, and C11-E sealed the selector with mean/hard/easy `+2.828078/+3.548762/+1.953362`, positive `0.985000`, severe `6/600`; however its locked one-shot reached mean/hard/easy `+1.449078/+1.558683/+1.248566` with positive `0.896000` and severe `48.60/600`, so it should not replace WD0375. v2.4 C12 then screened direct WD0375 distillation and failed: every tested checkpoint was negative on the held-out train-derived validation, so the fixed WD0375 teacher remains the deployment default. v2.5 C13 reframed the compression as A0-frozen residual learning and confirmed the line is learnable but not screen-ready: A2-A5 found real positive movement, but no candidate passed the written quick gate, with the best safety-leaning row still short on mean/hard and the strongest hard-gain rows failing positive/severe. | v2.1 remains `LOCKED_ONE_SHOT_FAIL_NO_TUNING`; v2.2 WD0375 remains the default strong locked-pass baseline; v2.3 C11 is `LOCKED_C11_SELECTOR_ONE_SHOT_RECORDED_DO_NOT_PROMOTE_OVER_WD0375`; v2.4 C12 is `C12_SCREEN_FAIL_KEEP_WD0375_TEACHER`; v2.5 C13 is `C13_INTERMEDIATE_GATE_FAIL_NO_B_SCREEN_LOCKED_UNTOUCHED`. Locked output is evidence only and must not tune alpha, features, checkpoints, profiles, actions, experts, thresholds, or distillation targets; distillation or residual compression needs a separate risk/utility-conditioned reopen. |
...
| Haze4K v2.5 C13 A0-Frozen Residual Distillation | Completed C13-0/A2-A5 and C13-F diagnostics; no B-screen | C13 reframed WD0375 compression as A0-frozen residual learning. C13-0 audit confirmed exact A0 parity for model_0 and that locked data stayed untouched. A2-A5 showed the current global residual / scalar family is learnable but not screen-ready: full-600 `c13a4_scale050` reached mean/hard/easy `+0.361713/+0.564971/+0.119759`, but positive `0.696667` and severe `115/600` failed; `a5_a4sweep_s030` kept severe at `57/600` but missed hard/positive with mean/hard/easy `+0.253058/+0.343011/+0.155960`, positive `0.743333`. C13-F oracle diagnostics then isolated the bottleneck: per-image scale oracle passed with mean/hard/easy `+0.554817/+0.730784/+0.338369`, positive `0.961667`, severe `0/600`; patch scale oracle passed with `+0.750215/+0.818064/+0.624435`, positive `1.0`, severe `0/600`; LL-only oracle passed with `+0.554681/+0.730671/+0.338372`, positive `0.961667`, severe `0/600`. | `C13_INTERMEDIATE_GATE_FAIL_NO_B_SCREEN_LOCKED_UNTOUCHED`; do not continue to C13-B from the current adapter/loss family, and do not touch locked Haze4K. Next compression route should be risk/utility-conditioned frequency residual distillation with image/patch/band gating and A0 safety anchoring. | [card](experiment_cards/2026-06-15-haze4k-v2-5-c13-a0-frozen-residual-distillation.md) | [logs](experiment_logs/haze4k_v2_5_c13_a0_frozen_residual_distill_20260615/) | `codex/haze4k-v2-5-c13-a0-frozen-residual-distill` |
