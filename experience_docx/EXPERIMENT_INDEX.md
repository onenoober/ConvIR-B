...
- `github/codex/haze4k-dta-v3-7-u-tqs-mix`: active DTA U-TQS-Mix route;
  ...
  utilization and launch one job per fresh probe to reduce GPU-race OOMs.
...
| [Depth-transmission adapters](family_summaries/dta_family_summary.md) | DTA-v3.6 proved the key bottleneck: conservative FDF/DTA actions have strict-pass oracle headroom, but hard reject loses too many positives. DTA-v3.7 U-TQS-Mix is now the authorized mainline: A0-preserving soft action-bank/shrink-mix plus transmission/airlight/quality/uncertainty gain-risk modeling. | Do not continue v3.6 hard-reject threshold search as the main strategy; do not add broad router/residual capacity first; run v3.7 Phase A table-only soft-oracle diagnostics, then parallelize TQS policy and real soft-blend/integrated-head validation on train-derived folds only. |
...
| Haze4K DTA-v3.7 U-TQS-Mix | D2 TAU soft-shrink policy completed; no formal promotion yet | Phase A passed soft-oracle headroom; Phase B/B2 deployable table policies strict-failed; Phase C1 real soft-blend oracle passed. Phase D1 integrated T/A/U staged screen was mechanism-positive but raw candidates failed tail/mean gates. Phase D2 then used the D1 table for a TAU soft-shrink policy: oracle headroom remains strong (`10/12` oracle rows strict-pass; best oracle mean `+0.136286`, hard `+0.120366`, worst `0/600`), but deployable nested policy still strict-fails (`T_pred` best mean `+0.006518`, hard `+0.007339`, positive `0.6400`, worst `0/600`). | `D2_TAU_SHRINK_POLICY_STRICT_FAIL_ORACLE_PASS_LOCKED_TEST_UNTOUCHED`; do not run raw D1 full `5x3` or locked test. Continue with rendered D1 soft-blend verification and stronger high-positive utility modeling; do not return to hard-reject threshold search. | [card](experiment_cards/2026-06-13-haze4k-dta-v3-7-u-tqs-mix.md) | [logs](experiment_logs/haze4k_dta_v3_7_u_tqs_mix_20260613/) | `github/codex/haze4k-dta-v3-7-u-tqs-mix` |
...
| `experiment_logs/haze4k_dta_v3_7_u_tqs_mix_20260613/` | 25 | DTA-v3.7 U-TQS-Mix route README, convir-4090 Phase A/B/B2 scripts/status/logs, positive-loss budget, soft action-bank oracle grid, false reject/accept taxonomy, feature ablation AUC report, T/A/Q/U preflight JSON, Phase A summary, image quality feature table, nested/enriched TQS policy reports, aggregate/action tables, and summaries. |
