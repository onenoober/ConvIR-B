...
| Haze4K v2.16 NoPost Wavelet Lowband Decoder | T0/T1/T2 diagnostics passed; training pending separate WLDB-A protocol | v2.16 closed the risk-selector-first NoPost-PBC-FGA line and switched to lowband-capacity-first diagnosis from the official architecture anchor. T0 found WD0375 severe-risk is decoupled from the new lowband objective: severe-vs-lowband-need Jaccard `0.027917`, severe-vs-A0-hard Jaccard `0.000000`. T1 train-derived RGB wavelet oracle found strong LL headroom on `2400` rows: all-image LL oracle mean dPSNR `+14.998694`, hard-bottom25 `+18.939359`, easy-top25 `+11.853745`, severe LL-oracle regressions `0`, lowband-need rate `1.0`. T2 added a zero-init WLDB contract after `Decoder[2]` and before `feat_extract[5]`; forbidden symbol hits `0`, identity max abs diff `1.788e-7` on `32` train-derived samples, official checkpoint loaded `602` keys with `6` WLDB-only misses by design. | `T2_CONTRACT_IDENTITY_PASS_TRAINING_STILL_BLOCKED_PENDING_REVIEW`; diagnostics authorize WLDB-A training protocol design, not immediate training or locked-test use. Locked Haze4K test untouched. | [card](experiment_cards/2026-07-03-haze4k-v2-16-nopost-wavelet-lowband-decoder.md) | [logs](experiment_logs/haze4k_v2_16_nopost_wavelet_lowband_decoder_20260703/) | `github/codex/haze4k-v2-16-nopost-wavelet-lowband-decoder` |
...
| `experiment_logs/haze4k_v2_16_nopost_wavelet_lowband_decoder_20260703/` | compact sync | v2.16 NoPost-WLDB no-training T0/T1/T2 diagnostics: target-decoupling overlap matrices, RGB wavelet LL/HF oracle summaries, per-image band-delta table, group report, contract/forbidden-symbol audit, forward signature, zero-init identity summaries, logs, status, and decision files. Checkpoints, datasets, images, arrays, and raw inference outputs are excluded. |
...
- Haze4K v2.16 deliberately stopped the risk-selector-first NoPost-PBC-FGA line and tested a lowband-capacity-first WLDB route. T0 showed WD0375 severe-risk is not a good proxy for lowband need; T1 showed very large train-derived LL-oracle headroom; T2 proved a zero-init WLDB insertion after `Decoder[2]` and before `feat_extract[5]` is source-clean and A0-equivalent. This authorizes a separate WLDB-A training protocol design only; it does not authorize direct training or locked-test use without the new protocol.
