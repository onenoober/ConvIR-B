...
| DTA-v2 calibrated confidence-gated adapter | Adapter-only five-fold done; multi-seed next | Adapter-only five-fold OOF20 completed for invert/normal/shuffle/zero over 3000 train-derived validation images: mean dPSNR `+0.0882/+0.0879/+0.0765/+0.0728`, hard `+0.0700/+0.0763/+0.0633/+0.0595`, easy `+0.0716/+0.0697/+0.0649/+0.0630`. Adapter-neighbors fold0 was preservation-negative. The five-fold signal is positive, but depth attribution is weak because zero/shuffle retain most gains and raw `normal` nearly ties `invert`; SSIM is slightly negative and worst regressions remain `421-479/3000`. | `IN_PROGRESS_ADAPTER_ONLY_FIVEFOLD_DONE_MULTI_SEED_NEXT`; run all real audit/preflight/training/eval on `convir-4090`, sync text evidence after every stage, run multi-seed adapter-only controls, and keep locked test blocked until one fixed internal-selected config. | [card](experiment_cards/2026-06-11-haze4k-dta-v2-calibrated.md) | [logs](experiment_logs/haze4k_dta_v2_calibrated_20260611/) | `github/codex/haze4k-dta-v2-calibrated` |
...
| `experiment_logs/haze4k_dta_v2_calibrated_20260611/` | in progress | DTA-v2 route README, convir-4090 setup/audit/preflight/train-eval scripts, synced depth-transmission audit, OOF splits, preflight, fold0 adapter-neighbors/control, adapter-only five-fold OOF evidence, and five-fold aggregate bootstrap/Wilcoxon report. |
