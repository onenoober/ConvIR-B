...
- `github/codex/haze4k-dta-v2-calibrated`: active calibrated confidence-gated
  DTA-v2 route for Innovation 1 on `convir-4090`; reopened from low-gate
  diagnosis with depth/transmission audit, supervised transmission, physics,
  preservation, and depth-control experiments.
...
| [DTA depth-guided transmission adapter](family_summaries/dta_family_summary.md) | Low-gate DTA is complete diagnostic/no-promotion; DTA-v2 CalGate has synced audit/preflight, adapter-neighbors fold0, adapter-only five-fold OOF, and multi-seed OOF evidence. Adapter-only is reproducibly positive internally, but depth attribution, SSIM, and tail-regression gates still fail. | Treat current CalGate as completed internal diagnostic/no-promotion; locked Haze4K test remains blocked unless a new mechanism passes train-derived gates. |
...
| DTA-v2 calibrated confidence-gated adapter | Completed multi-seed OOF/no locked test | Adapter-only multi-seed OOF20 across seeds `3407/3411/3413` completed for invert/normal/shuffle/zero over `9000` train-derived validation predictions per mode: mean dPSNR `+0.0887/+0.0888/+0.0775/+0.0740`, hard `+0.0665/+0.0705/+0.0583/+0.0571`, easy `+0.0736/+0.0719/+0.0669/+0.0638`. Adapter-neighbors fold0 was preservation-negative. The signal is stable and positive, but depth attribution is weak because `normal` slightly beats calibrated `invert` and zero/shuffle retain most gains; SSIM is slightly negative and worst regressions remain `1169-1357/9000`. | `COMPLETED_ADAPTER_ONLY_MULTI_SEED_OOF_NO_LOCKED_TEST`; keep locked Haze4K test blocked and treat current CalGate as positive internal diagnostic/no-promotion unless a new mechanism passes train-derived gates. | [card](experiment_cards/2026-06-11-haze4k-dta-v2-calibrated.md) | [logs](experiment_logs/haze4k_dta_v2_calibrated_20260611/) | `github/codex/haze4k-dta-v2-calibrated` |
...
| `experiment_logs/haze4k_dta_v2_calibrated_20260611/` | 200+ | DTA-v2 route README, convir-4090 setup/audit/preflight/train-eval scripts, synced depth-transmission audit, OOF splits, preflight, fold0 adapter-neighbors/control, adapter-only five-fold OOF evidence, multi-seed train/eval/tpred logs, compare/tpred JSON/CSV artifacts, and aggregate bootstrap/Wilcoxon reports. |
...
- DTA-v2 on `codex/haze4k-dta-v2-calibrated` completed the predeclared
  audit/preflight, adapter-only controls, adapter-neighbors fold0 diagnostic,
  five-fold OOF, and multi-seed OOF aggregate. Adapter-only is reproducibly
  positive internally but not locked-test-ready because attribution, SSIM, and
  tail-risk gates remain unresolved; do not use locked Haze4K test for this
