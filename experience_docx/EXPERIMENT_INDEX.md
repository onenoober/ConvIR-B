...
| [StrongExpert-GainMix](family_summaries/strongexpert_gainmix_family_summary.md) | C0 opened strong A0/FullUDP oracle capacity while global FullUDP stayed unsafe. C2d alpha-shrink (`alpha=0.25`) passed strict OOF and C3 shifted validation passed all 8 stress dimensions. C4 formal 5x3 passed the screen gate for all seeds (mean `+0.330556 dB`, easy `+0.473005 dB`, dSSIM `+0.00023663`, severe `37/600`) but failed the strong formal target because hard bottom-25 was only `+0.256389 dB` and positive ratio was `0.68`. | Locked test remains blocked for this C2d policy. Reopen with stronger hard-gain/positive-coverage router features, patch-level alpha, or additional compatible experts; do not run locked one-shot. |
