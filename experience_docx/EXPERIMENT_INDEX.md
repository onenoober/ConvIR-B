...
| DTA-v2 calibrated confidence-gated adapter | Adapter-only OOF20 done; adapter-neighbors next | Fold0 adapter-only OOF20 completed for invert/normal/shuffle/zero on 600 internal-val images: mean dPSNR `+0.1069/+0.1060/+0.0984/+0.0955`, hard `+0.0992/+0.1047/+0.0956/+0.0918`. Gains are positive but not uniquely depth-attributed, so continue the predeclared adapter-neighbors test. | `IN_PROGRESS_ADAPTER_ONLY_OOF20_DONE_NEIGHBORS_NEXT`; run all real audit/preflight/training/eval on `convir-4090`, sync text evidence after every stage, and keep locked test blocked until one fixed internal-selected config. | [card](experiment_cards/2026-06-11-haze4k-dta-v2-calibrated.md) | [logs](experiment_logs/haze4k_dta_v2_calibrated_20260611/) | `github/codex/haze4k-dta-v2-calibrated` |
