...
| [NoPost feature lowband routes](family_summaries/nopost_lowband_family_summary.md) | v2.21 fixed OOF safety replay passed and v2.22 gated-lowband N3 microfit passed stability review, but v2.23 real OOF train screen failed mean/hard and tail/strong safety gates. Current bottleneck is train-time safe-action/no-op control, not replay-only gate availability. | Reopen only with a materially different train-time controller or objective that predeclares how it prevents v2.23 severe/strong regressions while recovering mean/hard gain; do not simply increase epochs, samples, or loss weights under v2.23. |
...
| NoPost safety-calibrated lowband replay v2.21 | Completed gate-pass review-only | Fixed OOF safety replay selected `V221_risk_temperature_gamma0p50` and repaired v2.20 raw-action tail damage: mean `+2.2270 dB`, hard `+4.3031 dB`, easy `+0.7403 dB`, p05 `-0.0025 dB`, CVaR5 `-0.2089 dB`, severe `1.79%`, strong-reference regression `4.83%`, and fold tail pass `5/5`. Factorial audit passed for predicted action + predicted gate; shuffled-risk control failed. | `V221_P1_REPLAY_GATE_PASS_REVIEW_N3_MICROFIT_ROUTE_CARD_NO_TRAINING_LAUNCHED`; route authorizes only separate N3 microfit route-card review. No training launched; locked test untouched. | [card](experiment_cards/2026-07-04-haze4k-v2-21-nopost-safety-calibrated-lowband-replay.md) | [logs](experiment_logs/haze4k_v2_21_nopost_safety_calibrated_lowband_replay_20260704/) | `codex/haze4k-v2-21-nopost-safety-calibrated-lowband-replay` |
| Haze4K v2.22 NoPost gated lowband N3 microfit | Completed review-only | v2.21 `gamma0p50` replay was converted into a trainable internal gated lowband route. P0 strict partial-load/zero-init passed. Adapter-only microfit16/64/256 all passed stability gates with nonzero action and nondegenerate unsafe/no-op probabilities near `0.18`; microfit256 mean `-0.0029 dB`, hard bottom25 `+0.0107 dB`, p05 `-0.2146 dB`, severe `5.47%`. | `V222_N3_MICROFIT_PASS_REVIEW_ONLY_NO_LOCKED_TEST`; this is a training-stability pass, not a quality win. It authorizes only a separate OOF train-derived route review; locked test remains blocked. | [card](experiment_cards/2026-07-04-haze4k-v2-22-nopost-gated-lowband-n3-microfit.md) | [logs](experiment_logs/haze4k_v2_22_nopost_gated_lowband_n3_microfit_20260704/) | `codex/haze4k-v2-22-nopost-gated-lowband-n3-microfit` |
| Haze4K v2.23 NoPost OOF gated lowband train screen | Completed normal gate pause | Adapter-only train-derived OOF screen completed folds `0,1,2` with `384` train and `160` heldout eval samples per fold. Aggregate OOF mean was `+0.0367 dB`, hard bottom25 `+0.0227 dB`, easy top25 `+0.0165 dB`, p05 `-0.3528 dB`, CVaR5 `-0.4782 dB`, severe `14.79%`, strong-reference regression `32.08%`, and fold tail pass `1/3`. | `V223_OOF_SCREEN_GATE_FAIL_NORMAL_PAUSE_NO_LOCKED_TEST`; useful negative screen. Do not expand epochs/samples/loss weights under this run id; locked test remains blocked. | [card](experiment_cards/2026-07-04-haze4k-v2-23-nopost-oof-gated-lowband-train.md) | [logs](experiment_logs/haze4k_v2_23_nopost_oof_gated_lowband_train_20260704/) | `codex/haze4k-v2-23-nopost-oof-gated-lowband-train` |
...
| `experiment_logs/haze4k_v2_21_nopost_safety_calibrated_lowband_replay_20260704/` | compact | v2.21 replay-only safety calibration, fixed OOF threshold/gamma sweep, factorial action/gate audit, calibration summaries, tail residual reports, status, and README. Raw per-image tables remain on cloud. |
| `experiment_logs/haze4k_v2_22_nopost_gated_lowband_n3_microfit_20260704/` | compact | v2.22 gated-lowband N3 microfit preflight, stage summaries, train/eval JSON/CSV, compact run scripts/log, status, and README. Checkpoints, images, arrays, and raw per-image outputs remain off GitHub main. |
| `experiment_logs/haze4k_v2_23_nopost_oof_gated_lowband_train_20260704/` | compact | v2.23 OOF train screen preflight, fold train/eval summaries, OOF gate/closeout JSON, compact run log, status, and README. Checkpoints and per-image eval tables remain on cloud. |
