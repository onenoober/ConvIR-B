...
source experiment spec compiled to schema 6; bounded failure diagnostics and
receipt-bound no-data discard; isolated route workspaces; a deterministic
...
| --- | --- | --- |
| [External dehazing reproductions](family_summaries/external_dehazing_reproduction_summary.md) | DehazeFormer, AECR-Net, and FFA-Net engineering smokes passed with random weights; none establishes an author-weight quality result. | Provenance-check each supplied checkpoint and validate its model/data contract before a quality evaluation. |
...
| --- | --- | --- | --- | --- | --- | --- |
| AECR-Net and FFA-Net engineering smoke | Cloud engineering smoke passed; author checkpoints pending | Official pinned sources; both read Haze4K train pairs, ran finite forward/backward and one Adam step, and strictly reloaded one-step checkpoints. AECR's old DCNv2 binary was replaced by a state-key-compatible torchvision op, without numerical parity certification. | `COMPLETED_GATE_PASS_ENGINEERING_ONLY_RANDOM_WEIGHTS`; locked test untouched, no PSNR/SSIM claim. | [card](experiment_cards/2026-09-21-aecr-ffa-repro.md) | [logs](experiment_logs/aecr_ffa_repro_20260921/) | `github/codex/dehaze-aecr-ffa-repro-20260921` |
| DehazeFormer reproduction smoke | Cloud engineering smoke passed; official checkpoint pending | Official `IDKiro/DehazeFormer` commit `5af7c34d6f8e0784a88a2132a7add296bf99ca9c`; `dehazeformer-s` imported, ran finite CUDA forward, loaded a DataParallel-format random checkpoint, and completed the official one-image `test.py` path. | `COMPLETED_GATE_PASS_ENGINEERING_ONLY_RANDOM_WEIGHTS`; run a new official-weight evaluation after upload; random PSNR/SSIM are not a benchmark claim. | [card](experiment_cards/2026-09-19-dehazeformer-repro-smoke.md) | [logs](experiment_logs/dehazeformer_repro_20260919/) | `github/main` |
...
| --- | ---: | --- |
| `experiment_logs/aecr_ffa_repro_20260921/` | 7 | Cloud smoke command, status, two logs, two JSON reports, and README; no weights, images, or locked-test access. |
| `experiment_logs/dehazeformer_repro_20260919/` | 7 | DehazeFormer official-source provenance, corrected cloud smoke script/status/log, one-image fixture contract, and invalidated first-attempt diagnostics; no weights or image outputs. |
...
| `experiment_logs/haze4k_v5_r3_proposal_first_acv_20260717/` | compact | R3 completed S0, A0, A1 and the independently approved full A2 development OOF. A2 retained positive action-specific signal but failed material gain, retention, C3-over-C1 response increment and severe-risk gates. Terminal decision: `R3_A2_ACV_FULL_OOF_FAIL_STOP`; `authorizes=NONE`. Confirmation, historical A1X-432 outcomes, canary and locked test were not accessed. |
...
- CHD-RM v4a rules out projection order, the tested AdamW proposal correction,
  ...
  default continuation. R3 then repaired and froze the fresh ledger, passed the
  GT-free proposal oracle, and completed the independently amended A1 screen.
  Its sole safe survivor C3 completed the full four-fold/two-seed A2 OOF but
  failed material gain (`+0.006035 dB` LCB95), retention (`0.039107` LCB95),
  C3-over-C1 response increment (`-0.009481 dB` LCB95), and severe-risk gates
  despite passing the shuffle-specificity and structural checks. The frozen
  critic contract is closed as `R3_A2_ACV_FULL_OOF_FAIL_STOP`; it authorizes
  nothing further, and confirmation, canary and locked test remain blocked.
