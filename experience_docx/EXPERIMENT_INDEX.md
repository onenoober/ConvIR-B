...
| [Depth-transmission adapters](family_summaries/dta_family_summary.md) | DTA-v2 was positive but not depth-attributed enough. DTA-v3 R0 failed. DTA-v3.1 airlight/risk/light-hinge failed. DTA-v3.2 C3 improved SafeMix tail to `48/600` but failed hard/SSIM/positive-ratio gates. DTA-v3.3 RouterFusion triage failed. DTA-v3.4 FDF-TSR proved depth attribution but over-acted. DTA-v3.5 conservative FDF reduced worst to `60.5-82.5/600` and exposed strong oracle selector headroom. DTA-v3.6 HRCS formal 5-fold x 3-seed validation confirms L1/L3 oracle high-coverage strict headroom, but deployable selectors still strict-fail due coverage/positive-ratio loss despite relaxed-pass tail control. | Do not run locked Haze4K test from v3.4/v3.5. v3.6 may continue only as a relaxed selector/calibration route under the 2026-06-13 user override; locked test, if reached, must be one fixed-policy exploratory run with no post-test tuning. |
