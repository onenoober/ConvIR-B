...
| [StrongExpert-GainMix](family_summaries/strongexpert_gainmix_family_summary.md) | C0 opened strong capacity via A0/FullUDP endpoint oracle (mean `+0.741695 dB`, hard `+1.110910 dB`, easy `+0.397112 dB`, worst `0/600`), but global FullUDP remains unsafe (`252/600` severe regressions). C1 found a high-gain risk-map row, but it used split/name-param leakage. Corrected C1b A0-PSNR-only OOF replay kept mean `+0.170433 dB` and hard `+0.622967 dB`, but failed strict positive (`0.17`) and abstention-aware dSSIM/tail gates. | C2 is blocked until FullUDP outputs are reacquired/rendered on `convir-4090` and real FullUDP-A0 output-difference/depth/texture/artifact features show leakage-safe separability. Locked test remains blocked. |

## Route Summary

| Route | Status | Main result | Decision | Card | Evidence root | Source after cleanup |
| --- | --- | --- | --- | --- | --- | --- |
| Haze4K v2.0 StrongExpert-GainMix C0-C1b | C1b deployable-proxy audit completed; C2 blocked pending output-difference features | C0a ran on `convir-4090` from commit `885a9c0` with locked test untouched. Global FullUDP is unsafe (`252/600` severe regressions), while A0/FullUDP endpoint oracle is strong (mean `+0.741695 dB`, hard `+1.110910 dB`, easy `+0.397112 dB`, worst `0/600`). C1's best simple policy reached mean `+0.184478 dB` and severe `32/600`, but used validation split membership plus filename-derived metadata and had positive ratio `0.15`, so it is risk-map evidence only. C1b excluded leakage and used A0-PSNR-only OOF threshold replay: mean `+0.170433 dB`, hard `+0.622967 dB`, easy `0.0`, dSSIM `-0.00004448`, positive `0.17`, selected precision `0.653846`, nonnegative `0.91`, severe `46/600`. D8/D9 hygiene also completed with no DTA-v3.7 repair authorization. | `C1B_DEPLOYABLE_PROXY_FAIL_REACQUIRE_OUTPUTDIFF_FEATURES`; do not launch C2 from split/name-param or A0-PSNR-only policies. Reacquire/render FullUDP outputs on `convir-4090` and compute real output-difference/depth/texture/artifact features before router training. | [card](experiment_cards/2026-06-14-haze4k-v2-0-strongexpert-gainmix.md) | [logs](experiment_logs/haze4k_v2_0_strongexpert_gainmix_20260614/) | `github/codex/haze4k-v2-0-strongexpert-gainmix` |
