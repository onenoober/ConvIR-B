...
| [DTA depth-guided transmission adapter](family_summaries/dta_family_summary.md) | Low-gate DTA is complete diagnostic/no-promotion; DTA-v2 calibrated confidence-gated route has synced audit/preflight, adapter-neighbors fold0, and adapter-only five-fold OOF evidence. Adapter-only is positive internally, but depth attribution, SSIM, and tail-regression gates remain unresolved. | Continue multi-seed train-derived OOF controls; do not retune the old low-gate route or select from locked Haze4K test. |
...
| DTA-v2 calibrated confidence-gated adapter | Multi-seed adapter-only running | Adapter-only five-fold OOF20 completed for invert/normal/shuffle/zero over 3000 train-derived validation images: mean dPSNR `+0.0882/+0.0879/+0.0765/+0.0728`, hard `+0.0700/+0.0763/+0.0633/+0.0595`, easy `+0.0716/+0.0697/+0.0649/+0.0630`. Adapter-neighbors fold0 was preservation-negative. The five-fold signal is positive, but depth attribution is weak because zero/shuffle retain most gains and raw `normal` nearly ties `invert`; SSIM is slightly negative and worst regressions remain `421-479/3000`. Multi-seed controls for seeds `3411` and `3413` are running. | `RUNNING_ADAPTER_ONLY_MULTI_SEED_3411_3413`; run all real audit/preflight/training/eval on `convir-4090`, sync text evidence after every stage, complete multi-seed adapter-only controls, and keep locked test blocked until one fixed internal-selected config. | [card](experiment_cards/2026-06-11-haze4k-dta-v2-calibrated.md) | [logs](experiment_logs/haze4k_dta_v2_calibrated_20260611/) | `github/codex/haze4k-dta-v2-calibrated` |
...
| `experiment_logs/haze4k_dta_v2_calibrated_20260611/` | in progress | DTA-v2 route README, convir-4090 setup/audit/preflight/train-eval scripts, synced depth-transmission audit, OOF splits, preflight, fold0 adapter-neighbors/control, adapter-only five-fold OOF evidence, five-fold aggregate bootstrap/Wilcoxon report, and running multi-seed scheduler artifacts. |
...
- DTA-v2 is active on `codex/haze4k-dta-v2-calibrated`: depth-transmission
  audit, preflight, fold0 adapter-only controls, and fold0 adapter-neighbors
  controls are synced, and adapter-only five-fold OOF is complete. Adapter-only
  is internally positive but not locked-test-ready because attribution, SSIM,
  and tail-risk gates remain unresolved; continue multi-seed adapter-only
  controls and do not use locked Haze4K test for selection.
