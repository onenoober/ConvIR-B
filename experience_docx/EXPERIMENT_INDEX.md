...
| Haze4K DTA-v3.7 U-TQS-Mix | Authorized; Phase A table-only first | New mainline route: abandon hard reject as primary policy and use A0-preserving utility-aware soft action bank / shrink-mix over L2/L3/L1, with explicit transmission, airlight, quality, and uncertainty gain-risk modeling. Host probe selected `convir-4090`; `convir-5090` SSH failed. Phase A consumes v3.6 formal OOF rows and must write positive-loss budget, soft-action oracle grid, false reject/accept taxonomy, feature AUC ablations, and T/A/Q/U preflight. | `AUTHORIZED_PHASE_A_TABLE_ONLY_FIRST`; if soft oracle fails, stop policy training; if it passes, launch TQS predictor and real soft-blend/integrated-head validation in parallel on train-derived folds. | [card](experiment_cards/2026-06-13-haze4k-dta-v3-7-u-tqs-mix.md) | [logs](experiment_logs/haze4k_dta_v3_7_u_tqs_mix_20260613/) | `github/codex/haze4k-dta-v3-7-u-tqs-mix` |
...
| `experiment_logs/haze4k_dta_v3_7_u_tqs_mix_20260613/` | pending | DTA-v3.7 U-TQS-Mix route README, convir-4090 Phase A run script/status/log, positive-loss budget, soft action-bank oracle grid, false reject/accept taxonomy, feature ablation AUC report, T/A/Q/U preflight JSON, and Phase A summary. |
