...
| [Depth-transmission adapters](family_summaries/dta_family_summary.md) | DTA-v2 CalGate is positive but not depth-attributed enough for locked test. DTA-v3 R0 failed, while zero-R0 depthDirect train=`invert` is mechanism-positive (`+0.032286 dB` true-vs-zero) but not promotion-ready due hard/SSIM/tail failures. | Reopen/continue only with predeclared wide-gate depthDirect tail/SSIM guards or a new route that preserves true-depth surplus over zero/shuffle/wrong-orientation while improving SSIM/tail. |
...
| Haze4K DTA-v3 DAPC fine-tune | Mechanism-positive depthDirect diagnostic; not promotion-ready | `convir-4090` preflight passed and deterministic eval shuffle was fixed. Phase A R0 and four conservative R0 variants failed, blocking frozen-R0 Phase B. Zero-R0 depthDirect `scout5full` then tested train/eval depth matrices. Train=`invert`, eval=`invert` gave mean dPSNR `+0.013905`, hard `-0.005602`, dSSIM `-0.00002676`, pos ratio `0.5883`, worst `75/600`, but true-vs-zero mean surplus was `+0.032286 dB` with positive surplus ratio `0.625`; other train modes did not show clean attribution. | `COMPLETED_MECHANISM_POSITIVE_NO_PROMOTION_DEPTHDIRECT_INVERT_ONLY`; continue only with wide-gate tail/SSIM-guard depthDirect scouts; locked test blocked. | [card](experiment_cards/2026-06-11-haze4k-dta-v3-dapc.md) | [logs](experiment_logs/haze4k_dta_v3_dapc_20260611/) | `github/codex/haze4k-dta-v3-dapc-finetune` |
...
| `experiment_logs/haze4k_dta_v3_dapc_20260611/` | 158 | DTA-v3 DAPC convir-4090 setup/preflight, R0 and conservative R0 train/eval evidence, depthDirect train/eval depth matrices, attribution CSV/JSON, deterministic shuffle audits, contact-sheet logs/remote paths, tailguard launch scripts, status, README, and tmux transcripts. |
