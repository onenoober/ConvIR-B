...
| [NoPost feature lowband routes](family_summaries/nopost_lowband_family_summary.md) | WLDB-A, v2.18 global pooled WLDB-A2, v2.19 O2 final-only spatial, and v2.20 O3 mid+final/global-context predictor forms are not training-authorized. Later v2.24 diagnosed v2.23 as train-time risk-head collapse, v2.25A showed direct risk soft-label / scale distillation still does not recover trained-gate calibration, and v2.26 showed the current mid/final LL + scalar risk-input stack has weak separability and failed canary/minimal-optimizer trainability. v2.27 ILFRB-ACS passed source/identity and internal multi-stage capacity but its same-sample oracle bank lacked no-op/unsafe separation. v2.28-v2.31 closed the current discrete action-bank selector route. v2.32 proved the separate BILFCF carrier is contract-clean but its S5-only adapter-only canary failed trainability/tail gates. v2.33 found WDMamba teacher source evidence and P2/P3 sanity sufficient to launch P4, but masked teacher-benefit canary32 compressed essentially zero utility. v2.34 then showed the v2.33 first32 canary and a rebuilt table-positive balanced canary both fail crop-aligned direct teacher-benefit gates, so free-tensor projection was not authorized. | Reopen only with a materially changed bounded-field carrier/training design plus a predeclared crop-aligned teacher-positive diagnostic canary, or a materially new target-only action-value signal. Do not expand v2.23, v2.25A, v2.26, v2.27 same-sample bank, v2.28 unsafe prototype bank, v2.29 failed safe-envelope/table-policy bank, v2.30 failed compatibility-gated table-policy bank, v2.31 failed action-value identifiability bank, v2.32 S5-only loss_C adapter-only BILFCF, v2.33 masked WDMamba-alpha0.5 S5-BILFCF compression, or v2.34 table-selected/crop-misaligned teacher canaries by more epochs, folds, samples, selector probes, locked-test use, or simple loss-weight tuning. |
