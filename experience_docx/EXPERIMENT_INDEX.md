...
| [DTA depth-guided transmission adapter](family_summaries/dta_family_summary.md) | Low-gate DTA is complete diagnostic/no-promotion; DTA-v2 calibrated confidence-gated route has synced audit/preflight, fold0 adapter-only/control evidence, fold0 adapter-neighbors evidence, and adapter-only folds `1-2`. Adapter-only remains the internal candidate; adapter-neighbors is preservation-negative. | Continue through train-derived OOF expansion and controls; do not retune the old low-gate route or select from locked Haze4K test. |
...
| DTA-v2 calibrated confidence-gated adapter | Adapter-only f3/f4 running | Fold0 adapter-only OOF20 completed for invert/normal/shuffle/zero on 600 internal-val images: mean dPSNR `+0.1069/+0.1060/+0.0984/+0.0955`, hard `+0.0992/+0.1047/+0.0956/+0.0918`. Fold0 adapter-neighbors stayed only slightly positive in mean (`+0.0151/+0.0151/+0.0097/+0.0072`) while easy/top became negative and worst regressions rose to `142-146`, so adapter-neighbors is not active. Adapter-only folds `1-2` completed; fold0-2 averages are invert mean `+0.0893`, normal mean `+0.0887`, shuffle mean `+0.0774`, zero mean `+0.0736`. | `IN_PROGRESS_ADAPTER_ONLY_F3_F4_RUNNING`; run all real audit/preflight/training/eval on `convir-4090`, sync text evidence after every stage, finish five-fold adapter-only OOF controls, and keep locked test blocked until one fixed internal-selected config. | [card](experiment_cards/2026-06-11-haze4k-dta-v2-calibrated.md) | [logs](experiment_logs/haze4k_dta_v2_calibrated_20260611/) | `github/codex/haze4k-dta-v2-calibrated` |
...
| `experiment_logs/haze4k_dta_v2_calibrated_20260611/` | in progress | DTA-v2 route README, convir-4090 setup/audit/preflight/train-eval scripts, synced depth-transmission audit, OOF splits, preflight, fold0 adapter-only/control, fold0 adapter-neighbors/control, and adapter-only folds `1-2` evidence; adapter-only folds `3-4` are running. |
...
- DTA-v2 is active on `codex/haze4k-dta-v2-calibrated`: depth-transmission
  ...
  controls are synced, and adapter-only folds `1-2` are synced. Adapter-neighbors
  is preservation-negative, so continue adapter-only folds `3-4` plus
  true/zero/shuffle/invert controls; do not use locked Haze4K test for
  selection.
