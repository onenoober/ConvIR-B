...
| [StrongExpert-GainMix](family_summaries/strongexpert_gainmix_family_summary.md) | C0 opened strong A0/FullUDP oracle capacity (mean `+0.741695 dB`, hard `+1.110910 dB`, worst `0/600`) while global FullUDP stayed unsafe. C1/C1b showed leakage-prone or insufficient proxy routing. C1c reacquired/rendered FullUDP on `convir-4090`. C2/C2b/C2c endpoint routers failed OOF preservation/tail gates, but C2d alpha-shrink (`alpha=0.25`) passed strict OOF: mean `+0.332524 dB`, hard `+0.257771 dB`, easy `+0.477047 dB`, dSSIM `+0.000238`, severe `37/600`. | Proceed to C3 train-only shifted validation, then formal 5x3 only if C3 passes. Locked test remains blocked. |

## Route Summary

| Route | Status | Main result | Decision | Card | Evidence root | Source after cleanup |
| --- | --- | --- | --- | --- | --- | --- |
| Haze4K v2.0 StrongExpert-GainMix C0-C2d | C2d alpha-shrink router passed OOF; C3 shifted validation next | C0a ran on `convir-4090` from commit `885a9c0` with locked test untouched. Global FullUDP is unsafe (`252/600` severe regressions), while A0/FullUDP endpoint oracle is strong (mean `+0.741695 dB`, hard `+1.110910 dB`, easy `+0.397112 dB`, worst `0/600`). C1/C1b showed split/name leakage and A0-PSNR-only proxy insufficiency. C1c confirmed FullUDP render readiness. C2 single-threshold endpoint failed easy preservation; C2b multi-rule endpoint failed OOF by easy `-0.033002`; C2c MLP endpoint over-selected tail risk. C2d alpha-shrink passed strict OOF with coverage `0.84`, mean `+0.332524`, hard `+0.257771`, easy `+0.477047`, dSSIM `+0.000238`, selected precision `0.811508`, nonnegative `0.841667`, severe `37/600`. D8/D9 hygiene also completed with no DTA-v3.7 repair authorization. | `C2D_ALPHA_STRICT_SCREEN_PASS_START_C3_SHIFTED`; run C3 train-only shifted validation next. Locked test remains blocked until C3 and formal 5x3 pass. | [card](experiment_cards/2026-06-14-haze4k-v2-0-strongexpert-gainmix.md) | [logs](experiment_logs/haze4k_v2_0_strongexpert_gainmix_20260614/) | `github/codex/haze4k-v2-0-strongexpert-gainmix` |
