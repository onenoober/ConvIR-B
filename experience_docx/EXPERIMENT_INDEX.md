...
| [Depth-transmission adapters](family_summaries/dta_family_summary.md) | DTA-v2 was positive but not depth-attributed enough. DTA-v3 R0 failed. DTA-v3.1 airlight/risk/light-hinge failed. DTA-v3.2 C3 improved SafeMix tail to `48/600` but failed hard/SSIM/positive-ratio gates. DTA-v3.3 RouterFusion triage failed: D1/D2 had strong mean/hard/surplus but worst around `80/600`; D3 was too suppressive/misrouted. DTA-v3.4 FDF-TSR one-shot test is complete: true depth is strongly attributed, but absolute mean/positive-ratio/tail fail (`-0.014802` mean, `0.489` positive ratio, `257/1000` worst). | Do not launch 5-fold x 3-seed formal validation from B0-B4/C/D rows, and do not run additional Haze4K test variants from v3.4 test results. |

## Route Summary

| Route | Status | Main result | Decision | Card | Evidence root | Source after cleanup |
| --- | --- | --- | --- | --- | --- | --- |
| Haze4K DTA-v3.4 FDF-TSR fine-tune | One-shot Haze4K test completed by explicit user override | `E2=e2_tiny_residual` fine-tuned from official A0 on full Haze4K train for 5 epochs on `convir-5090`; true-depth test row is depth-attributed (`true-vs-zero +0.093063`, `true-vs-shuffle +0.106035`, `true-vs-normal +0.138891`) and dSSIM is positive (`+0.00004687`), but mean dPSNR is `-0.014802`, positive ratio `0.489`, and worst regressions `257/1000`. | `COMPLETED_ONE_SHOT_TEST_FAIL_NO_FURTHER_TEST_SELECTION`; no iterative test tuning. | [card](experiment_cards/2026-06-12-haze4k-dta-v3-4-fdf-tsr.md) | [logs](experiment_logs/haze4k_dta_v3_dapc_20260611/) | `github/codex/haze4k-dta-v3-4-fdf-tsr-finetune` |
