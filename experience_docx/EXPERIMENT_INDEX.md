...
| DTA-v2 calibrated confidence-gated adapter | First-stage cloud evidence in progress | Setup/static py_compile and OOF split generation passed on convir-4090; preflight passed (`0.0` no-op diff, `602` loaded / `25` DTA missing keys, DTA grad sum `0.66677364`, finite trans/phys losses). Depth-transmission audit is running; training/eval not started yet. | `IN_PROGRESS_FIRST_STAGE_PREFLIGHT_PASS_AUDIT_RUNNING`; run all real audit/preflight/training/eval on `convir-4090`, sync text evidence after every stage, and keep locked test blocked until one fixed internal-selected config. | [card](experiment_cards/2026-06-11-haze4k-dta-v2-calibrated.md) | [logs](experiment_logs/haze4k_dta_v2_calibrated_20260611/) | `github/codex/haze4k-dta-v2-calibrated` |
