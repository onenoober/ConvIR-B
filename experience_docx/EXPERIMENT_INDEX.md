...
| [Depth-transmission adapters](family_summaries/dta_family_summary.md) | DTA-v2 is positive but not depth-attributed enough. DTA-v3 R0 failed; zero-R0 depthDirect train=`invert` proved surplus (`+0.032286 dB`) but failed tail/SSIM; the first wide-gate tailguard queue over-suppressed the branch. | Continue only with lighter wide-gate guard variants or a new route that preserves true-depth surplus while improving SSIM/tail; locked test remains blocked. |
...
| Haze4K DTA-v3 DAPC fine-tune | Mechanism-positive depthDirect; first tailguard queue failed | `convir-4090` preflight passed and deterministic eval shuffle was fixed. R0 routes failed. Zero-R0 depthDirect train=`invert` gave mean `+0.013905 dB`, hard `-0.005602`, dSSIM `-0.00002676`, worst `75/600`, and true-vs-zero surplus `+0.032286 dB`. The first wide-gate tail/SSIM queue reduced worst regressions to `37..46/600`, but all true means became negative (`-0.014497..-0.040233 dB`) and true-vs-zero surplus fell to `+0.000958..+0.016380 dB`. | `COMPLETED_GATE_FAIL_TAILGUARD_WIDE_GATE_SCOUT`; continue only with lighter wide-gate guard scouts; locked test blocked. | [card](experiment_cards/2026-06-11-haze4k-dta-v3-dapc.md) | [logs](experiment_logs/haze4k_dta_v3_dapc_20260611/) | `github/codex/haze4k-dta-v3-dapc-finetune` |
...
| `experiment_logs/haze4k_dta_v3_dapc_20260611/` | 256 | DTA-v3 DAPC convir-4090 setup/preflight, R0/conservative R0 evidence, depthDirect and wide-gate tailguard train/eval depth matrices, attribution CSV/JSON, deterministic shuffle audits, contact-sheet logs/remote paths, launch scripts, status, README, and tmux transcripts. |
