...
| [NoPost feature lowband routes](family_summaries/nopost_lowband_family_summary.md) | WLDB-A, v2.18 global pooled WLDB-A2, v2.19 O2 final-only spatial, and v2.20 O3 mid+final/global-context predictor forms are not training-authorized. Later v2.24 diagnosed v2.23 as train-time risk-head collapse, and v2.25A showed direct risk soft-label / scale distillation still does not recover trained-gate calibration. | Reopen only with a materially new calibration mechanism that first explains the constant-probability collapse, or a new safety-first NoPost route with no-op/tail gates before training. Do not expand v2.23 or v2.25A by more epochs, folds, samples, or simple loss-weight tuning. |
