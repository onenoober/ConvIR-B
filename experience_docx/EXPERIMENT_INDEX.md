...
| Haze4K DTA-v3 DAPC fine-tune | DTA-v3.2 CTDG-SafeMix audits completed | `convir-4090` preflight passed and deterministic eval shuffle was fixed. R0 routes failed. Tail-lite `wg18_base_s008_b14` kept true-depth mechanism surplus but failed SSIM/tail. DTA-v3.1 confirmed output semantics and ruled out GT-A as rescue. DTA-v3.2 no/low-training audits found alpha-only shrink cannot pass (`alpha=0.75` still below surplus/SSIM/tail gates), while 60% image/patch/pixel oracle action is strongly positive with zero worst regressions. t-error correlation is weak, but low-transmission images concentrate failures; same-fold selector is high quality only at low coverage and nested fold0 thresholding is unstable. | `COMPLETED_CTDG_AUDIT_C0_FAIL_C1_C3_JUSTIFIED_LOCKED_TEST_BLOCKED`; next fixed scout is SafeMix gate/residual training, not 5-fold formal validation; locked test blocked. | [card](experiment_cards/2026-06-11-haze4k-dta-v3-dapc.md) | [logs](experiment_logs/haze4k_dta_v3_dapc_20260611/) | `github/codex/haze4k-dta-v3-dapc-finetune` |
