...
| [Depth-transmission adapters](family_summaries/dta_family_summary.md) | DTA-v2 was positive but not depth-attributed enough. DTA-v3 R0 failed. DTA-v3.1 airlight/risk/light-hinge failed. DTA-v3.2 C3 improved SafeMix tail to `48/600` but failed hard/SSIM/positive-ratio gates. DTA-v3.3 RouterFusion triage failed: D1/D2 had strong mean/hard/surplus but worst around `80/600`; D3 was too suppressive/misrouted. DTA-v3.4 FDF-TSR one-shot test was depth-attributed but failed absolute test quality; follow-up train-derived E1-E4 triage also failed because positive ratio stayed `0.579-0.590` and worst stayed `116-128/600` despite strong surplus. DTA-v3.5 FDF-RCS-Lite is planned to reduce feature action budget and add nested train-derived selector calibration. | Do not launch 5-fold x 3-seed formal validation from B0-B4/C/D/E rows, and do not run additional Haze4K test variants from v3.4 results. v3.5 may run relaxed train-derived diagnostics only; locked test remains blocked. |
...
| Haze4K DTA-v3.5 FDF-RCS-Lite | Planned relaxed train-derived diagnostic | Builds on the anchor-descended v3.4 FDF route after v3.4 proved strong depth attribution but failed tail safety. v3.5 lowers feature fusion strength/gate/bias, adds explicit A0 MSE regression and feature/safe action-budget losses, and produces OOF action tables plus nested selector calibration reports. | `PLANNED_RELAXED_TRAIN_DERIVED_FLOW`; run only on `convir-4090`; locked Haze4K test remains blocked. | [card](experiment_cards/2026-06-12-haze4k-dta-v3-5-fdf-rcs-lite.md) | [logs](experiment_logs/haze4k_dta_v3_5_fdf_rcs_lite_20260612/) | `github/codex/haze4k-dta-v3-5-fdf-rcs-lite` |
...
| `experiment_logs/haze4k_dta_v3_5_fdf_rcs_lite_20260612/` | planned | DTA-v3.5 FDF-RCS-Lite route card, convir-4090 single-run and parallel-launch scripts, relaxed train-derived triage summaries, OOF action table, oracle risk-coverage curve, and nested selector calibration reports. |
