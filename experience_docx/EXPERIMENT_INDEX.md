...
| [Depth-transmission adapters](family_summaries/dta_family_summary.md) | DTA-v2 CalGate is a positive diagnostic but not depth-attributed enough for locked test; DTA-v3 DAPC preflight passed on `convir-4090`, but Phase A R0 OOF20 fold0 failed the safety/quality gate. | Reopen with a redesigned, smaller/conservative R0 Phase A or another predeclared route that proves true-depth surplus over zero/shuffle/wrong-orientation while preserving SSIM/tail. |
...
| Haze4K DTA-v3 DAPC fine-tune | Phase A gate failed; Phase B blocked | `convir-4090` preflight passed: A0 partial load `602` keys, DTA-only missing keys, deterministic eval shuffle audit `same_image_ratio=0.0`. Phase A R0 OOF20 fold0 completed but was negative: mean dPSNR `-0.012119`, hard bottom-25 `-0.069025`, easy top-25 `+0.044643`, mean dSSIM `-0.00001218`, positive ratio `0.45`, strong regressions `48/150`, worst regressions `70/600`. Four conservative `scout5full` R0 variants also failed; best mean was `-0.021837 dB` with hard `-0.056151 dB`. | `COMPLETED_GATE_FAIL_PHASE_A_R0_NO_PHASE_B`; do not launch Phase B from this checkpoint; locked test blocked. | [card](experiment_cards/2026-06-11-haze4k-dta-v3-dapc.md) | [logs](experiment_logs/haze4k_dta_v3_dapc_20260611/) | `github/codex/haze4k-dta-v3-dapc-finetune` |
...
| `experiment_logs/haze4k_dta_v3_dapc_20260611/` | 22 | DTA-v3 DAPC convir-4090 setup/preflight scripts, OOF split and deterministic shuffle audit, Phase A R0 train/eval/tpred logs, compare JSON/CSV, status, README, and tmux transcript. |
