...
| [NoPost feature lowband routes](family_summaries/nopost_lowband_family_summary.md) | WLDB-A, v2.18 global pooled WLDB-A2, v2.19 O2 final-only spatial, and v2.20 O3 mid+final/global-context predictor forms are not training-authorized. Later v2.24 diagnosed v2.23 as train-time risk-head collapse, v2.25A showed direct risk soft-label / scale distillation still does not recover trained-gate calibration, and v2.26 showed the current mid/final LL + scalar risk-input stack has weak separability and failed canary/minimal-optimizer trainability. v2.27 ILFRB-ACS passed source/identity and internal multi-stage capacity but its same-sample oracle bank lacked no-op/unsafe separation. v2.28 OOF prototype replay created no-op/useful/unsafe strata but failed the negative-control unsafe-rate bound. v2.29 bucket-aware safe-envelope calibration preserved selected-policy signal but failed plausible cross-bucket, hard-to-easy, and overstrong safety controls. v2.30 compatibility-gated LCB table policy removed accepted hard-to-easy/cross-bucket risk and preserved restricted-oracle signal, but the deployable GT-free table remained too weak. v2.31 added target-only physics/frequency/internal diagnostics and nested ranking; ranking improved over the table, but useful-action, easy-noop, and tail-safety gates failed, closing the current discrete action-bank selector route. | Reopen only with a separate NoPost bounded internal low-frequency correction-field route or a materially new target-only action-value signal. Do not expand v2.23, v2.25A, v2.26, v2.27 same-sample bank, v2.28 unsafe prototype bank, v2.29 failed safe-envelope/table-policy bank, v2.30 failed compatibility-gated table-policy bank, or v2.31 failed action-value identifiability bank by more epochs, folds, samples, action-joint training, feature/ranker/table micro-tuning, selector probes, locked-test use, or simple loss-weight tuning. |
