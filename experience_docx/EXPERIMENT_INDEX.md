...
| Haze4K v2.26 NoPost Risk Signal Separability Audit | Completed diagnostic; current risk input weak and trainability failed; locked test blocked | P0 invalidated the v2.25A tuple-sort AP artifact: old AP `0.4938`, tie-aware AP `0.1397` near base rate `0.1271`; v2.25A fail remains with probability std `0.001669`, ROC-AUC `0.5501`, and target MAE `0.2488`. P1 replay passed (`missing=0`, v2.21 scalar MLP replay AUC `0.9286`, AP `0.6995`). P2 found current risk features inconclusive/weak: positive-control cached v2.21 scalar MLP AUC `0.9603`, AP `0.7788`, while best current feature `B_final_ll_pooled` had linear AUC `0.6436`, AP `0.2061`. P3 canary32/canary64 both failed at AUC `0.5`, prob std `0.0`; P4 minimal optimizer ablation did not rescue (`best val_auc=0.7407`, prob std `0.0224`). | `V226_DIAGNOSTIC_COMPLETE_CURRENT_RISK_INPUT_WEAK_TRAINABILITY_FAIL_LOCKED_TEST_BLOCKED`; do not continue rescue, action joint training, or locked Haze4K from this route. | [card](experiment_cards/2026-07-04-haze4k-v2-26-nopost-risk-signal-separability-audit.md) | [logs](experiment_logs/haze4k_v2_26_nopost_risk_signal_separability_audit_20260704/) | `github/codex/haze4k-v2-26-nopost-risk-signal-separability-audit` |
...
| `experiment_logs/haze4k_v2_26_nopost_risk_signal_separability_audit_20260704/` | 23 | v2.26 NoPost risk signal separability/trainability diagnostic README, durable cloud launch/monitor scripts, status/logs, P0 tie-aware v2.25A AP recomputation, P1 join/replay audit, P2 feature separability probes, P3 canary trainability curves, P4 minimal optimizer ablation, phase decisions, and closeout JSON. Raw checkpoints and per-image runtime artifacts remain cloud-only. |
