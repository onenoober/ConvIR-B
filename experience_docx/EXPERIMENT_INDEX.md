...
| [StrongExpert-GainMix](family_summaries/strongexpert_gainmix_family_summary.md) | New strong-model mainline opened by C0a: FullUDP endpoint is unsafe globally (`252/600` severe regressions), but A0/FullUDP endpoint oracle has mean `+0.741695 dB`, hard `+1.110910 dB`, easy `+0.397112 dB`, dSSIM `+0.00022958`, and worst `0/600`. Strict positive/intervention coverage is only `0.53`, so C1 risk/correctability mapping is required before router training. | C1 must show high-gain and high-risk cases are separable using train-derived/internal-validation features. Locked test remains blocked until C2 router and C3 shifted validation pass written gates. |

## Route Summary

| Route | Status | Main result | Decision | Card | Evidence root | Source after cleanup |
| --- | --- | --- | --- | --- | --- | --- |
| Haze4K v2.0 StrongExpert-GainMix C0 | C0 capacity audit completed; C1 authorized | C0a ran on `convir-4090` from commit `885a9c0` with locked test untouched. FullUDP endpoint all-scope mean is only `+0.062005 dB` and easy top-25 is `-0.686496 dB` with `252/600` severe regressions, so global FullUDP remains blocked. The A0/FullUDP endpoint oracle has mean `+0.741695 dB`, hard bottom-25 `+1.110910 dB`, easy top-25 `+0.397112 dB`, dSSIM `+0.00022958`, nonnegative ratio `1.0`, and worst `0/600`; strict positive/intervention coverage is `0.53`. D8/D9 hygiene also completed: D8 metric record is usable but metadata retained D7 labels, and D9 forensic is failure attribution only. | `C0_CAPACITY_OPEN_POSITIVE_COVERAGE_RISK_MAP_REQUIRED`; proceed to C1 risk/correctability mapping, not direct router promotion or DTA-v3.7 repair. | [card](experiment_cards/2026-06-14-haze4k-v2-0-strongexpert-gainmix.md) | [logs](experiment_logs/haze4k_v2_0_strongexpert_gainmix_20260614/) | `github/codex/haze4k-v2-0-strongexpert-gainmix` |
