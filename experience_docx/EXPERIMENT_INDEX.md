...
| [StrongExpert-GainMix](family_summaries/strongexpert_gainmix_family_summary.md) | v2.1 sealed C10 `riskcap36_no075` passed formal 5x3 but failed its single locked one-shot. v2.2 C8-Mini then preregistered the minimal multi-expert complementarity proof, but stopped at asset preflight: WDMamba, FSNet+UDP, and MB-TaylorFormerV2-L Haze4K checkpoints/result packages were unavailable on `convir-4090`; locked test remained untouched. | `C8_STOP_PREFLIGHT_FAILED_ENGINEERING_ASSET_UNAVAILABLE`. No C9 router/MoE, no locked run, no distillation, and no scientific complementarity claim until reproducible Haze4K expert assets with sha256 are supplied. |

## Route Summary

| Route | Status | Main result | Decision | Card | Evidence root | Source after cleanup |
| --- | --- | --- | --- | --- | --- | --- |
| Haze4K v2.2 C8-Mini Multi-Expert Complementarity Oracle | Asset preflight completed; no expert rendering | C8-0 through C8-3 ran on `convir-4090` with locked test untouched. Source repos for WDMamba and MB-TaylorFormerV2 were cloned/audited, current UDPNet/FullUDP was available, and FSNet+UDP source was compared to current ConvIR+UDP. Required Haze4K checkpoints/result packages for WDMamba, FSNet+UDP, and MB-TaylorFormerV2-L were not present; Baidu shared-link probes returned landing/auth pages. FSNet+UDP is not source-identical to current ConvIR+UDP (`ConvIR_UDPNet.py` sha256 `038c349e...`, `FSNet_UDPNet.py` sha256 `c0f0769...`), but no checkpoint exists for render-diff duplicate proof. | `C8_STOP_PREFLIGHT_FAILED_ENGINEERING_ASSET_UNAVAILABLE`; no C9 router authorization and no complementarity claim. Resume C8-1 only after a reproducible WDMamba Haze4K checkpoint/result package is available and hashed. | [card](experiment_cards/2026-06-15-haze4k-v2-2-c8-mini-expert-oracle.md) | [logs](experiment_logs/haze4k_v2_2_c8_mini_expert_oracle_20260615/) | `github/codex/haze4k-v2-2-c8-mini-expert-oracle` |
