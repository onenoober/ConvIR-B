...
risk-controlled shrink/mix router failed OOF plus train-heldout gates. v1.8
closed the BiDPFM1 fusion-neighbor capacity route as negative. v1.9 confirmed
large patch-level teacher/oracle headroom, but the tested deployable mask head
and trainable conditional student failed gates.
...
- Cards:
  ...
  - `../experiment_cards/2026-06-06-haze4k-convir-v1-9-conditional-teacher-guided.md`
- Evidence roots:
  ...
  - `../experiment_logs/haze4k_v19_conditional_teacher_guided_20260606/`
...
| ConvIR-Dehaze-v1.9-ConditionalTeacherGuided | Completed the planned conditional teacher-guided queue without early stop: physical priors were available, pre-router and post-expert predictability were mean-positive but tail-unsafe, patch alpha oracle was very strong (`+1.6086 dB` mean, `+1.4500 dB` hard bottom-25, `+1.6249 dB` easy top-25, worst/strong ratios `0`), patch mask head failed tail gates, and the 3-seed conditional student collapsed (`-1.0591 dB` regular mean, `-0.6418 dB` hard mean, hard bottom-25 `-0.6731 dB`). | `MULTISEED_SCREEN_FAIL_CONTINUE_OTHER_EXPERIMENTS`; keep patch oracle evidence as mechanism-positive, but do not promote or continue the current mask/student implementation. Locked test remained blocked. |
...
v1.9 moves the family one level above the v1.8 capacity screen: it tests whether
UDPNet can be used as a conditional teacher at patch/region level. The result is
diagnostically important. The patch alpha oracle is much stronger than the
image-level v1.6/v1.7 oracles and has no tail regressions, so the expert signal
is not exhausted. However, the deployable mask head still over-opens risky
regions, and the trainable conditional student sharply degrades both regular
and hard splits. This is not a promotion route; it is evidence that the next
reopen must first explain the student collapse and improve tail-calibrated
patch selection before another training queue.
...
- Do not continue the current v1.9 conditional student or mask-head form by
  adding seeds, epochs, or optimizer variants alone. The patch oracle is a
  mechanism asset, but the current deployable training path failed by a large
  margin.
...
v1.9 satisfies the next teacher-guided diagnostic requirement and narrows the
reopen condition further: a follow-up must either produce a tail-safe
patch/region policy from train-derived data or diagnose and fix why the
conditional student diverged from the strong patch oracle. Threshold polishing,
extra seeds, or optimizer-only sweeps are not enough.
