...
| DTA-v2 calibrated confidence-gated | Setup/static py_compile, OOF splits, preflight, and depth-transmission audit passed on convir-4090. Audit found reversed cached-depth orientation (`depth` Spearman about `-0.93`, `1-depth` about `+0.93`), so `invert` is the calibrated primary run and `normal/zero/shuffle` are controls. | `IN_PROGRESS_AUDIT_PREFLIGHT_PASS_TRAIN_QUEUE`; execute on `convir-4090` and keep locked Haze4K test blocked until one fixed internal-selected configuration. |
