...
| DTA-v2 calibrated confidence-gated | Audit/preflight passed; adapter-only fold0 scout5 controls completed. Internal 128-image dPSNR: invert `+0.0589`, normal `+0.0642`, shuffle `+0.0355`, zero `+0.0243`; hard gains only for invert/normal. Mechanism attribution remains unresolved, so continue to full-fold OOF20 controls. | `IN_PROGRESS_SCOUT5_CONTROLS_DONE_OOF20_RUNNING_NEXT`; execute on `convir-4090` and keep locked Haze4K test blocked until one fixed internal-selected configuration. |
