...
| DTA-v2 calibrated confidence-gated | Setup/static py_compile, OOF splits, and preflight passed on convir-4090; depth-transmission audit is running. Preflight no-op diff `0.0`, partial load `602/25`, DTA grad sum `0.66677364`, finite trans/phys losses. | `IN_PROGRESS_FIRST_STAGE_PREFLIGHT_PASS_AUDIT_RUNNING`; execute on `convir-4090` and keep locked Haze4K test blocked until one fixed internal-selected configuration. |
