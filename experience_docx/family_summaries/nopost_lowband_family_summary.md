...
Status: WLDB-A and the tested v2.18/v2.19/v2.20 deployable lowband predictor
forms are not training-authorized. NoPost lowband remains directionally open
only for a new route that fixes safety/no-op calibration and tail preservation
before training.
...
v2.19 O2 final-only spatial predictor, and v2.20 O3 mid+final/global-context
predictor as training candidates. Do not close the underlying lowband-capacity
direction, but treat it as requiring a materially new safety/no-op/tail route.
...
WLDB-A rerun or a direct v2.19/v2.20 training launch. It should use the v2.17
O2/O3 oracle headroom and the v2.20 mechanism-positive context result as
capacity evidence, keep the source-clean and identity contract requirements,
and make no-op calibration, p05/CVaR/severe, strong/easy preservation,
