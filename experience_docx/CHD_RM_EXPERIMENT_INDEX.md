...
Status: v3n conservative first-step label preflight closed at fail-stop. v3m
showed that block16 common-action oracle value and deployable local label
observability are real, but the frozen A2 calibrated policy converts them into
unsafe image-level tails. v3n then tested a stricter false-intervention
semantics using only `.125 -> .25` and a fixed 99th-percentile train-negative
`direct_step_energy` threshold; it selected zero held-out blocks and therefore
did not authorize replay. No route-confirm audit, canary expansion, locked-test
access, controller training, learned ranker, or policy deployment is
authorized.
