...
- Intervention or factor contrast and reference: for every tile, choose one common action for D_ref/D_rep from no-op, positive-full and negative-full. An active action is eligible only if its local PSNR gain is at least `+0.005 dB` under both operators; among eligible actions choose the largest worse-operator local gain, with ties favoring no-op, positive, then negative. Compare this region oracle with the best whole-image action satisfying nonnegative image gain under both operators and with the mean of 16 name-seeded permutations of the same regional action histogram.
...
- Primary gate, uncertainty and threshold source: PASS requires exact R5 whole-image target replay, complete 384x2 units, exact nonoverlapping 8x8 partitions, every selected active tile gain `>=+0.005 dB` under both operators, pooled region gain LCB95 `>=+0.020 dB`, pooled region-minus-safe-global and region-minus-shuffle LCB95 each `>=+0.005 dB`, region-minus-global CVaR5 LCB95 `>=-0.005 dB`, zero region severe/hard images, and fold0/fold1 point estimates separately meeting the same `+0.020/+0.005/+0.005 dB` mean-gain lines. A mixed image must allocate at least 10% of pixels each to no-op and active actions; a bidirectional image must allocate at least 5% each to positive and negative actions. Their exact-binomial image-fraction LCB95 values must be `>=0.25` and `>=0.10`, respectively. Margins predate R10 in R4/R5/R9; area and support fractions are prespecified material-support thresholds.
...
- Matched baseline and budget: region/global/shuffle share exact images, operators, three actions and rendered pixels. The global baseline chooses one dual-operator-safe action per image; each shuffle permutes only among tiles with identical pixel counts, preserving both the region map's exact action-tile histogram and exact action-pixel-area histogram. Region support is measured by exact pixel area rather than tile count. Work is fixed at 768 cache units, 64 tiles, 16 shuffles and 4,000 bootstrap draws.
