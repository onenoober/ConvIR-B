...
- Cloud runtime: `convir-4090`
  `/sda/home/wangyuxin/ConvIR-B/repos/ConvIR-B-haze4k-v5-v2f-chd-rm-need-target-head-redesign-f4-044b7798`.
...
F4 density-stratified frozen-side heads completed on `convir-4090` with status
`COMPLETED_GATE_FAIL`. ConvIR-B remained frozen, D3 remained frozen, D2 was not
run, RARM was not connected or trained, v3 was not run, and locked Haze4K test
usage was `none`.

F4 did not find a variant that satisfied the original v2e safety/LDHN contract:
all selected variants had `safe_and_ldhn_points = 0`. The high-LDHN variants
raised unsafe false-tail responses, while safer global variants kept LDHN
recall below the gate.

Because F4 could have failed from insufficient tail pressure, F4b ran a bounded
tail-rescue matrix. F4b also completed with status `COMPLETED_GATE_FAIL`:
...
Final decision: keep v2f paused. Do not run F5 controls, v3 no-op RARM audit,
RARM connection/training, D2, or locked Haze4K test from this state. A future
continuation would need a new route decision that changes the target semantics
or available information, not another strength sweep of this F4/F4b family.
