...
Plan the route as the fewest decision packages that preserve scientific
ownership and written authorization:

1. one `R3` decision package freezes the route card's question, estimand,
   design, gates, evidence roles, stop rules, and first-stage authorization;
2. one `R2` build package creates the fresh local route workspace, completes
   the frozen card's operational fields, and implements the operations manifest,
   tracked runner, and all static/pre-smoke checks without changing scientific
   semantics;
3. one `R1` stage package performs an exact authorized start, runner-owned
   pre-smoke, the stage, and typed closeout validation; adjacent authorized
   stages may remain in the same qualified task only when each transition is
   mechanically gated by the prior closeout;
4. one persistent `R0`/`R1` observation package owns healthy long-running
   monitoring; never create a dispatcher child for each poll;
5. one `R3` terminal package interprets the formal gate, followed by one
   unchanged-verdict archive package when archival is large enough to
   amortize a separate `R2` task.

These are task packages, not mandatory experiment stages. Omit any package
that has no work. Do not split one warm package into preflight, launch,
monitor, fetch, and sync children merely because each operation can be named
separately. Do not combine packages across an engineering repair, scientific
interpretation, changed authorization, or locked-test boundary.
...
- deterministic input-name, native-shape, grouping, and batch-compatibility
  inventory needed by the runner's integrated pre-smoke;
- exact no-op/neutral-action construction, canonicalization, and tolerance
  checks before any expensive stage can start;
...
tuple fields. It also seals the workspace policy, GPU resource floor, monitor
profile, heartbeat timeout, and closeout provenance fields. Those values are
reviewed once in the route card and are not regenerated by monitoring tasks.
...
| --- | --- |
| `audit_evaluation` | static contract -> integrated pre-smoke/smoke -> formal evaluation |
| `feasibility_oracle` | integrated integrity check -> privileged upper-bound analysis -> stop or authorize learnability work |
| `paired_single_intervention_training` | static contract -> integrated pre-smoke/smoke -> matched scout/hard gate -> independent formal decision budget |
| `factorial_screening` | integrated integrity/smoke -> decision-relevant factor cells -> interaction-aware selection -> independent frozen-candidate confirmation |
| `adaptive_decision` | common integrated pre-smoke -> preregistered branch trigger -> only the authorized branch -> independent confirmation when promotion is claimed |
| `policy_replay` | integrated integrity smoke -> out-of-fold development/replay -> freeze full operator/policy -> sealed confirmation only if authorized |
...
- cloud code, run-output, evidence-stage, runner, status, and closeout paths are
  named;
- expected phase-level wall time, runner heartbeat cadence, stale-heartbeat
  limit, monitor profile, and maximum model-visible observation budget are
  written;
- pre-smoke runs inside the tracked runner before expensive work and checks the
  route-specific shape/no-op/finite/identity conditions in one launch;
