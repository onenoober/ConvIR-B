...
This module is intentionally standalone.  It accepts only the concatenated
15-channel deployable tensors named by the frozen route card and returns a
zero-initialized, bounded three-channel exact-half correction.
"""
...
class A1X_ACCESS_Head(nn.Module):
    """One fixed 24/48/96 global nonlinear head on the exact-half lattice."""

    input_channels = 15
    output_channels = 3

    def __init__(self, support=1.0):
        super().__init__()
    ...
    def forward(self, deployable_tensors):
        ...
        context = self.global_projection(x96).expand(-1, -1, x24.shape[-2], x24.shape[-1])
        up48 = torch.nn.functional.interpolate(x96, size=x48.shape[-2:], mode="bilinear", align_corners=False)
        d48 = self.decoder_48(torch.cat((up48, x48), dim=1))
        up24 = torch.nn.functional.interpolate(d48, size=x24.shape[-2:], mode="bilinear", align_corners=False)
        d24 = self.decoder_24(torch.cat((up24, x24, context), dim=1))
        return torch.tanh(self.final_projection(d24)) * self.support
