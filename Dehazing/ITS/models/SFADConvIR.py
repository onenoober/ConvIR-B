...
class SpatialDegradationFieldModulation(nn.Module):
    ...
    def forward(self, image, fused, condition):
        ...
        degradation_field = self.field(torch.cat([image_feature, fused, condition], dim=1))
        gamma, beta = self.affine(degradation_field).chunk(2, dim=1)
        out = fused * (1 + self.alpha * torch.tanh(gamma)) + self.alpha * torch.tanh(beta)
        self._last_stats = self._summarize(degradation_field, gamma, beta)
        return out

    def _summarize(self, degradation_field, gamma, beta):
        ...
        return {
            ...
            'beta_min': float(beta.min().cpu()),
            'beta_max': float(beta.max().cpu()),
...
def build_sfad_sdfm_net(version, data, fam_mode='original'):
    if fam_mode != 'original':
        raise ValueError("SFAD-SDFM route starts from fam_mode='original'.")
    return SFADSDFMConvIR(version, data)
...
def configure_sfad_train_scope(model, scope):
    ...
    for name, param in model.named_parameters():
        trainable = scope == 'all' or name.startswith('SFAD_')
