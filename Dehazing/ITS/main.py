...
def load_init_model(model, args):
    ...
    if args.arch == 'sfad_sdfm_gst':
        report = load_haze4k_partial(
            model,
            args.init_model,
            allowed_new_prefixes=('SFAD_',),
        )
...
def main(args):
    ...
    if args.arch == 'sfad_sdfm_gst':
        model = build_sfad_sdfm_gst_net(args.version, args.data, args.fam_mode)
    else:
    ...
    if args.arch == 'sfad_sdfm_gst' and args.mode == 'train':
        scope_report = configure_sfad_train_scope(model, args.sfad_train_scope)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', default='official_convir', choices=['official_convir', 'convir', 'sfad_sdfm_gst'], type=str)
    ...
    if args.arch not in ('official_convir', 'convir', 'sfad_sdfm_gst'):
    ...
    if not os.path.exists(args.model_save_dir):
        os.makedirs(args.model_save_dir)
    for source in ('models/layers.py', 'models/ConvIR.py', 'models/SFADConvIR.py', 'models/SFADGSTConvIR.py', 'models/SFADSDFMGSTConvIR.py', 'data/data_load.py', 'data/data_augment.py', 'train.py', 'valid.py', 'eval.py', 'main.py'):
