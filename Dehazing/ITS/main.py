...
from models.SDCConvIR import (
    build_sdc_lite_net,
    configure_sdc_train_scope,
    load_haze4k_partial,
)
...
def load_init_model(model, args):
    ...
    if args.arch == 'sdc_lite':
        report = load_haze4k_partial(
            model,
            args.init_model,
            allowed_new_prefixes=('SFAD_SDC',),
        )
...
def main(args):
    ...
    if args.arch == 'sdc_lite':
        model = build_sdc_lite_net(args.version, args.data, args.fam_mode)
    else:
    ...
    if args.arch == 'sdc_lite' and args.mode == 'train':
        scope_report = configure_sdc_train_scope(model, args.sfad_train_scope)
        print('SFAD_TRAIN_SCOPE ' + json.dumps(scope_report, sort_keys=True))
    if args.mode == 'train':
        _train(model, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', default='official_convir', choices=['official_convir', 'convir', 'sdc_lite'], type=str)
    parser.add_argument('--sfad_train_scope', default='all', choices=['adapter_only', 'all'], type=str)
    ...
    if args.arch not in ('official_convir', 'convir', 'sdc_lite'):
    ...
    if not os.path.exists(args.model_save_dir):
        os.makedirs(args.model_save_dir)
    for source in ('models/layers.py', 'models/ConvIR.py', 'models/SDCConvIR.py', 'data/data_load.py', 'data/data_augment.py', 'train.py', 'valid.py', 'eval.py', 'main.py'):
