...
def load_init_model(model, args):
    ...
    if args.arch == 'sfad_gst':
        report = load_haze4k_partial(
            model,
            args.init_model,
            allowed_new_prefixes=('SFAD_GST',),
        )
...
def main(args):
    ...
    if args.arch == 'sfad_gst':
        model = build_sfad_gst_net(args.version, args.data, args.fam_mode)
    else:
    ...
    if args.arch == 'sfad_gst' and args.mode == 'train':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', default='official_convir', choices=['official_convir', 'convir', 'sfad_gst'], type=str)
    ...
    if args.arch not in ('official_convir', 'convir', 'sfad_gst'):
    ...
    if not os.path.exists(args.model_save_dir):
        os.makedirs(args.model_save_dir)
    for source in ('models/layers.py', 'models/ConvIR.py', 'models/SFADGSTConvIR.py', 'data/data_load.py', 'data/data_augment.py', 'train.py', 'valid.py', 'eval.py', 'main.py'):
