...
from models.DCFSBConvIR import (
    build_dcfsb_bottleneck_net,
    configure_dcfsb_train_scope,
    load_haze4k_partial,
)
...
def load_init_model(model, args):
    ...
    if args.arch == 'dcfsb_bottleneck':
        report = load_haze4k_partial(
            model,
            args.init_model,
            allowed_new_prefixes=('DCFSB_Bottleneck',),
        )
        print('INIT_MODEL_PARTIAL_LOAD ' + json.dumps(report, sort_keys=True))
        return
...
def main(args):
    ...
    if args.arch == 'dcfsb_bottleneck':
        model = build_dcfsb_bottleneck_net(args.version, args.data, args.fam_mode)
    else:
        model = build_official_net(args.version, args.data, args.fam_mode)
    # print(model)

    if torch.cuda.is_available():
        model.cuda()
    load_init_model(model, args)
    if args.arch == 'dcfsb_bottleneck' and args.mode == 'train':
        scope_report = configure_dcfsb_train_scope(model, args.dcfsb_train_scope)
        print('DCFSB_TRAIN_SCOPE ' + json.dumps(scope_report, sort_keys=True))
    if args.mode == 'train':
        _train(model, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', default='official_convir', choices=['official_convir', 'convir', 'dcfsb_bottleneck'], type=str)
    parser.add_argument('--dcfsb_train_scope', default='all', choices=['adapter_only', 'all'], type=str)
    parser.add_argument('--train_split_file', default='', type=str)
    ...
    if args.arch not in ('official_convir', 'convir', 'dcfsb_bottleneck'):
        raise ValueError('Unsupported architecture route.')
    ...
    if not os.path.exists(args.model_save_dir):
        os.makedirs(args.model_save_dir)
    for source in ('models/layers.py', 'models/ConvIR.py', 'models/DCFSBConvIR.py', 'data/data_load.py', 'data/data_augment.py', 'train.py', 'valid.py', 'eval.py', 'main.py'):
